...
void SpMV_VNEC_D_FP32(perfSpB_vector y, const perfSpB_matrix A, const perfSpB_vector x, VNEC_D_FP32 *mat_thd)
{
    perfSpB_matrix_CSC_or_CSR *a = (perfSpB_matrix_CSC_or_CSR *)(A);
...
#pragma omp parallel for schedule(static) num_threads(NUM_THREADS)
    for (int tid = 0; tid < NUM_THREADS; tid++)
    {
        ...
        for (int i = thread_coord_start.x; i < thread_coord_end.x; ++i)
        {
            perfSpB_index ptr_start = mat_thd->v_row_ptr[i];
            perfSpB_index n_one_line = mat_thd->v_row_ptr[i + 1] - ptr_start;
            __m512 v_tmp = _mm512_setzero_ps();
            for (perfSpB_index j = 0; j < n_one_line; j++)
            {
                perfSpB_index col = ptr_start + j;
...
#pragma omp parallel for schedule(static) num_threads(NUM_THREADS)
    ...
    for (perfSpB_index i = 0; i < nvals + nLanes_f64; i++)
    {
...
void SpMV_VNEC_D_FP64(perfSpB_vector y, const perfSpB_matrix A, const perfSpB_vector x, VNEC_D_FP64 *mat_thd)
{
    perfSpB_matrix_CSC_or_CSR *a = (perfSpB_matrix_CSC_or_CSR *)(A);
    perfSpB_vector_Dense *yy = (perfSpB_vector_Dense *)(y);
    perfSpB_vector_Dense *xx = (perfSpB_vector_Dense *)(x);
