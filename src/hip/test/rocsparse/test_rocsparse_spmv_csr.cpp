...
// #include <rocsparse.h>           // cusparseSpMV
#include <hipsparse.h>
...
void PerfSpB_mxv_FP32_serial_rocsparse(perfSpB_vector w, const perfSpB_matrix A, const perfSpB_vector u)
{
...
inline int Perf_test_hipsparse_spmv_csr_fp32(const std::string &input_path, const std::string &result_path, bool iscsc)
{
    ...
    std::vector<int> temp_csrOffsets(A_num_rows + 1);
    std::vector<int> temp_columns(A_nnz);
    ...
    {
        if (a->ptr[i] > INT_MAX || a->ptr[i] < INT_MIN)
        {
            std::cerr << "Index out of bounds for int type at position " << i << std::endl;
            return EXIT_FAILURE; // 或者处理溢出情况
        }
        temp_csrOffsets[i] = static_cast<int>(a->ptr[i]);
    }

    for (size_t i = 0; i < A_nnz; ++i)
    {
        if (a->indices[i] > INT_MAX || a->indices[i] < INT_MIN)
        {
            std::cerr << "Index out of bounds for int type at position " << i << std::endl;
            return EXIT_FAILURE; // 或者处理溢出情况
        }
        temp_columns[i] = static_cast<int>(a->indices[i]);
    }
    ...
    // CHECK_HIP(hipMalloc((void **)&dA_csrOffsets, (A_num_rows + 1) * sizeof(perfSpB_index)))
    // CHECK_HIP(hipMalloc((void **)&dA_columns, A_nnz * sizeof(perfSpB_index)))
    CHECK_HIP(hipMalloc((void **)&dA_csrOffsets, (A_num_rows + 1) * sizeof(int)))
    CHECK_HIP(hipMalloc((void **)&dA_columns, A_nnz * sizeof(int)))
    CHECK_HIP(hipMalloc((void **)&dA_values, A_nnz * sizeof(float)))
    CHECK_HIP(hipMalloc((void **)&dX, A_num_cols * sizeof(float)))
    CHECK_HIP(hipMalloc((void **)&dY, A_num_rows * sizeof(float)))

    // CHECK_HIP(hipMemcpy(dA_csrOffsets, hA_rows, (A_num_rows + 1) * sizeof(perfSpB_index), hipMemcpyHostToDevice))
    // CHECK_HIP(hipMemcpy(dA_columns, hA_columns, A_nnz * sizeof(perfSpB_index), hipMemcpyHostToDevice))
    CHECK_HIP(hipMemcpy(dA_csrOffsets, temp_csrOffsets.data(), (A_num_rows + 1) * sizeof(int), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(dA_columns, temp_columns.data(), A_nnz * sizeof(int), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(dA_values, a->val, A_nnz * sizeof(float), hipMemcpyHostToDevice))
    CHECK_HIP(hipMemcpy(dX, hX, A_num_cols * sizeof(float), hipMemcpyHostToDevice))
    CHECK_HIP(hipMemcpy(dY, hY, A_num_rows * sizeof(float), hipMemcpyHostToDevice))
    ...
    printf("\n hipsparse SpMV Performance  = %lf GFLOPS\n \n", gflops);

    printf("\nmatrix has %ld nnz \n", A_nnz);

    // Copy result back to host
    CHECK_HIP(hipMemcpy(hY, dY, sizeof(float) * nrows, hipMemcpyDeviceToHost));
    ...
    {
        PerfSpB_mxv_FP32_serial_rocsparse(result_benchmark, G, x);
    }
    ...
    for (perfSpB_index i = 0; i < A_num_rows; ++i)
    {
        ...
        if ((abs(hY[i] - value_benchmark)) > value_benchmark * 1e-4)
...
void PerfSpB_mxv_FP64_serial_rocsparse(perfSpB_vector w, const perfSpB_matrix A, const perfSpB_vector u)
{
    perfSpB_vector_Dense *ww = (perfSpB_vector_Dense *)(w);
    perfSpB_vector_Dense *uu = (perfSpB_vector_Dense *)(u);
...
inline int Perf_test_hipsparse_spmv_csr_fp64(const std::string &input_path, const std::string &result_path, bool iscsc)
{
    std::vector<perfSpB_index> row_indices;
    std::vector<perfSpB_index> col_indices;
    ...
    CHECK_HIP(hipMalloc((void **)&dA_csrOffsets, (A_num_rows + 1) * sizeof(int)))
    CHECK_HIP(hipMalloc((void **)&dA_columns, A_nnz * sizeof(int)))
    CHECK_HIP(hipMalloc((void **)&dA_values, A_nnz * sizeof(double)))
    CHECK_HIP(hipMalloc((void **)&dX, A_num_cols * sizeof(double)))
    CHECK_HIP(hipMalloc((void **)&dY, A_num_rows * sizeof(double)))

    // CHECK_HIP(hipMemcpy(dA_csrOffsets, hA_rows, (A_num_rows + 1) * sizeof(perfSpB_index), hipMemcpyHostToDevice))
    // CHECK_HIP(hipMemcpy(dA_columns, hA_columns, A_nnz * sizeof(perfSpB_index), hipMemcpyHostToDevice))
    CHECK_HIP(hipMemcpy(dA_csrOffsets, temp_csrOffsets.data(), (A_num_rows + 1) * sizeof(int), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(dA_columns, temp_columns.data(), A_nnz * sizeof(int), hipMemcpyHostToDevice));
    CHECK_HIP(hipMemcpy(dA_values, a->val, A_nnz * sizeof(double), hipMemcpyHostToDevice))
    CHECK_HIP(hipMemcpy(dX, hX, A_num_cols * sizeof(double), hipMemcpyHostToDevice))
    CHECK_HIP(hipMemcpy(dY, hY, A_num_rows * sizeof(double), hipMemcpyHostToDevice))
    ...
    // Copy result back to host
    CHECK_HIP(hipMemcpy(hY, dY, sizeof(double) * nrows, hipMemcpyDeviceToHost));
    ...
    for (perfSpB_index i = 0; i < 1; i++)
    {
        PerfSpB_mxv_FP64_serial_rocsparse(result_benchmark, G, x);
    }
...
/*
TEST(test_perf_hipsparse_spmv_csr_FP64, 3Dspectralwave)
{
    std::string input_file(MATRIX_FULL_PATH(3Dspectralwave ));
    std::string output_file(RESULT_FULL_PATH(3Dspectralwave ));
    Perf_test_hipsparse_spmv_csr_fp64(input_file, output_file, false);
}

TEST(test_perf_hipsparse_spmv_csr_FP64, adaptive)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_shell10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, amazon_2008)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, apache2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, asia_osm)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ASIC_680ks)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ASIC_680k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, as_Skitter)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, atmosmodd)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, atmosmodj)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, atmosmodl)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, atmosmodm)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, audikw_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, belgium_osm)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bone010)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, boneS10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cage14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cage15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, channel_500x100x100_b050)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, circuit5M_dc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, circuit5M)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cit_Patents)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, coPapersDBLP)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, debr)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dielFilterV2clx)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dielFilterV2real)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dielFilterV3real)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ecology1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ecology2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Emilia_923)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, eu_2005)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Fault_639)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, flickr)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Freescale1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, FullChip)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G3_circuit)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Geo_1438)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, germany_osm)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, great_britain_osm)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gsm_106857)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Hamrle3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Hook_1498)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hugebubbles_00000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hugebubbles_00010)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hugebubbles_00020)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hugetrace_00000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hugetrace_00010)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hugetrace_00020)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hugetric_00000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hugetric_00010)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hugetric_00020)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, in_2004)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, italy_osm)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, kkt_power)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, kron_g500_logn20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ldoor)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ljournal_2008)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lp1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, memchip)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, netherlands_osm)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nlpkkt120)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nlpkkt80)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, packing_500x100x100_b050)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, parabolic_fem)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, patents)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pre2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat29)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat31)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rgg_n_2_20_s0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rgg_n_2_21_s0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rgg_n_2_22_s0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, road_central)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, roadNet_CA)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, roadNet_PA)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, roadNet_TX)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, road_usa)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Serena)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, soc_LiveJournal1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Stanford_Berkeley)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, StocF_1465)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t2em)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, thermal2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tmt_sym)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tmt_unsym)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, venturiLevel3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wb_edu)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, webbase_1M)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, web_BerkStan)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, web_Google)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wikipedia_20051105)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wikipedia_20060925)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wikipedia_20061104)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wikipedia_20070206)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wiki_Talk)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 1138_bus)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 144)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 2cubes_sphere)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 2D_27628_bjtcai)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 2D_54019_highK)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 3D_28984_Tetra)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 3D_51448_3D)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 3Dspectralwave2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 3dtube)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 3elt_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 3elt)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 494_bus)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 598a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 662_bus)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, 685_bus)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, a0nsdsil)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, a2nnsnsl)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, a5esindl)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ABACUS_shell_hd)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ABACUS_shell_ld)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ABACUS_shell_md)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ABACUS_shell_ud)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, add20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, add32)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_27)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_28)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_29)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_31)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_32)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_33)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_34)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_35)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_36)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_37)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_38)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_39)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_40)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_41)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_42)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_43)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_44)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_45)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_46)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_47)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_48)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_49)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_50)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_51)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_52)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_53)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_54)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_55)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_56)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_57)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_58)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_59)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_60)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_61)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_62)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_63)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_64)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_65)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_66)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_67)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_68)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_dcop_69)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_trans_01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, adder_trans_02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_0_k101)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_1_k101)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af23560)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_2_k101)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_3_k101)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_4_k101)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_5_k101)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_shell1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_shell2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_shell3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_shell4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_shell5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_shell6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_shell7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_shell8)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, af_shell9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, aft01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, aft02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, airfoil1_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, airfoil1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, airfoil_2d)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Alemdar)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, amazon0302)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, amazon0312)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, amazon0505)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, amazon0601)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Andrews)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, apache1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, appu)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, as_22july06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, as_735)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, as_caida)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ash292)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ASIC_100ks)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ASIC_100k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ASIC_320ks)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ASIC_320k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, astro_ph)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, aug2dc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, aug2d)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, aug3dcqp)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, aug3d)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, auto)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, av41092)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, b2_ss)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barrier2_10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barrier2_11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barrier2_12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barrier2_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barrier2_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barrier2_3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barrier2_4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barrier2_9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barth4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barth5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, barth)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Baumann)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bauru5727)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bayer01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bayer02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bayer03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bayer04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bayer05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bayer06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bayer07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bayer08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bayer09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bayer10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bbmat)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcircuit)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcspwr04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcspwr05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcspwr06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcspwr07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcspwr08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcspwr09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcspwr10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk27)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk28)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk29)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk31)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk32)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk33)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk34)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk35)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk36)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk37)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk38)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstk39)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm27)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm34)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm35)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm36)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm37)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm38)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bcsstm39)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, b_dyn)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, BenElechi1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, benzene)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bfly)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bfwa398)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bfwa782)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bfwb398)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bfwb782)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bibd_81_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, big_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, big)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, biplane_9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bips07_1693)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bips07_1998)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bips07_2476)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bips07_3078)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bips98_1142)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bips98_1450)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bips98_606)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, blckhole)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, blockqp1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bloweya)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bloweybl)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bloweybq)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bmw3_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bmw7st_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bmwcra_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bodyy4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bodyy5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bodyy6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, boneS01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, boyd1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, boyd2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bp_0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bp_1000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bp_1200)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bp_1400)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bp_1600)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bp_200)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bp_400)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bp_600)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bp_800)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, brack2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, brainpc2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bratu3d)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bundle1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, bwm2000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_27)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_28)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_29)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_31)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_32)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_33)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_34)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_35)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_36)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_37)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_38)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_39)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_40)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_41)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_42)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_43)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_44)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_45)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_46)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_47)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_48)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_49)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_50)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_51)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_52)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_53)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_54)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_55)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_56)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_57)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_58)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_59)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_60)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_61)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_62ghs)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_62)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_63)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_64b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_64)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_65)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_66b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_66)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_67b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_67)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_68)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_69)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_70)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_71)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_72)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_73b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_73)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ca_AstroPh)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ca_CondMat)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cage10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cage11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cage12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cage13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cage7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cage8)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cage9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cage)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, CAG_mat1916)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, CAG_mat364)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ca_GrQc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ca_HepPh)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ca_HepTh)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, caidaRouterLevel)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, California)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_1054)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_1072)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_144)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_161)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_187)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_229)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_256)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_268)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_292)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_445)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_634)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_715)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, can_838)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cant)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, case39)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, case9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cavity26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, c_big)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cbuckle)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cca)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ccc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cdde1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cdde2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cdde3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cdde4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cdde5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cdde6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cegb2802)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cegb2919)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cegb3024)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cegb3306)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, celegans_metabolic)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, celegansneural)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cell1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cell2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cfd1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cfd2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ch5_5_b3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ch7_8_b5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Chebyshev1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Chebyshev2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Chebyshev3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Chebyshev4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Chem97ZtZ)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, chem_master1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, chipcool0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, chipcool1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, circuit_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, circuit204)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, circuit_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, circuit_3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, circuit_4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, citationCiteseer)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cit_HepPh)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cit_HepTh)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ck400)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ck656)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ckt11752_dc_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ckt11752_tr_0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cnr_2000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, coater1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, coater2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, coAuthorsCiteseer)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, coAuthorsDBLP)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, commanche_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, comsol)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cond_mat_2003)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cond_mat_2005)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cond_mat)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf5_0_4x4_10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf5_0_4x4_14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf5_0_4x4_18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf5_0_4x4_22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf5_0_4x4_26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf5_4_8x8_05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf5_4_8x8_10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf5_4_8x8_15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf5_4_8x8_20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf6_0_4x4_20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf6_0_4x4_30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf6_0_8x8_20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf6_0_8x8_30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, conf6_0_8x8_80)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, consph)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cont_201)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cont_300)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cop20k_A)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, coPapersCiteseer)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, copter1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, copter2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, CO)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, coupled)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crack_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crack)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crankseg_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crankseg_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crashbasis)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crplat2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cryg10000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cryg2500)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crystk01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crystk02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crystk03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crystm01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crystm02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, crystm03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cs4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, CSphd)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ct20stif)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cti)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cvxbqp1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cvxqp3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, cyl6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, darcy003)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, data)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dawson5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dblp_2010)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dc1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dc2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dc3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, delaunay_n19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, denormal)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, descriptor_xingo6u)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, diag)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dictionary28)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dielFilterV3clx)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dixmaanl)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, DK01R)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, d_pretok)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dtoc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Dubcova1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Dubcova2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Dubcova3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dw1024)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dw2048)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dw256A)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dw256B)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dw4096)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dw8192)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwa512)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwb512)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwg961a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwg961b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_1005)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_1007)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_1242)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_162)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_193)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_198)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_209)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_221)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_245)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_2680)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_307)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_310)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_346)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_361)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_419)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_492)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_503)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_512)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_592)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_607)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_758)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_869)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_878)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_918)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, dwt_992)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, e40r0100)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, EAT_RS)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, EAT_SR)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ecl32)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, email_Enron)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, email_EuAll)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, email)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, engine)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, enron)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, EPA)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, epb0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, epb1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, epb2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, epb3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Erdos02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Erdos971)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Erdos972)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Erdos981)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Erdos982)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Erdos991)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Erdos992)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, eris1176)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, eurqsa)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, EVA)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex10hs)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex27)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex28)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex29)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex31)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex32)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex33)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex35)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex36)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex37)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex3sta1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex40)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, EX5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex8)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ex9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, exdata_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, extr1b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, extr1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, F1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, F2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, FA)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fcondp2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fd12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fd15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fd18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fe_4elt2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fe_body)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, FEM_3D_thermal1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, FEM_3D_thermal2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fem_filter)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fem_hifreq_circuit)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fe_ocean)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fe_rotor)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fe_sphere)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fe_tooth)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, filter2D)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, filter3D)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, finan512)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, finance256)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, flowmeter0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, flowmeter5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, foldoc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ford1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ford2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_27)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_28)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_29)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_31)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_32)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_33)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_34)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_35)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_36)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_37)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_38)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_39)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_40)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_41)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_42)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_43)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_44)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_45)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_46)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_47)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_48)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_49)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_50)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_dcop_51)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_trans_01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fpga_trans_02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fp)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_183_3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_183_4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_541_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_541_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_541_3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_541_4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_680_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_680_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_680_3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_760_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_760_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fs_760_3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fullb)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fv1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fv2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fv3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fxm3_6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, fxm4_6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G27)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G28)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G29)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G2_circuit)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G31)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G32)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G33)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G34)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G35)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G36)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G37)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G38)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G39)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g3rmt3m3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G40)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G41)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G42)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G43)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G44)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G45)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G46)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G47)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G48)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G49)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G50)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G51)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G52)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G53)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G54)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G55)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G56)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G57)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G58)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G59)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G60)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G61)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G62)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G63)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G64)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G65)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G66)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G67)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac010sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac010)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac020sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac020)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac040sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac040)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac050sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac060sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac060)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac080sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac080)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac100sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac100)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac120sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac120)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac140sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac140)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac160sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac160)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac180sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac180)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac200sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, g7jac200)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G8)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, G9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Ga10As10H30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Ga19As19H42)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Ga3As3H12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Ga41As41H72)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, GaAsH6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, garon1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, garon2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gas_sensor)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, GD00_c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, GD06_Java)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, GD96_a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Ge87H76)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Ge99H100)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gearbox)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gemat11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gemat12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, geom)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, goodwin)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gr_30_30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, graham1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gre_1107)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gre_343)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gre_512)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, grid2_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, grid2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gridgena)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, GT01R)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gupta1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gupta2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gupta3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gyro_k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gyro_m)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, gyro)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, H2O)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, halfb)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Hamrle2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Harvard500)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hcircuit)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, heart1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, heart2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, heart3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, helm2d03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, helm3d01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, HEP_th_new)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hep_th)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hood)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hor_131)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, HTC_336_4438)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, HTC_336_9129)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, human_gene1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, human_gene2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hvdc1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hvdc2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hydr1c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, hydr1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ibm_matrix_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, igbt3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Ill_Stokes)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, impcol_c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, impcol_d)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, impcol_e)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, init_adder1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, inlet)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, inline_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ins2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, internet)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, invextr1_new)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, iprob)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jagmesh1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jagmesh2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jagmesh3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jagmesh4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jagmesh5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jagmesh6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jagmesh7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jagmesh8)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jagmesh9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac020sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac020)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac040sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac040)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac060sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac060)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac080sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac080)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac100sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac100)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac120sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jan99jac120)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jazz)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jnlbrng1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, jpwh_991)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, juba40k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, k1_san)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, k3plates)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Kaufhold)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, kim1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, kim2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Kohonen)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, kron_g500_logn16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, kron_g500_logn17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, kron_g500_logn18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, kron_g500_logn19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Kuu)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, L_9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, laminar_duct3D)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, language)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, largebasis)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, laser)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Lederberg)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, LF10000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, LFAT5000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr04c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr07c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr10c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr11c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr14c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr17c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr34c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr34)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr71c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lhr71)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, light_in_tissue)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Lin)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, linverse)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, li)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lns_3937)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lns_511)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lnsp3937)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lnsp_511)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lock1074)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lock2232)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lock3491)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lock_700)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lpl1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp1009)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp1270)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp1561)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp1882)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp2233)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp2614)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp_265)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp3025)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp3466)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp_406)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp_577)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lshp_778)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, L)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lund_a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lund_b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lung1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, lung2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, luxembourg_osm)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, M10PI_n1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, M10PI_n)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, m133_b3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, m14b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, M20PI_n1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, M20PI_n)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, m3plates)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, M40PI_n1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, M40PI_n)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, M80PI_n1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, M80PI_n)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mac_econ_fwd500)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mahindas)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, majorbasis)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, man_5976)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mario001)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mario002)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac020sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac020)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac040sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac040)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac060sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac060)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac080sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac080)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac100sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac100)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac120sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac120)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac140sc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mark3jac140)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, matrix_9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, matrix_new_3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mbeacxc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mbeaflw)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mbeause)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mc2depi)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mcca)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mcfe)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, meg1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, meg4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, memplus)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mesh2e1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mesh2em5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mesh3e1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mesh3em5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mhd1280a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mhd1280b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mhd3200a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mhd3200b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mhd4800a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mhd4800b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mhda416)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mhdb416)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mimo28x28_system)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mimo46x46_system)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mimo8x8_system)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, minnesota)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, minsurfo)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mip1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mixtank_new)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mono_500Hz)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mouse_gene)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mplate)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, msc00726)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, msc01050)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, msc01440)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, msc04515)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, msc10848)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, msc23052)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, msdoor)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, m_t1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mult_dcop_01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mult_dcop_02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, mult_dcop_03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Muu)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, n3c6_b7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Na5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nasa1824)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nasa2146)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nasa2910)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nasa4704)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nasasrb)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ncvxbqp1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ncvxqp1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ncvxqp3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ncvxqp5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ncvxqp7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ncvxqp9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nd12k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nd24k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nd3k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nd6k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nemeth26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, net100)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, net125)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, net150)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, net25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, net4_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, net50)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, net75)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, netscience)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, netz4504_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, netz4504)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nmos3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nnc1374)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nnc261)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nnc666)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nopoly)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nopss_11k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nos2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nos3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nos5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nos6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, nos7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, NotreDame_www)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, NotreDame_yeast)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ns3Da)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, obstclae)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, odepa400)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ODLIS)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, offshore)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ohne2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oilpan)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, olafu)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, olesnik0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, olm1000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, olm2000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, olm5000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, olm500)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, onera_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, onetone1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, onetone2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, OPF_10000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, OPF_3754)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, OPF_6000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, opt1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, orani678)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Oregon_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Oregon_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, orsirr_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, orsirr_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, orsreg_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_27)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_28)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_29)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_31)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_32)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_33)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_34)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_35)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_36)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_37)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_38)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_39)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_40)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_41)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_42)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_43)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_44)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_45)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_46)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_47)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_48)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_49)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_50)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_51)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_52)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_53)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_54)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_55)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_56)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_dcop_57)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, oscil_trans_01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, p2p_Gnutella04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, p2p_Gnutella05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, p2p_Gnutella06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, p2p_Gnutella08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, p2p_Gnutella09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, p2p_Gnutella24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, p2p_Gnutella25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, p2p_Gnutella30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, p2p_Gnutella31)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, para_10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, para_4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, para_5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, para_6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, para_7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, para_8)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, para_9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, patents_main)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pattern1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pcrystk02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pcrystk03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pct20stif)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pdb1HYS)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pde225)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pde2961)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pde900)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pds10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Pd)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pesa)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pf2177)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pfinan512)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, PGPgiantcompo)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, piston)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pkustk14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, plat1919)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, plat362)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, plbuckle)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pli)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, plsk1919)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, plskz362)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, poisson2D)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, poisson3Da)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, poisson3Db)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, polblogs)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, poli3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, poli4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, poli_large)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, poli)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pores_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pores_3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, powersim)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, power)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, PR02R)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Pres_Poisson)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, problem1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, psmigr_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, psmigr_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, psmigr_3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pwtk)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, pwt)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, qa8fk)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, qa8fm)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, qc2534)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, qc324)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, qh1484)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, qh768)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, qh882)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, qpband)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, radfr1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, raefsky1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, raefsky2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, raefsky3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, raefsky4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, raefsky5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, raefsky6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rail_1357)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rail_20209)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rail_5177)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rail_79841)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Raj1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat03)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat04)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat05)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat06)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat07)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat08)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat09)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat11)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat13)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat14)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat17)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat20)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat21)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat22)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat23)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat24)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat26)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat27)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rajat28)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ramage02)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rbsa480)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rbsb480)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb1250l)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb1250)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb200l)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb200)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb2048_noL)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb2048)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb3200l)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb450l)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb450)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb5000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb800l)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdb968)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdist1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdist2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rdist3a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Reuters911)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, RFdevice)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rgg_n_2_15_s0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rgg_n_2_16_s0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rgg_n_2_17_s0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rgg_n_2_18_s0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rgg_n_2_19_s0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rim)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, RM07R)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rma10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Roget)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rotor2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rw136)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rw496)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, rw5151)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, S10PI_n1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, S10PI_n)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, s1rmq4m1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, s1rmt3m1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, S20PI_n1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, S20PI_n)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, s2rmq4m1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, s2rmt3m1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, s3dkq4m2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, s3dkt3m2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, s3rmq4m1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, s3rmt3m1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, s3rmt3m3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, S40PI_n1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, S40PI_n)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, s4dkt3m2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, S80PI_n1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, S80PI_n)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, saylr1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, saylr3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, saylr4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, SciMet)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, scircuit)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, se)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shallow_water1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shallow_water2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shar_te2_b3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sherman1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sherman2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sherman3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sherman4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sherman5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shermanACa)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shermanACb)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shermanACd)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ship_001)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ship_003)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shipsec1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shipsec5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shipsec8)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shl_0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shl_200)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shl_400)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shock_9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shuttle_eddy)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shyy161)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, shyy41)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Si10H16)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Si2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Si34H36)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Si41Ge41H72)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Si5H12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Si87H76)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Sieber)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, SiH4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, SiNa)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sinc12)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sinc15)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sinc18)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, SiO2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, SiO)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sit100)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, skirt)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, SmaGri)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sme3Da)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sme3Db)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sme3Dc)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, smt)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, soc_Epinions1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, soc_sign_epinions)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, soc_sign_Slashdot081106)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, soc_sign_Slashdot090216)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, soc_sign_Slashdot090221)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, soc_Slashdot0811)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, soc_Slashdot0902)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sparsine)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sphere3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, spiral)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, spmsrtls)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, srb1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sstmodel)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Stanford)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, std1_Jac2_db)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, std1_Jac2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, std1_Jac3_db)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, std1_Jac3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, steam1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, steam2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, stokes128)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, stokes64s)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, stokes64)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, stomach)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, str_0)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, str_200)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, str_400)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, str_600)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, struct3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, struct4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, sts4098)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, stufe_10)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, stufe)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, swang1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, swang2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t2dah_a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t2dah_e)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t2dah)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t2dal_a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t2dal_bci)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t2dal_e)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t2dal)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t2d_q4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t2d_q9)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t3dh_a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t3dh_e)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t3dh)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t3dl_a)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t3dl_e)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t3dl)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t520)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, t60k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tandem_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tandem_vtx)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ted_AB)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ted_AB_unscaled)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ted_A)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ted_A_unscaled)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ted_B)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ted_B_unscaled)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, thermal1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, thermal)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, thermomech_dK)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, thermomech_dM)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, thermomech_TC)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, thermomech_TK)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, thread)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tols1090)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tols2000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tols340)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tols4000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tomography)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, torsion1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, torso1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, torso2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, torso3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, trans4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, trans5)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, transient)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, trdheim)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Trefethen_150)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Trefethen_20000b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Trefethen_20000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Trefethen_2000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Trefethen_200b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Trefethen_200)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Trefethen_300)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Trefethen_500)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Trefethen_700)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, troll)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSC_OPF_1047)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSC_OPF_300)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b162_c1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b162_c3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b162_c4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b300_c1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b300_c2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b300_c3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b300)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b39_c19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b39_c30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b39_c7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b9_c1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_FS_b9_c6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b162_c1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b162_c3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b162_c4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b2052_c1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b2383_c1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b2383)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b300_c1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b300_c2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b300_c3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b39_c19)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b39_c30)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b39_c7)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b678_c1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b678_c2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TSOPF_RS_b9_c6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, TS)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tsyl201)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tub1000)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tube1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tube2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tuma1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, tuma2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, turon_m)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, twotone)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ukerbe1_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ukerbe1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, uk)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, USAir97)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, USpowerGrid)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, usroads_48)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, usroads)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, utm1700b)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, utm300)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, utm3060)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, utm5940)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, vanbody)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, venkat01)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, venkat25)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, venkat50)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, vfem)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, vibrobox)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, viscoplastic1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, viscoplastic2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, viscorocks)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wang1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wang2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wang3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wang4)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, water_tank)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wathen100)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wathen120)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, watt_1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, watt_2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, waveguide3D)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wave)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wb_cs_stanford)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, web_NotreDame)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, web_Stanford)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, west0381)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, west0479)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, west0497)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, west0655)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, west0989)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, west1505)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, west2021)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, whitaker3_dual)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, whitaker3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wiki_Vote)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, windscreen)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wing_nodal)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, wing)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Wordnet3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, ww_vref_6405)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, x104)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, xenon1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, xenon2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, xingo3012)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, xingo_afonso_itaipu)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, yeast)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, young1c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, young2c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, young3c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, young4c)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Zd_Jac2_db)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Zd_Jac2)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Zd_Jac3_db)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Zd_Jac3)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Zd_Jac6_db)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Zd_Jac6)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, zenios)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, zeros_nopss_13k)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Zewail)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Zhao1)
{
...
TEST(test_perf_hipsparse_spmv_csr_FP64, Zhao2)
{
