...
// #define DATA_PATH "/data/share/opengraphblas/graph/"
// #define DATA_PATH "/home/wangluhan/code/TPDS/open-graph-blas/data/graph/"
...
void PerfSpB_mxv_FP32_serial_cusparse(perfSpB_vector w, const perfSpB_matrix A, const perfSpB_vector u)
{
...
void PerfSpB_mxv_FP64_serial_cusparse(perfSpB_vector w, const perfSpB_matrix A, const perfSpB_vector u)
{
    perfSpB_vector_Dense *ww = (perfSpB_vector_Dense *)(w);
    perfSpB_vector_Dense *uu = (perfSpB_vector_Dense *)(u);
...
inline int Perf_test_cusparse_spmv_csr_fp32(const std::string &input_path, const std::string &result_path, bool iscsc)
{
    // Builder<perfSpB_index, perfSpB_index> builder(input_path);
    // CSRGraph<perfSpB_index, perfSpB_index> g = builder.MakeGraph();

    // perfSpB_index A_num_rows, A_num_cols, A_nnz;

    // A_num_rows = static_cast<perfSpB_index>(g.num_nodes());
    // A_num_cols = static_cast<perfSpB_index>(g.num_nodes());
    // A_nnz = static_cast<perfSpB_index>(g.num_edges());

    // perfSpB_index *hA_rows;
    // perfSpB_index *hA_columns;
    // float *hA_values = new float[A_nnz];

    // hA_rows = new perfSpB_index[A_nnz];
    // hA_columns = g.out_destValues();
    // perfSpB_index *offsets = g.out_offsets();
    // perfSpB_index index = 0;
    // for (perfSpB_index i = 0; i < (A_num_rows); i++)
    // {
    ...
    //         if (index >= (A_nnz))
    //         {
    //             std::cerr << "Error index >= A_nnz" << index << " " << A_nnz << std::endl;
    //             exit(-1);
    //         }
    //         hA_values[index] = (float)(6);
    //         hA_rows[index++] = static_cast<perfSpB_index>(i);
    //     }
    // }
    ...
    perfSpB_index* hA_columns = col_indices.data();
    perfSpB_index* hA_rows = row_indices.data();
    float* hA_values = values.data();

    perfSpB_index A_num_rows = nrows;
    perfSpB_index A_num_cols = ncols;
    perfSpB_index A_nnz = nvals;
    ...
    perfSpB_info info;
    SAFE_CALL(perfSpB_matrix_new(&G, perfSpB_FP32, A_num_rows, A_num_cols));
    perfSpB_matrix_build_FP32(G, hA_rows, hA_columns, hA_values, A_nnz);
    ...
    for (perfSpB_index i = 0; i < A_num_cols; ++i)
    {
        hX[i] = (float)(i+(i % 66));
    }
    ...
    // printf("\n hA_rows[]=%ld,%ld,%ld,%ld,%ld,%ld \n",a->ptr[0],a->ptr[1],a->ptr[2],a->ptr[3],a->ptr[4],a->ptr[5]);
    // printf("\n hA_columns[]=%ld,%ld,%ld,%ld,%ld,%ld \n",hA_columns[0],hA_columns[1],hA_columns[2],hA_columns[3],hA_columns[4],hA_columns[5]);
    // printf("\n s_columns[]=%ld,%ld,%ld,%ld,%ld,%ld \n",a->indices[0],a->indices[1],a->indices[2],a->indices[3],a->indices[4],a->indices[5]);
    // printf("\n hA_values[]=%f,%f,%f,%f,%f,%f \n",hA_values[0],hA_values[1],hA_values[2],hA_values[3],hA_values[4],hA_values[5]);
    // printf("\n hX[]=%f,%f,%f,%f,%f,%f \n",hX[0],hX[1],hX[2],hX[3],hX[4],hX[5]);

    //--------------------------------------------------------------------------
    // Device memory management
    perfSpB_index *dA_csrOffsets, *dA_columns;
    float *dA_values, *dX, *dY;
    CHECK_CUDA(cudaMalloc((void **)&dA_csrOffsets,
                          (A_num_rows + 1) * sizeof(perfSpB_index)))
    CHECK_CUDA(cudaMalloc((void **)&dA_columns, A_nnz * sizeof(perfSpB_index)))
    CHECK_CUDA(cudaMalloc((void **)&dA_values, A_nnz * sizeof(float)))
    CHECK_CUDA(cudaMalloc((void **)&dX, A_num_cols * sizeof(float)))
    CHECK_CUDA(cudaMalloc((void **)&dY, A_num_rows * sizeof(float)))

    CHECK_CUDA(cudaMemcpy(dA_csrOffsets, a->ptr,
                          (A_num_rows + 1) * sizeof(perfSpB_index),
                          cudaMemcpyHostToDevice))
    CHECK_CUDA(cudaMemcpy(dA_columns, a->indices, A_nnz * sizeof(perfSpB_index),
                          cudaMemcpyHostToDevice))
    CHECK_CUDA(cudaMemcpy(dA_values, a->val, A_nnz * sizeof(float),
                          cudaMemcpyHostToDevice))
    ...
    cuda_time_test_end();
    double runtime = (elapsedTime) / ntest;
    double gflops = (2.0 * A_nnz) / ((runtime / 1000) * 1e9);
    cudaDeviceSynchronize();
    // printf("\n cusparse SpMV CUDA kernel runtime = %g ms\n", runtime);
    printf("\n cusparse SpMV Performance  = %lf GFLOPS\n \n", gflops);

    printf("\nmatrix has %ld nnz \n", A_nnz);

    perfSpB_vector x;
    SAFE_CALL(perfSpB_vector_new(&x, perfSpB_FP32, A_num_cols));
    for (perfSpB_index j = 0; j < A_num_cols; j++)
    {
        perfSpB_vector_set_element_FP32(x, hX[j], j);
    }

    perfSpB_vector result_benchmark;
    SAFE_CALL(perfSpB_vector_new(&result_benchmark, perfSpB_FP32, A_num_rows));

    for (perfSpB_index i = 0; i < 1; i++)
    {
        PerfSpB_mxv_FP32_serial_cusparse(result_benchmark, G, x);
    }
    ...
    for (perfSpB_index i = 0; i < A_num_rows; ++i)
    {
        float value_benchmark;
        perfSpB_vector_extract_element_FP32(&value_benchmark, result_benchmark, i);
        // printf("\n cuda = %f and cpu = %f \n", hY[i],value_benchmark);
        if ((hY[i] - value_benchmark) > value_benchmark * 1e-4)
...
inline int Perf_test_cusparse_spmv_csr_fp64(const std::string &input_path, const std::string &result_path, bool iscsc)
{
    std::vector<perfSpB_index> row_indices;
    std::vector<perfSpB_index> col_indices;
    ...
    perfSpB_matrix G = NULL;
    perfSpB_info info;
    SAFE_CALL(perfSpB_matrix_new(&G, perfSpB_FP64, A_num_rows, A_num_cols));
    perfSpB_matrix_build_FP64(G, hA_rows, hA_columns, hA_values, A_nnz);
    perfSpB_matrix_CSC_or_CSR *a = (perfSpB_matrix_CSC_or_CSR *)(G);
    for (int i = 0; i < A_num_rows; i++)
    {
        if ((a->ptr[i + 1] - a->ptr[i]) < 0)
    ...
    double *hX = new double[A_num_cols];
    double *hY = new double[A_num_rows];
    for (perfSpB_index i = 0; i < A_num_rows; ++i)
    {
        hY[i] = 0;
    }
    ...
    CHECK_CUDA(cudaMemcpy(dA_columns, a->indices, A_nnz * sizeof(perfSpB_index),
                          cudaMemcpyHostToDevice))
    CHECK_CUDA(cudaMemcpy(dA_values, a->val, A_nnz * sizeof(double),
                          cudaMemcpyHostToDevice))
    ...
    perfSpB_vector x;
    SAFE_CALL(perfSpB_vector_new(&x, perfSpB_FP64, A_num_cols));
    for (perfSpB_index j = 0; j < A_num_cols; j++)
    {
        perfSpB_vector_set_element_FP64(x, hX[j], j);
    }

    perfSpB_vector result_benchmark;
    SAFE_CALL(perfSpB_vector_new(&result_benchmark, perfSpB_FP64, A_num_rows));

    for (perfSpB_index i = 0; i < 1; i++)
    {
        PerfSpB_mxv_FP64_serial_cusparse(result_benchmark, G, x);
    }
...
TEST(test_perf_cusparse_spmv_csr_FP64, 3Dspectralwave)
{
    std::string input_file(MATRIX_FULL_PATH(3Dspectralwave ));
    std::string output_file(RESULT_FULL_PATH(3Dspectralwave ));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}

TEST(test_perf_cusparse_spmv_csr_FP64, adaptive)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_shell10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, amazon_2008)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, apache2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, asia_osm)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ASIC_680ks)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ASIC_680k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, as_Skitter)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, atmosmodd)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, atmosmodj)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, atmosmodl)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, atmosmodm)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, audikw_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, belgium_osm)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bone010)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, boneS10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cage14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cage15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, channel_500x100x100_b050)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, circuit5M_dc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, circuit5M)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cit_Patents)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, coPapersDBLP)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, debr)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dielFilterV2clx)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dielFilterV2real)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dielFilterV3real)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ecology1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ecology2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Emilia_923)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, eu_2005)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Fault_639)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, flickr)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Freescale1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, FullChip)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G3_circuit)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Geo_1438)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, germany_osm)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, great_britain_osm)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gsm_106857)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Hamrle3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Hook_1498)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hugebubbles_00000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hugebubbles_00010)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hugebubbles_00020)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hugetrace_00000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hugetrace_00010)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hugetrace_00020)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hugetric_00000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hugetric_00010)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hugetric_00020)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, in_2004)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, italy_osm)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, kkt_power)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, kron_g500_logn20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ldoor)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ljournal_2008)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lp1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, memchip)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, netherlands_osm)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nlpkkt120)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nlpkkt80)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, packing_500x100x100_b050)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, parabolic_fem)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, patents)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pre2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat29)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat31)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rgg_n_2_20_s0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rgg_n_2_21_s0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rgg_n_2_22_s0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, road_central)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, roadNet_CA)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, roadNet_PA)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, roadNet_TX)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, road_usa)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Serena)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, soc_LiveJournal1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Stanford_Berkeley)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, StocF_1465)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t2em)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, thermal2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tmt_sym)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tmt_unsym)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, venturiLevel3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wb_edu)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, webbase_1M)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, web_BerkStan)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, web_Google)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wikipedia_20051105)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wikipedia_20060925)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wikipedia_20061104)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wikipedia_20070206)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wiki_Talk)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 1138_bus)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 144)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 2cubes_sphere)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 2D_27628_bjtcai)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 2D_54019_highK)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 3D_28984_Tetra)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 3D_51448_3D)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 3Dspectralwave2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 3dtube)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 3elt_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 3elt)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 494_bus)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 598a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 662_bus)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, 685_bus)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, a0nsdsil)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, a2nnsnsl)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, a5esindl)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ABACUS_shell_hd)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ABACUS_shell_ld)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ABACUS_shell_md)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ABACUS_shell_ud)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, add20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, add32)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_27)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_28)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_29)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_31)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_32)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_33)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_34)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_35)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_36)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_37)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_38)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_39)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_40)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_41)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_42)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_43)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_44)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_45)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_46)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_47)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_48)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_49)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_50)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_51)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_52)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_53)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_54)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_55)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_56)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_57)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_58)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_59)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_60)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_61)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_62)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_63)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_64)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_65)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_66)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_67)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_68)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_dcop_69)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_trans_01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, adder_trans_02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_0_k101)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_1_k101)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af23560)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_2_k101)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_3_k101)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_4_k101)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_5_k101)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_shell1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_shell2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_shell3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_shell4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_shell5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_shell6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_shell7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_shell8)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, af_shell9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, aft01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, aft02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, airfoil1_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, airfoil1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, airfoil_2d)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Alemdar)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, amazon0302)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, amazon0312)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, amazon0505)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, amazon0601)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Andrews)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, apache1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, appu)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, as_22july06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, as_735)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, as_caida)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ash292)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ASIC_100ks)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ASIC_100k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ASIC_320ks)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ASIC_320k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, astro_ph)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, aug2dc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, aug2d)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, aug3dcqp)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, aug3d)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, auto)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, av41092)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, b2_ss)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barrier2_10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barrier2_11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barrier2_12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barrier2_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barrier2_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barrier2_3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barrier2_4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barrier2_9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barth4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barth5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, barth)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Baumann)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bauru5727)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bayer01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bayer02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bayer03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bayer04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bayer05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bayer06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bayer07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bayer08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bayer09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bayer10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bbmat)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcircuit)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcspwr04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcspwr05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcspwr06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcspwr07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcspwr08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcspwr09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcspwr10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk27)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk28)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk29)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk31)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk32)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk33)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk34)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk35)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk36)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk37)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk38)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstk39)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm27)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm34)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm35)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm36)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm37)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm38)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bcsstm39)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, b_dyn)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, BenElechi1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, benzene)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bfly)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bfwa398)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bfwa782)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bfwb398)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bfwb782)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bibd_81_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, big_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, big)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, biplane_9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bips07_1693)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bips07_1998)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bips07_2476)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bips07_3078)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bips98_1142)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bips98_1450)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bips98_606)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, blckhole)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, blockqp1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bloweya)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bloweybl)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bloweybq)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bmw3_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bmw7st_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bmwcra_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bodyy4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bodyy5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bodyy6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, boneS01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, boyd1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, boyd2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bp_0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bp_1000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bp_1200)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bp_1400)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bp_1600)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bp_200)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bp_400)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bp_600)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bp_800)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, brack2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, brainpc2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bratu3d)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bundle1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, bwm2000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_27)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_28)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_29)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_31)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_32)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_33)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_34)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_35)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_36)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_37)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_38)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_39)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_40)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_41)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_42)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_43)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_44)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_45)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_46)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_47)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_48)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_49)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_50)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_51)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_52)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_53)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_54)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_55)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_56)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_57)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_58)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_59)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_60)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_61)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_62ghs)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_62)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_63)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_64b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_64)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_65)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_66b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_66)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_67b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_67)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_68)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_69)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_70)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_71)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_72)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_73b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_73)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ca_AstroPh)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ca_CondMat)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cage10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cage11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cage12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cage13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cage7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cage8)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cage9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cage)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, CAG_mat1916)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, CAG_mat364)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ca_GrQc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ca_HepPh)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ca_HepTh)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, caidaRouterLevel)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, California)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_1054)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_1072)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_144)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_161)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_187)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_229)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_256)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_268)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_292)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_445)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_634)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_715)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, can_838)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cant)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, case39)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, case9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cavity26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, c_big)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cbuckle)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cca)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ccc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cdde1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cdde2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cdde3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cdde4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cdde5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cdde6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cegb2802)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cegb2919)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cegb3024)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cegb3306)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, celegans_metabolic)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, celegansneural)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cell1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cell2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cfd1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cfd2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ch5_5_b3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ch7_8_b5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Chebyshev1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Chebyshev2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Chebyshev3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Chebyshev4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Chem97ZtZ)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, chem_master1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, chipcool0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, chipcool1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, circuit_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, circuit204)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, circuit_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, circuit_3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, circuit_4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, citationCiteseer)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cit_HepPh)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cit_HepTh)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ck400)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ck656)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ckt11752_dc_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ckt11752_tr_0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cnr_2000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, coater1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, coater2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, coAuthorsCiteseer)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, coAuthorsDBLP)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, commanche_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, comsol)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cond_mat_2003)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cond_mat_2005)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cond_mat)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf5_0_4x4_10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf5_0_4x4_14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf5_0_4x4_18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf5_0_4x4_22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf5_0_4x4_26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf5_4_8x8_05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf5_4_8x8_10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf5_4_8x8_15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf5_4_8x8_20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf6_0_4x4_20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf6_0_4x4_30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf6_0_8x8_20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf6_0_8x8_30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, conf6_0_8x8_80)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, consph)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cont_201)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cont_300)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cop20k_A)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, coPapersCiteseer)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, copter1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, copter2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, CO)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, coupled)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crack_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crack)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crankseg_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crankseg_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crashbasis)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crplat2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cryg10000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cryg2500)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crystk01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crystk02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crystk03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crystm01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crystm02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, crystm03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cs4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, CSphd)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ct20stif)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cti)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cvxbqp1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cvxqp3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, cyl6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, darcy003)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, data)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dawson5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dblp_2010)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dc1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dc2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dc3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, delaunay_n19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, denormal)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, descriptor_xingo6u)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, diag)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dictionary28)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dielFilterV3clx)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dixmaanl)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, DK01R)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, d_pretok)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dtoc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Dubcova1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Dubcova2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Dubcova3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dw1024)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dw2048)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dw256A)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dw256B)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dw4096)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dw8192)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwa512)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwb512)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwg961a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwg961b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_1005)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_1007)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_1242)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_162)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_193)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_198)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_209)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_221)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_245)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_2680)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_307)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_310)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_346)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_361)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_419)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_492)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_503)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_512)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_592)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_607)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_758)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_869)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_878)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_918)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, dwt_992)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, e40r0100)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, EAT_RS)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, EAT_SR)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ecl32)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, email_Enron)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, email_EuAll)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, email)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, engine)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, enron)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, EPA)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, epb0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, epb1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, epb2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, epb3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Erdos02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Erdos971)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Erdos972)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Erdos981)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Erdos982)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Erdos991)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Erdos992)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, eris1176)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, eurqsa)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, EVA)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex10hs)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex27)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex28)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex29)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex31)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex32)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex33)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex35)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex36)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex37)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex3sta1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex40)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, EX5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex8)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ex9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, exdata_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, extr1b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, extr1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, F1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, F2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, FA)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fcondp2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fd12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fd15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fd18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fe_4elt2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fe_body)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, FEM_3D_thermal1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, FEM_3D_thermal2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fem_filter)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fem_hifreq_circuit)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fe_ocean)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fe_rotor)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fe_sphere)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fe_tooth)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, filter2D)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, filter3D)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, finan512)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, finance256)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, flowmeter0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, flowmeter5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, foldoc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ford1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ford2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_27)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_28)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_29)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_31)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_32)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_33)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_34)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_35)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_36)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_37)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_38)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_39)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_40)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_41)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_42)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_43)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_44)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_45)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_46)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_47)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_48)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_49)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_50)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_dcop_51)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_trans_01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fpga_trans_02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fp)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_183_3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_183_4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_541_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_541_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_541_3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_541_4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_680_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_680_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_680_3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_760_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_760_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fs_760_3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fullb)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fv1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fv2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fv3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fxm3_6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, fxm4_6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G27)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G28)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G29)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G2_circuit)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G31)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G32)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G33)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G34)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G35)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G36)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G37)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G38)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G39)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g3rmt3m3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G40)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G41)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G42)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G43)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G44)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G45)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G46)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G47)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G48)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G49)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G50)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G51)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G52)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G53)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G54)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G55)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G56)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G57)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G58)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G59)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G60)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G61)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G62)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G63)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G64)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G65)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G66)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G67)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac010sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac010)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac020sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac020)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac040sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac040)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac050sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac060sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac060)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac080sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac080)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac100sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac100)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac120sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac120)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac140sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac140)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac160sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac160)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac180sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac180)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac200sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, g7jac200)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G8)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, G9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Ga10As10H30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Ga19As19H42)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Ga3As3H12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Ga41As41H72)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, GaAsH6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, garon1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, garon2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gas_sensor)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, GD00_c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, GD06_Java)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, GD96_a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Ge87H76)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Ge99H100)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gearbox)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gemat11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gemat12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, geom)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, goodwin)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gr_30_30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, graham1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gre_1107)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gre_343)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gre_512)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, grid2_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, grid2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gridgena)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, GT01R)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gupta1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gupta2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gupta3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gyro_k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gyro_m)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, gyro)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, H2O)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, halfb)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Hamrle2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Harvard500)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hcircuit)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, heart1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, heart2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, heart3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, helm2d03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, helm3d01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, HEP_th_new)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hep_th)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hood)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hor_131)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, HTC_336_4438)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, HTC_336_9129)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, human_gene1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, human_gene2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hvdc1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hvdc2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hydr1c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, hydr1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ibm_matrix_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, igbt3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Ill_Stokes)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, impcol_c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, impcol_d)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, impcol_e)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, init_adder1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, inlet)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, inline_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ins2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, internet)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, invextr1_new)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, iprob)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jagmesh1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jagmesh2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jagmesh3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jagmesh4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jagmesh5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jagmesh6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jagmesh7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jagmesh8)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jagmesh9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac020sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac020)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac040sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac040)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac060sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac060)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac080sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac080)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac100sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac100)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac120sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jan99jac120)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jazz)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jnlbrng1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, jpwh_991)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, juba40k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, k1_san)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, k3plates)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Kaufhold)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, kim1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, kim2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Kohonen)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, kron_g500_logn16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, kron_g500_logn17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, kron_g500_logn18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, kron_g500_logn19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Kuu)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, L_9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, laminar_duct3D)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, language)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, largebasis)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, laser)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Lederberg)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, LF10000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, LFAT5000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr04c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr07c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr10c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr11c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr14c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr17c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr34c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr34)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr71c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lhr71)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, light_in_tissue)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Lin)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, linverse)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, li)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lns_3937)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lns_511)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lnsp3937)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lnsp_511)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lock1074)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lock2232)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lock3491)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lock_700)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lpl1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp1009)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp1270)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp1561)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp1882)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp2233)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp2614)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp_265)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp3025)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp3466)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp_406)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp_577)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lshp_778)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, L)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lund_a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lund_b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lung1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, lung2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, luxembourg_osm)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, M10PI_n1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, M10PI_n)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, m133_b3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, m14b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, M20PI_n1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, M20PI_n)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, m3plates)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, M40PI_n1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, M40PI_n)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, M80PI_n1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, M80PI_n)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mac_econ_fwd500)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mahindas)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, majorbasis)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, man_5976)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mario001)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mario002)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac020sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac020)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac040sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac040)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac060sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac060)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac080sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac080)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac100sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac100)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac120sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac120)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac140sc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mark3jac140)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, matrix_9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, matrix_new_3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mbeacxc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mbeaflw)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mbeause)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mc2depi)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mcca)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mcfe)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, meg1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, meg4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, memplus)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mesh2e1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mesh2em5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mesh3e1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mesh3em5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mhd1280a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mhd1280b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mhd3200a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mhd3200b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mhd4800a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mhd4800b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mhda416)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mhdb416)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mimo28x28_system)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mimo46x46_system)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mimo8x8_system)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, minnesota)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, minsurfo)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mip1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mixtank_new)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mono_500Hz)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mouse_gene)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mplate)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, msc00726)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, msc01050)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, msc01440)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, msc04515)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, msc10848)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, msc23052)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, msdoor)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, m_t1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mult_dcop_01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mult_dcop_02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, mult_dcop_03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Muu)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, n3c6_b7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Na5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nasa1824)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nasa2146)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nasa2910)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nasa4704)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nasasrb)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ncvxbqp1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ncvxqp1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ncvxqp3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ncvxqp5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ncvxqp7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ncvxqp9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nd12k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nd24k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nd3k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nd6k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nemeth26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, net100)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, net125)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, net150)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, net25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, net4_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, net50)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, net75)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, netscience)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, netz4504_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, netz4504)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nmos3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nnc1374)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nnc261)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nnc666)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nopoly)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nopss_11k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nos2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nos3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nos5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nos6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, nos7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, NotreDame_www)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, NotreDame_yeast)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ns3Da)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, obstclae)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, odepa400)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ODLIS)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, offshore)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ohne2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oilpan)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, olafu)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, olesnik0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, olm1000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, olm2000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, olm5000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, olm500)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, onera_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, onetone1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, onetone2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, OPF_10000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, OPF_3754)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, OPF_6000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, opt1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, orani678)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Oregon_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Oregon_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, orsirr_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, orsirr_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, orsreg_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_27)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_28)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_29)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_31)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_32)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_33)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_34)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_35)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_36)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_37)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_38)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_39)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_40)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_41)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_42)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_43)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_44)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_45)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_46)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_47)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_48)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_49)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_50)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_51)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_52)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_53)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_54)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_55)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_56)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_dcop_57)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, oscil_trans_01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, p2p_Gnutella04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, p2p_Gnutella05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, p2p_Gnutella06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, p2p_Gnutella08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, p2p_Gnutella09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, p2p_Gnutella24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, p2p_Gnutella25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, p2p_Gnutella30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, p2p_Gnutella31)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, para_10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, para_4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, para_5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, para_6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, para_7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, para_8)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, para_9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, patents_main)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pattern1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pcrystk02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pcrystk03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pct20stif)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pdb1HYS)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pde225)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pde2961)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pde900)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pds10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Pd)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pesa)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pf2177)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pfinan512)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, PGPgiantcompo)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, piston)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pkustk14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, plat1919)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, plat362)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, plbuckle)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pli)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, plsk1919)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, plskz362)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, poisson2D)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, poisson3Da)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, poisson3Db)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, polblogs)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, poli3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, poli4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, poli_large)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, poli)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pores_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pores_3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, powersim)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, power)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, PR02R)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Pres_Poisson)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, problem1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, psmigr_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, psmigr_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, psmigr_3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pwtk)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, pwt)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, qa8fk)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, qa8fm)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, qc2534)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, qc324)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, qh1484)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, qh768)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, qh882)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, qpband)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, radfr1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, raefsky1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, raefsky2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, raefsky3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, raefsky4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, raefsky5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, raefsky6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rail_1357)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rail_20209)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rail_5177)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rail_79841)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Raj1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat03)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat04)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat05)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat06)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat07)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat08)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat09)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat11)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat13)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat14)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat17)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat20)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat21)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat22)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat23)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat24)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat26)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat27)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rajat28)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ramage02)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rbsa480)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rbsb480)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb1250l)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb1250)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb200l)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb200)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb2048_noL)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb2048)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb3200l)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb450l)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb450)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb5000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb800l)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdb968)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdist1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdist2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rdist3a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Reuters911)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, RFdevice)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rgg_n_2_15_s0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rgg_n_2_16_s0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rgg_n_2_17_s0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rgg_n_2_18_s0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rgg_n_2_19_s0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rim)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, RM07R)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rma10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Roget)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rotor2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rw136)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rw496)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, rw5151)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, S10PI_n1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, S10PI_n)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, s1rmq4m1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, s1rmt3m1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, S20PI_n1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, S20PI_n)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, s2rmq4m1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, s2rmt3m1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, s3dkq4m2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, s3dkt3m2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, s3rmq4m1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, s3rmt3m1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, s3rmt3m3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, S40PI_n1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, S40PI_n)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, s4dkt3m2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, S80PI_n1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, S80PI_n)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, saylr1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, saylr3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, saylr4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, SciMet)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, scircuit)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, se)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shallow_water1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shallow_water2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shar_te2_b3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sherman1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sherman2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sherman3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sherman4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sherman5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shermanACa)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shermanACb)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shermanACd)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ship_001)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ship_003)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shipsec1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shipsec5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shipsec8)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shl_0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shl_200)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shl_400)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shock_9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shuttle_eddy)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shyy161)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, shyy41)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Si10H16)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Si2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Si34H36)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Si41Ge41H72)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Si5H12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Si87H76)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Sieber)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, SiH4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, SiNa)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sinc12)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sinc15)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sinc18)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, SiO2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, SiO)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sit100)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, skirt)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, SmaGri)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sme3Da)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sme3Db)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sme3Dc)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, smt)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, soc_Epinions1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, soc_sign_epinions)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, soc_sign_Slashdot081106)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, soc_sign_Slashdot090216)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, soc_sign_Slashdot090221)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, soc_Slashdot0811)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, soc_Slashdot0902)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sparsine)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sphere3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, spiral)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, spmsrtls)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, srb1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sstmodel)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Stanford)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, std1_Jac2_db)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, std1_Jac2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, std1_Jac3_db)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, std1_Jac3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, steam1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, steam2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, stokes128)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, stokes64s)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, stokes64)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, stomach)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, str_0)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, str_200)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, str_400)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, str_600)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, struct3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, struct4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, sts4098)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, stufe_10)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, stufe)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, swang1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, swang2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t2dah_a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t2dah_e)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t2dah)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t2dal_a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t2dal_bci)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t2dal_e)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t2dal)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t2d_q4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t2d_q9)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t3dh_a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t3dh_e)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t3dh)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t3dl_a)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t3dl_e)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t3dl)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t520)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, t60k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tandem_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tandem_vtx)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ted_AB)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ted_AB_unscaled)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ted_A)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ted_A_unscaled)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ted_B)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ted_B_unscaled)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, thermal1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, thermal)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, thermomech_dK)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, thermomech_dM)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, thermomech_TC)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, thermomech_TK)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, thread)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tols1090)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tols2000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tols340)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tols4000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tomography)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, torsion1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, torso1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, torso2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, torso3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, trans4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, trans5)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, transient)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, trdheim)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Trefethen_150)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Trefethen_20000b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Trefethen_20000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Trefethen_2000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Trefethen_200b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Trefethen_200)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Trefethen_300)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Trefethen_500)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Trefethen_700)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, troll)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSC_OPF_1047)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSC_OPF_300)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b162_c1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b162_c3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b162_c4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b300_c1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b300_c2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b300_c3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b300)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b39_c19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b39_c30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b39_c7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b9_c1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_FS_b9_c6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b162_c1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b162_c3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b162_c4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b2052_c1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b2383_c1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b2383)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b300_c1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b300_c2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b300_c3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b39_c19)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b39_c30)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b39_c7)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b678_c1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b678_c2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TSOPF_RS_b9_c6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, TS)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tsyl201)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tub1000)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tube1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tube2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tuma1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, tuma2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, turon_m)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, twotone)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ukerbe1_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ukerbe1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, uk)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, USAir97)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, USpowerGrid)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, usroads_48)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, usroads)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, utm1700b)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, utm300)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, utm3060)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, utm5940)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, vanbody)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, venkat01)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, venkat25)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, venkat50)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, vfem)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, vibrobox)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, viscoplastic1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, viscoplastic2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, viscorocks)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wang1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wang2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wang3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wang4)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, water_tank)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wathen100)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wathen120)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, watt_1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, watt_2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, waveguide3D)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wave)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wb_cs_stanford)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, web_NotreDame)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, web_Stanford)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, west0381)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, west0479)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, west0497)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, west0655)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, west0989)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, west1505)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, west2021)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, whitaker3_dual)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, whitaker3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wiki_Vote)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, windscreen)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wing_nodal)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, wing)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Wordnet3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, ww_vref_6405)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, x104)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, xenon1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, xenon2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, xingo3012)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, xingo_afonso_itaipu)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, yeast)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, young1c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, young2c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, young3c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, young4c)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Zd_Jac2_db)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Zd_Jac2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Zd_Jac3_db)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Zd_Jac3)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Zd_Jac6_db)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Zd_Jac6)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, zenios)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, zeros_nopss_13k)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Zewail)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Zhao1)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64, Zhao2)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, roadNet_CA)
{
    std::string input_file(MATRIX_FULL_PATH(roadNet-CA));
    std::string output_file(RESULT_FULL_PATH(roadNet-CA));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, amazon0312)
{
    std::string input_file(MATRIX_FULL_PATH(amazon0312));
    std::string output_file(RESULT_FULL_PATH(amazon0312));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, com_Youtube)
{
    std::string input_file(MATRIX_FULL_PATH(com-Youtube));
    std::string output_file(RESULT_FULL_PATH(com-Youtube));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, web_Stanford)
{
    std::string input_file(MATRIX_FULL_PATH(web-Stanford));
    std::string output_file(RESULT_FULL_PATH(web-Stanford));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, as_Skitter)
{
    std::string input_file(MATRIX_FULL_PATH(as-Skitter));
    std::string output_file(RESULT_FULL_PATH(as-Skitter));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, road_usa)
{
    std::string input_file(MATRIX_FULL_PATH(road_usa));
    std::string output_file(RESULT_FULL_PATH(road_usa));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, coAuthorsCiteseer)
{
    std::string input_file(MATRIX_FULL_PATH(coAuthorsCiteseer));
    std::string output_file(RESULT_FULL_PATH(coAuthorsCiteseer));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, coAuthorsDBLP)
{
    std::string input_file(MATRIX_FULL_PATH(coAuthorsDBLP));
    std::string output_file(RESULT_FULL_PATH(coAuthorsDBLP));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, web_Google)
{
    std::string input_file(MATRIX_FULL_PATH(web-Google));
    std::string output_file(RESULT_FULL_PATH(web-Google));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, com_Orkut)
{
...
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, TSOPF_RS_b2383)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b2383));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b2383));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, coPapersCiteseer)
{
    std::string input_file(MATRIX_FULL_PATH(coPapersCiteseer));
    std::string output_file(RESULT_FULL_PATH(coPapersCiteseer));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, eu_2005)
{
    std::string input_file(MATRIX_FULL_PATH(eu-2005));
    std::string output_file(RESULT_FULL_PATH(eu-2005));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, crankseg_2)
{
    std::string input_file(MATRIX_FULL_PATH(crankseg_2));
    std::string output_file(RESULT_FULL_PATH(crankseg_2));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, nd12k)
{
    std::string input_file(MATRIX_FULL_PATH(nd12k));
    std::string output_file(RESULT_FULL_PATH(nd12k));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, coPapersDBLP)
{
    std::string input_file(MATRIX_FULL_PATH(coPapersDBLP));
    std::string output_file(RESULT_FULL_PATH(coPapersDBLP));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, cnr_2000)
{
    std::string input_file(MATRIX_FULL_PATH(cnr-2000));
    std::string output_file(RESULT_FULL_PATH(cnr-2000));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, msdoor)
{
    std::string input_file(MATRIX_FULL_PATH(msdoor));
    std::string output_file(RESULT_FULL_PATH(msdoor));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, bundle_adj)
{
    std::string input_file(MATRIX_FULL_PATH(bundle_adj));
    std::string output_file(RESULT_FULL_PATH(bundle_adj));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, F1)
{
    std::string input_file(MATRIX_FULL_PATH(F1));
    std::string output_file(RESULT_FULL_PATH(F1));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, inline_1)
{
    std::string input_file(MATRIX_FULL_PATH(inline_1));
    std::string output_file(RESULT_FULL_PATH(inline_1));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, audikw_1)
{
    std::string input_file(MATRIX_FULL_PATH(audikw_1));
    std::string output_file(RESULT_FULL_PATH(audikw_1));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, ML_Laplace)
{
    std::string input_file(MATRIX_FULL_PATH(ML_Laplace));
    std::string output_file(RESULT_FULL_PATH(ML_Laplace));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, dielFilterV3real)
{
    std::string input_file(MATRIX_FULL_PATH(dielFilterV3real));
    std::string output_file(RESULT_FULL_PATH(dielFilterV3real));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, Flan_1565)
{
    std::string input_file(MATRIX_FULL_PATH(Flan_1565));
    std::string output_file(RESULT_FULL_PATH(Flan_1565));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, Long_Coup_dt0)
{
    std::string input_file(MATRIX_FULL_PATH(Long_Coup_dt0));
    std::string output_file(RESULT_FULL_PATH(Long_Coup_dt0));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP64_COMMON, mip1)
{
    std::string input_file(MATRIX_FULL_PATH(mip1));
    std::string output_file(RESULT_FULL_PATH(mip1));
    Perf_test_cusparse_spmv_csr_fp64(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, roadNet_CA)
{
    std::string input_file(MATRIX_FULL_PATH(roadNet-CA));
    std::string output_file(RESULT_FULL_PATH(roadNet-CA));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, amazon0312)
{
    std::string input_file(MATRIX_FULL_PATH(amazon0312));
    std::string output_file(RESULT_FULL_PATH(amazon0312));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, com_Youtube)
{
    std::string input_file(MATRIX_FULL_PATH(com-Youtube));
    std::string output_file(RESULT_FULL_PATH(com-Youtube));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, web_Stanford)
{
    std::string input_file(MATRIX_FULL_PATH(web-Stanford));
    std::string output_file(RESULT_FULL_PATH(web-Stanford));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, as_Skitter)
{
    std::string input_file(MATRIX_FULL_PATH(as-Skitter));
    std::string output_file(RESULT_FULL_PATH(as-Skitter));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, road_usa)
{
    std::string input_file(MATRIX_FULL_PATH(road_usa));
    std::string output_file(RESULT_FULL_PATH(road_usa));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, coAuthorsCiteseer)
{
    std::string input_file(MATRIX_FULL_PATH(coAuthorsCiteseer));
    std::string output_file(RESULT_FULL_PATH(coAuthorsCiteseer));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, coAuthorsDBLP)
{
    std::string input_file(MATRIX_FULL_PATH(coAuthorsDBLP));
    std::string output_file(RESULT_FULL_PATH(coAuthorsDBLP));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, web_Google)
{
    std::string input_file(MATRIX_FULL_PATH(web-Google));
    std::string output_file(RESULT_FULL_PATH(web-Google));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, com_Orkut)
{
    std::string input_file(MATRIX_FULL_PATH(com-Orkut));
    std::string output_file(RESULT_FULL_PATH(com-Orkut));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, higgs_twitter)
{
    std::string input_file(MATRIX_FULL_PATH(higgs-twitter));
    std::string output_file(RESULT_FULL_PATH(higgs-twitter));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, wiki_topcats)
{
    std::string input_file(MATRIX_FULL_PATH(wiki-topcats));
    std::string output_file(RESULT_FULL_PATH(wiki-topcats));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, web_BerkStan)
{
    std::string input_file(MATRIX_FULL_PATH(web-BerkStan));
    std::string output_file(RESULT_FULL_PATH(web-BerkStan));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, com_Amazon)
{
    std::string input_file(MATRIX_FULL_PATH(com-Amazon));
    std::string output_file(RESULT_FULL_PATH(com-Amazon));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, com_DBLP)
{
    std::string input_file(MATRIX_FULL_PATH(com-DBLP));
    std::string output_file(RESULT_FULL_PATH(com-DBLP));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, TSOPF_RS_b2383)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b2383));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b2383));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, coPapersCiteseer)
{
    std::string input_file(MATRIX_FULL_PATH(coPapersCiteseer));
    std::string output_file(RESULT_FULL_PATH(coPapersCiteseer));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, eu_2005)
{
    std::string input_file(MATRIX_FULL_PATH(eu-2005));
    std::string output_file(RESULT_FULL_PATH(eu-2005));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, crankseg_2)
{
    std::string input_file(MATRIX_FULL_PATH(crankseg_2));
    std::string output_file(RESULT_FULL_PATH(crankseg_2));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, nd12k)
{
    std::string input_file(MATRIX_FULL_PATH(nd12k));
    std::string output_file(RESULT_FULL_PATH(nd12k));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, coPapersDBLP)
{
    std::string input_file(MATRIX_FULL_PATH(coPapersDBLP));
    std::string output_file(RESULT_FULL_PATH(coPapersDBLP));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, cnr_2000)
{
    std::string input_file(MATRIX_FULL_PATH(cnr-2000));
    std::string output_file(RESULT_FULL_PATH(cnr-2000));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, msdoor)
{
    std::string input_file(MATRIX_FULL_PATH(msdoor));
    std::string output_file(RESULT_FULL_PATH(msdoor));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, bundle_adj)
{
    std::string input_file(MATRIX_FULL_PATH(bundle_adj));
    std::string output_file(RESULT_FULL_PATH(bundle_adj));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, F1)
{
    std::string input_file(MATRIX_FULL_PATH(F1));
    std::string output_file(RESULT_FULL_PATH(F1));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, inline_1)
{
    std::string input_file(MATRIX_FULL_PATH(inline_1));
    std::string output_file(RESULT_FULL_PATH(inline_1));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, audikw_1)
{
    std::string input_file(MATRIX_FULL_PATH(audikw_1));
    std::string output_file(RESULT_FULL_PATH(audikw_1));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, ML_Laplace)
{
    std::string input_file(MATRIX_FULL_PATH(ML_Laplace));
    std::string output_file(RESULT_FULL_PATH(ML_Laplace));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, dielFilterV3real)
{
    std::string input_file(MATRIX_FULL_PATH(dielFilterV3real));
    std::string output_file(RESULT_FULL_PATH(dielFilterV3real));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, Flan_1565)
{
    std::string input_file(MATRIX_FULL_PATH(Flan_1565));
    std::string output_file(RESULT_FULL_PATH(Flan_1565));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, Long_Coup_dt0)
{
    std::string input_file(MATRIX_FULL_PATH(Long_Coup_dt0));
    std::string output_file(RESULT_FULL_PATH(Long_Coup_dt0));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
}
TEST(test_perf_cusparse_spmv_csr_FP32_COMMON, mip1)
{
    std::string input_file(MATRIX_FULL_PATH(mip1));
    std::string output_file(RESULT_FULL_PATH(mip1));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 3Dspectralwave)
{
    std::string input_file(MATRIX_FULL_PATH(3Dspectralwave ));
    std::string output_file(RESULT_FULL_PATH(3Dspectralwave ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adaptive)
{
    std::string input_file(MATRIX_FULL_PATH(adaptive ));
    std::string output_file(RESULT_FULL_PATH(adaptive ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_shell10)
{
    std::string input_file(MATRIX_FULL_PATH(af_shell10 ));
    std::string output_file(RESULT_FULL_PATH(af_shell10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, amazon_2008)
{
    std::string input_file(MATRIX_FULL_PATH(amazon-2008 ));
    std::string output_file(RESULT_FULL_PATH(amazon-2008 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, apache2)
{
    std::string input_file(MATRIX_FULL_PATH(apache2 ));
    std::string output_file(RESULT_FULL_PATH(apache2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, asia_osm)
{
    std::string input_file(MATRIX_FULL_PATH(asia_osm ));
    std::string output_file(RESULT_FULL_PATH(asia_osm ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ASIC_680ks)
{
    std::string input_file(MATRIX_FULL_PATH(ASIC_680ks ));
    std::string output_file(RESULT_FULL_PATH(ASIC_680ks ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ASIC_680k)
{
    std::string input_file(MATRIX_FULL_PATH(ASIC_680k ));
    std::string output_file(RESULT_FULL_PATH(ASIC_680k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, as_Skitter)
{
    std::string input_file(MATRIX_FULL_PATH(as-Skitter ));
    std::string output_file(RESULT_FULL_PATH(as-Skitter ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, atmosmodd)
{
    std::string input_file(MATRIX_FULL_PATH(atmosmodd ));
    std::string output_file(RESULT_FULL_PATH(atmosmodd ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, atmosmodj)
{
    std::string input_file(MATRIX_FULL_PATH(atmosmodj ));
    std::string output_file(RESULT_FULL_PATH(atmosmodj ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, atmosmodl)
{
    std::string input_file(MATRIX_FULL_PATH(atmosmodl ));
    std::string output_file(RESULT_FULL_PATH(atmosmodl ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, atmosmodm)
{
    std::string input_file(MATRIX_FULL_PATH(atmosmodm ));
    std::string output_file(RESULT_FULL_PATH(atmosmodm ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, audikw_1)
{
    std::string input_file(MATRIX_FULL_PATH(audikw_1 ));
    std::string output_file(RESULT_FULL_PATH(audikw_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, belgium_osm)
{
    std::string input_file(MATRIX_FULL_PATH(belgium_osm ));
    std::string output_file(RESULT_FULL_PATH(belgium_osm ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bone010)
{
    std::string input_file(MATRIX_FULL_PATH(bone010 ));
    std::string output_file(RESULT_FULL_PATH(bone010 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, boneS10)
{
    std::string input_file(MATRIX_FULL_PATH(boneS10 ));
    std::string output_file(RESULT_FULL_PATH(boneS10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cage14)
{
    std::string input_file(MATRIX_FULL_PATH(cage14 ));
    std::string output_file(RESULT_FULL_PATH(cage14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cage15)
{
    std::string input_file(MATRIX_FULL_PATH(cage15 ));
    std::string output_file(RESULT_FULL_PATH(cage15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, channel_500x100x100_b050)
{
    std::string input_file(MATRIX_FULL_PATH(channel-500x100x100-b050 ));
    std::string output_file(RESULT_FULL_PATH(channel-500x100x100-b050 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, circuit5M_dc)
{
    std::string input_file(MATRIX_FULL_PATH(circuit5M_dc ));
    std::string output_file(RESULT_FULL_PATH(circuit5M_dc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, circuit5M)
{
    std::string input_file(MATRIX_FULL_PATH(circuit5M ));
    std::string output_file(RESULT_FULL_PATH(circuit5M ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cit_Patents)
{
    std::string input_file(MATRIX_FULL_PATH(cit-Patents ));
    std::string output_file(RESULT_FULL_PATH(cit-Patents ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, coPapersDBLP)
{
    std::string input_file(MATRIX_FULL_PATH(coPapersDBLP ));
    std::string output_file(RESULT_FULL_PATH(coPapersDBLP ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, debr)
{
    std::string input_file(MATRIX_FULL_PATH(debr ));
    std::string output_file(RESULT_FULL_PATH(debr ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n20)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n20 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n21)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n21 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n22)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n22 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n23)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n23 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dielFilterV2clx)
{
    std::string input_file(MATRIX_FULL_PATH(dielFilterV2clx ));
    std::string output_file(RESULT_FULL_PATH(dielFilterV2clx ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dielFilterV2real)
{
    std::string input_file(MATRIX_FULL_PATH(dielFilterV2real ));
    std::string output_file(RESULT_FULL_PATH(dielFilterV2real ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dielFilterV3real)
{
    std::string input_file(MATRIX_FULL_PATH(dielFilterV3real ));
    std::string output_file(RESULT_FULL_PATH(dielFilterV3real ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ecology1)
{
    std::string input_file(MATRIX_FULL_PATH(ecology1 ));
    std::string output_file(RESULT_FULL_PATH(ecology1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ecology2)
{
    std::string input_file(MATRIX_FULL_PATH(ecology2 ));
    std::string output_file(RESULT_FULL_PATH(ecology2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Emilia_923)
{
    std::string input_file(MATRIX_FULL_PATH(Emilia_923 ));
    std::string output_file(RESULT_FULL_PATH(Emilia_923 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, eu_2005)
{
    std::string input_file(MATRIX_FULL_PATH(eu-2005 ));
    std::string output_file(RESULT_FULL_PATH(eu-2005 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Fault_639)
{
    std::string input_file(MATRIX_FULL_PATH(Fault_639 ));
    std::string output_file(RESULT_FULL_PATH(Fault_639 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, flickr)
{
    std::string input_file(MATRIX_FULL_PATH(flickr ));
    std::string output_file(RESULT_FULL_PATH(flickr ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Freescale1)
{
    std::string input_file(MATRIX_FULL_PATH(Freescale1 ));
    std::string output_file(RESULT_FULL_PATH(Freescale1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, FullChip)
{
    std::string input_file(MATRIX_FULL_PATH(FullChip ));
    std::string output_file(RESULT_FULL_PATH(FullChip ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G3_circuit)
{
    std::string input_file(MATRIX_FULL_PATH(G3_circuit ));
    std::string output_file(RESULT_FULL_PATH(G3_circuit ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Geo_1438)
{
    std::string input_file(MATRIX_FULL_PATH(Geo_1438 ));
    std::string output_file(RESULT_FULL_PATH(Geo_1438 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, germany_osm)
{
    std::string input_file(MATRIX_FULL_PATH(germany_osm ));
    std::string output_file(RESULT_FULL_PATH(germany_osm ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, great_britain_osm)
{
    std::string input_file(MATRIX_FULL_PATH(great-britain_osm ));
    std::string output_file(RESULT_FULL_PATH(great-britain_osm ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gsm_106857)
{
    std::string input_file(MATRIX_FULL_PATH(gsm_106857 ));
    std::string output_file(RESULT_FULL_PATH(gsm_106857 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Hamrle3)
{
    std::string input_file(MATRIX_FULL_PATH(Hamrle3 ));
    std::string output_file(RESULT_FULL_PATH(Hamrle3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Hook_1498)
{
    std::string input_file(MATRIX_FULL_PATH(Hook_1498 ));
    std::string output_file(RESULT_FULL_PATH(Hook_1498 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hugebubbles_00000)
{
    std::string input_file(MATRIX_FULL_PATH(hugebubbles-00000 ));
    std::string output_file(RESULT_FULL_PATH(hugebubbles-00000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hugebubbles_00010)
{
    std::string input_file(MATRIX_FULL_PATH(hugebubbles-00010 ));
    std::string output_file(RESULT_FULL_PATH(hugebubbles-00010 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hugebubbles_00020)
{
    std::string input_file(MATRIX_FULL_PATH(hugebubbles-00020 ));
    std::string output_file(RESULT_FULL_PATH(hugebubbles-00020 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hugetrace_00000)
{
    std::string input_file(MATRIX_FULL_PATH(hugetrace-00000 ));
    std::string output_file(RESULT_FULL_PATH(hugetrace-00000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hugetrace_00010)
{
    std::string input_file(MATRIX_FULL_PATH(hugetrace-00010 ));
    std::string output_file(RESULT_FULL_PATH(hugetrace-00010 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hugetrace_00020)
{
    std::string input_file(MATRIX_FULL_PATH(hugetrace-00020 ));
    std::string output_file(RESULT_FULL_PATH(hugetrace-00020 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hugetric_00000)
{
    std::string input_file(MATRIX_FULL_PATH(hugetric-00000 ));
    std::string output_file(RESULT_FULL_PATH(hugetric-00000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hugetric_00010)
{
    std::string input_file(MATRIX_FULL_PATH(hugetric-00010 ));
    std::string output_file(RESULT_FULL_PATH(hugetric-00010 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hugetric_00020)
{
    std::string input_file(MATRIX_FULL_PATH(hugetric-00020 ));
    std::string output_file(RESULT_FULL_PATH(hugetric-00020 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, in_2004)
{
    std::string input_file(MATRIX_FULL_PATH(in-2004 ));
    std::string output_file(RESULT_FULL_PATH(in-2004 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, italy_osm)
{
    std::string input_file(MATRIX_FULL_PATH(italy_osm ));
    std::string output_file(RESULT_FULL_PATH(italy_osm ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, kkt_power)
{
    std::string input_file(MATRIX_FULL_PATH(kkt_power ));
    std::string output_file(RESULT_FULL_PATH(kkt_power ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, kron_g500_logn20)
{
    std::string input_file(MATRIX_FULL_PATH(kron_g500-logn20 ));
    std::string output_file(RESULT_FULL_PATH(kron_g500-logn20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ldoor)
{
    std::string input_file(MATRIX_FULL_PATH(ldoor ));
    std::string output_file(RESULT_FULL_PATH(ldoor ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ljournal_2008)
{
    std::string input_file(MATRIX_FULL_PATH(ljournal-2008 ));
    std::string output_file(RESULT_FULL_PATH(ljournal-2008 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lp1)
{
    std::string input_file(MATRIX_FULL_PATH(lp1 ));
    std::string output_file(RESULT_FULL_PATH(lp1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, memchip)
{
    std::string input_file(MATRIX_FULL_PATH(memchip ));
    std::string output_file(RESULT_FULL_PATH(memchip ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, netherlands_osm)
{
    std::string input_file(MATRIX_FULL_PATH(netherlands_osm ));
    std::string output_file(RESULT_FULL_PATH(netherlands_osm ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nlpkkt120)
{
    std::string input_file(MATRIX_FULL_PATH(nlpkkt120 ));
    std::string output_file(RESULT_FULL_PATH(nlpkkt120 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nlpkkt80)
{
    std::string input_file(MATRIX_FULL_PATH(nlpkkt80 ));
    std::string output_file(RESULT_FULL_PATH(nlpkkt80 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, packing_500x100x100_b050)
{
    std::string input_file(MATRIX_FULL_PATH(packing-500x100x100-b050 ));
    std::string output_file(RESULT_FULL_PATH(packing-500x100x100-b050 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, parabolic_fem)
{
    std::string input_file(MATRIX_FULL_PATH(parabolic_fem ));
    std::string output_file(RESULT_FULL_PATH(parabolic_fem ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, patents)
{
    std::string input_file(MATRIX_FULL_PATH(patents ));
    std::string output_file(RESULT_FULL_PATH(patents ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pre2)
{
    std::string input_file(MATRIX_FULL_PATH(pre2 ));
    std::string output_file(RESULT_FULL_PATH(pre2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat29)
{
    std::string input_file(MATRIX_FULL_PATH(rajat29 ));
    std::string output_file(RESULT_FULL_PATH(rajat29 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat30)
{
    std::string input_file(MATRIX_FULL_PATH(rajat30 ));
    std::string output_file(RESULT_FULL_PATH(rajat30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat31)
{
    std::string input_file(MATRIX_FULL_PATH(rajat31 ));
    std::string output_file(RESULT_FULL_PATH(rajat31 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rgg_n_2_20_s0)
{
    std::string input_file(MATRIX_FULL_PATH(rgg_n_2_20_s0 ));
    std::string output_file(RESULT_FULL_PATH(rgg_n_2_20_s0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rgg_n_2_21_s0)
{
    std::string input_file(MATRIX_FULL_PATH(rgg_n_2_21_s0 ));
    std::string output_file(RESULT_FULL_PATH(rgg_n_2_21_s0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rgg_n_2_22_s0)
{
    std::string input_file(MATRIX_FULL_PATH(rgg_n_2_22_s0 ));
    std::string output_file(RESULT_FULL_PATH(rgg_n_2_22_s0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, road_central)
{
    std::string input_file(MATRIX_FULL_PATH(road_central ));
    std::string output_file(RESULT_FULL_PATH(road_central ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, roadNet_CA)
{
    std::string input_file(MATRIX_FULL_PATH(roadNet-CA ));
    std::string output_file(RESULT_FULL_PATH(roadNet-CA ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, roadNet_PA)
{
    std::string input_file(MATRIX_FULL_PATH(roadNet-PA ));
    std::string output_file(RESULT_FULL_PATH(roadNet-PA ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, roadNet_TX)
{
    std::string input_file(MATRIX_FULL_PATH(roadNet-TX ));
    std::string output_file(RESULT_FULL_PATH(roadNet-TX ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, road_usa)
{
    std::string input_file(MATRIX_FULL_PATH(road_usa ));
    std::string output_file(RESULT_FULL_PATH(road_usa ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Serena)
{
    std::string input_file(MATRIX_FULL_PATH(Serena ));
    std::string output_file(RESULT_FULL_PATH(Serena ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, soc_LiveJournal1)
{
    std::string input_file(MATRIX_FULL_PATH(soc-LiveJournal1 ));
    std::string output_file(RESULT_FULL_PATH(soc-LiveJournal1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Stanford_Berkeley)
{
    std::string input_file(MATRIX_FULL_PATH(Stanford_Berkeley ));
    std::string output_file(RESULT_FULL_PATH(Stanford_Berkeley ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, StocF_1465)
{
    std::string input_file(MATRIX_FULL_PATH(StocF-1465 ));
    std::string output_file(RESULT_FULL_PATH(StocF-1465 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t2em)
{
    std::string input_file(MATRIX_FULL_PATH(t2em ));
    std::string output_file(RESULT_FULL_PATH(t2em ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, thermal2)
{
    std::string input_file(MATRIX_FULL_PATH(thermal2 ));
    std::string output_file(RESULT_FULL_PATH(thermal2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tmt_sym)
{
    std::string input_file(MATRIX_FULL_PATH(tmt_sym ));
    std::string output_file(RESULT_FULL_PATH(tmt_sym ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tmt_unsym)
{
    std::string input_file(MATRIX_FULL_PATH(tmt_unsym ));
    std::string output_file(RESULT_FULL_PATH(tmt_unsym ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, venturiLevel3)
{
    std::string input_file(MATRIX_FULL_PATH(venturiLevel3 ));
    std::string output_file(RESULT_FULL_PATH(venturiLevel3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wb_edu)
{
    std::string input_file(MATRIX_FULL_PATH(wb-edu ));
    std::string output_file(RESULT_FULL_PATH(wb-edu ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, webbase_1M)
{
    std::string input_file(MATRIX_FULL_PATH(webbase-1M ));
    std::string output_file(RESULT_FULL_PATH(webbase-1M ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, web_BerkStan)
{
    std::string input_file(MATRIX_FULL_PATH(web-BerkStan ));
    std::string output_file(RESULT_FULL_PATH(web-BerkStan ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, web_Google)
{
    std::string input_file(MATRIX_FULL_PATH(web-Google ));
    std::string output_file(RESULT_FULL_PATH(web-Google ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wikipedia_20051105)
{
    std::string input_file(MATRIX_FULL_PATH(wikipedia-20051105 ));
    std::string output_file(RESULT_FULL_PATH(wikipedia-20051105 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wikipedia_20060925)
{
    std::string input_file(MATRIX_FULL_PATH(wikipedia-20060925 ));
    std::string output_file(RESULT_FULL_PATH(wikipedia-20060925 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wikipedia_20061104)
{
    std::string input_file(MATRIX_FULL_PATH(wikipedia-20061104 ));
    std::string output_file(RESULT_FULL_PATH(wikipedia-20061104 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wikipedia_20070206)
{
    std::string input_file(MATRIX_FULL_PATH(wikipedia-20070206 ));
    std::string output_file(RESULT_FULL_PATH(wikipedia-20070206 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wiki_Talk)
{
    std::string input_file(MATRIX_FULL_PATH(wiki-Talk ));
    std::string output_file(RESULT_FULL_PATH(wiki-Talk ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 1138_bus)
{
    std::string input_file(MATRIX_FULL_PATH(1138_bus ));
    std::string output_file(RESULT_FULL_PATH(1138_bus ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 144)
{
    std::string input_file(MATRIX_FULL_PATH(144 ));
    std::string output_file(RESULT_FULL_PATH(144 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 2cubes_sphere)
{
    std::string input_file(MATRIX_FULL_PATH(2cubes_sphere ));
    std::string output_file(RESULT_FULL_PATH(2cubes_sphere ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 2D_27628_bjtcai)
{
    std::string input_file(MATRIX_FULL_PATH(2D_27628_bjtcai ));
    std::string output_file(RESULT_FULL_PATH(2D_27628_bjtcai ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 2D_54019_highK)
{
    std::string input_file(MATRIX_FULL_PATH(2D_54019_highK ));
    std::string output_file(RESULT_FULL_PATH(2D_54019_highK ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 3D_28984_Tetra)
{
    std::string input_file(MATRIX_FULL_PATH(3D_28984_Tetra ));
    std::string output_file(RESULT_FULL_PATH(3D_28984_Tetra ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 3D_51448_3D)
{
    std::string input_file(MATRIX_FULL_PATH(3D_51448_3D ));
    std::string output_file(RESULT_FULL_PATH(3D_51448_3D ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 3Dspectralwave2)
{
    std::string input_file(MATRIX_FULL_PATH(3Dspectralwave2 ));
    std::string output_file(RESULT_FULL_PATH(3Dspectralwave2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 3dtube)
{
    std::string input_file(MATRIX_FULL_PATH(3dtube ));
    std::string output_file(RESULT_FULL_PATH(3dtube ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 3elt_dual)
{
    std::string input_file(MATRIX_FULL_PATH(3elt_dual ));
    std::string output_file(RESULT_FULL_PATH(3elt_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 3elt)
{
    std::string input_file(MATRIX_FULL_PATH(3elt ));
    std::string output_file(RESULT_FULL_PATH(3elt ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 494_bus)
{
    std::string input_file(MATRIX_FULL_PATH(494_bus ));
    std::string output_file(RESULT_FULL_PATH(494_bus ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 598a)
{
    std::string input_file(MATRIX_FULL_PATH(598a ));
    std::string output_file(RESULT_FULL_PATH(598a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 662_bus)
{
    std::string input_file(MATRIX_FULL_PATH(662_bus ));
    std::string output_file(RESULT_FULL_PATH(662_bus ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, 685_bus)
{
    std::string input_file(MATRIX_FULL_PATH(685_bus ));
    std::string output_file(RESULT_FULL_PATH(685_bus ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, a0nsdsil)
{
    std::string input_file(MATRIX_FULL_PATH(a0nsdsil ));
    std::string output_file(RESULT_FULL_PATH(a0nsdsil ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, a2nnsnsl)
{
    std::string input_file(MATRIX_FULL_PATH(a2nnsnsl ));
    std::string output_file(RESULT_FULL_PATH(a2nnsnsl ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, a5esindl)
{
    std::string input_file(MATRIX_FULL_PATH(a5esindl ));
    std::string output_file(RESULT_FULL_PATH(a5esindl ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ABACUS_shell_hd)
{
    std::string input_file(MATRIX_FULL_PATH(ABACUS_shell_hd ));
    std::string output_file(RESULT_FULL_PATH(ABACUS_shell_hd ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ABACUS_shell_ld)
{
    std::string input_file(MATRIX_FULL_PATH(ABACUS_shell_ld ));
    std::string output_file(RESULT_FULL_PATH(ABACUS_shell_ld ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ABACUS_shell_md)
{
    std::string input_file(MATRIX_FULL_PATH(ABACUS_shell_md ));
    std::string output_file(RESULT_FULL_PATH(ABACUS_shell_md ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ABACUS_shell_ud)
{
    std::string input_file(MATRIX_FULL_PATH(ABACUS_shell_ud ));
    std::string output_file(RESULT_FULL_PATH(ABACUS_shell_ud ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, add20)
{
    std::string input_file(MATRIX_FULL_PATH(add20 ));
    std::string output_file(RESULT_FULL_PATH(add20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, add32)
{
    std::string input_file(MATRIX_FULL_PATH(add32 ));
    std::string output_file(RESULT_FULL_PATH(add32 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_01)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_01 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_02)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_02 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_03)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_03 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_04)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_04 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_05)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_05 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_06)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_06 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_07)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_07 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_08)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_08 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_09)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_09 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_10)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_10 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_11)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_11 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_12)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_12 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_13)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_13 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_14)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_14 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_15)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_15 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_16)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_16 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_17)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_17 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_18)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_18 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_19)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_19 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_20)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_20 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_21)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_21 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_22)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_22 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_23)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_23 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_24)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_24 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_25)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_25 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_26)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_26 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_27)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_27 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_27 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_28)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_28 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_28 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_29)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_29 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_29 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_30)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_30 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_31)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_31 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_31 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_32)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_32 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_32 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_33)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_33 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_33 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_34)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_34 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_34 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_35)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_35 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_35 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_36)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_36 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_36 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_37)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_37 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_37 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_38)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_38 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_38 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_39)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_39 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_39 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_40)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_40 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_40 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_41)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_41 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_41 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_42)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_42 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_42 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_43)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_43 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_43 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_44)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_44 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_44 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_45)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_45 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_45 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_46)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_46 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_46 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_47)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_47 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_47 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_48)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_48 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_48 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_49)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_49 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_49 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_50)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_50 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_50 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_51)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_51 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_51 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_52)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_52 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_52 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_53)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_53 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_53 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_54)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_54 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_54 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_55)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_55 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_55 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_56)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_56 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_56 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_57)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_57 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_57 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_58)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_58 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_58 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_59)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_59 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_59 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_60)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_60 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_60 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_61)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_61 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_61 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_62)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_62 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_62 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_63)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_63 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_63 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_64)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_64 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_64 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_65)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_65 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_65 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_66)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_66 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_66 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_67)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_67 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_67 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_68)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_68 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_68 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_dcop_69)
{
    std::string input_file(MATRIX_FULL_PATH(adder_dcop_69 ));
    std::string output_file(RESULT_FULL_PATH(adder_dcop_69 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_trans_01)
{
    std::string input_file(MATRIX_FULL_PATH(adder_trans_01 ));
    std::string output_file(RESULT_FULL_PATH(adder_trans_01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, adder_trans_02)
{
    std::string input_file(MATRIX_FULL_PATH(adder_trans_02 ));
    std::string output_file(RESULT_FULL_PATH(adder_trans_02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_0_k101)
{
    std::string input_file(MATRIX_FULL_PATH(af_0_k101 ));
    std::string output_file(RESULT_FULL_PATH(af_0_k101 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_1_k101)
{
    std::string input_file(MATRIX_FULL_PATH(af_1_k101 ));
    std::string output_file(RESULT_FULL_PATH(af_1_k101 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af23560)
{
    std::string input_file(MATRIX_FULL_PATH(af23560 ));
    std::string output_file(RESULT_FULL_PATH(af23560 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_2_k101)
{
    std::string input_file(MATRIX_FULL_PATH(af_2_k101 ));
    std::string output_file(RESULT_FULL_PATH(af_2_k101 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_3_k101)
{
    std::string input_file(MATRIX_FULL_PATH(af_3_k101 ));
    std::string output_file(RESULT_FULL_PATH(af_3_k101 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_4_k101)
{
    std::string input_file(MATRIX_FULL_PATH(af_4_k101 ));
    std::string output_file(RESULT_FULL_PATH(af_4_k101 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_5_k101)
{
    std::string input_file(MATRIX_FULL_PATH(af_5_k101 ));
    std::string output_file(RESULT_FULL_PATH(af_5_k101 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_shell1)
{
    std::string input_file(MATRIX_FULL_PATH(af_shell1 ));
    std::string output_file(RESULT_FULL_PATH(af_shell1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_shell2)
{
    std::string input_file(MATRIX_FULL_PATH(af_shell2 ));
    std::string output_file(RESULT_FULL_PATH(af_shell2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_shell3)
{
    std::string input_file(MATRIX_FULL_PATH(af_shell3 ));
    std::string output_file(RESULT_FULL_PATH(af_shell3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_shell4)
{
    std::string input_file(MATRIX_FULL_PATH(af_shell4 ));
    std::string output_file(RESULT_FULL_PATH(af_shell4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_shell5)
{
    std::string input_file(MATRIX_FULL_PATH(af_shell5 ));
    std::string output_file(RESULT_FULL_PATH(af_shell5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_shell6)
{
    std::string input_file(MATRIX_FULL_PATH(af_shell6 ));
    std::string output_file(RESULT_FULL_PATH(af_shell6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_shell7)
{
    std::string input_file(MATRIX_FULL_PATH(af_shell7 ));
    std::string output_file(RESULT_FULL_PATH(af_shell7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_shell8)
{
    std::string input_file(MATRIX_FULL_PATH(af_shell8 ));
    std::string output_file(RESULT_FULL_PATH(af_shell8 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, af_shell9)
{
    std::string input_file(MATRIX_FULL_PATH(af_shell9 ));
    std::string output_file(RESULT_FULL_PATH(af_shell9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, aft01)
{
    std::string input_file(MATRIX_FULL_PATH(aft01 ));
    std::string output_file(RESULT_FULL_PATH(aft01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, aft02)
{
    std::string input_file(MATRIX_FULL_PATH(aft02 ));
    std::string output_file(RESULT_FULL_PATH(aft02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, airfoil1_dual)
{
    std::string input_file(MATRIX_FULL_PATH(airfoil1_dual ));
    std::string output_file(RESULT_FULL_PATH(airfoil1_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, airfoil1)
{
    std::string input_file(MATRIX_FULL_PATH(airfoil1 ));
    std::string output_file(RESULT_FULL_PATH(airfoil1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, airfoil_2d)
{
    std::string input_file(MATRIX_FULL_PATH(airfoil_2d ));
    std::string output_file(RESULT_FULL_PATH(airfoil_2d ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Alemdar)
{
    std::string input_file(MATRIX_FULL_PATH(Alemdar ));
    std::string output_file(RESULT_FULL_PATH(Alemdar ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, amazon0302)
{
    std::string input_file(MATRIX_FULL_PATH(amazon0302 ));
    std::string output_file(RESULT_FULL_PATH(amazon0302 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, amazon0312)
{
    std::string input_file(MATRIX_FULL_PATH(amazon0312 ));
    std::string output_file(RESULT_FULL_PATH(amazon0312 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, amazon0505)
{
    std::string input_file(MATRIX_FULL_PATH(amazon0505 ));
    std::string output_file(RESULT_FULL_PATH(amazon0505 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, amazon0601)
{
    std::string input_file(MATRIX_FULL_PATH(amazon0601 ));
    std::string output_file(RESULT_FULL_PATH(amazon0601 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Andrews)
{
    std::string input_file(MATRIX_FULL_PATH(Andrews ));
    std::string output_file(RESULT_FULL_PATH(Andrews ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, apache1)
{
    std::string input_file(MATRIX_FULL_PATH(apache1 ));
    std::string output_file(RESULT_FULL_PATH(apache1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, appu)
{
    std::string input_file(MATRIX_FULL_PATH(appu ));
    std::string output_file(RESULT_FULL_PATH(appu ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, as_22july06)
{
    std::string input_file(MATRIX_FULL_PATH(as-22july06 ));
    std::string output_file(RESULT_FULL_PATH(as-22july06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, as_735)
{
    std::string input_file(MATRIX_FULL_PATH(as-735 ));
    std::string output_file(RESULT_FULL_PATH(as-735 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, as_caida)
{
    std::string input_file(MATRIX_FULL_PATH(as-caida ));
    std::string output_file(RESULT_FULL_PATH(as-caida ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ash292)
{
    std::string input_file(MATRIX_FULL_PATH(ash292 ));
    std::string output_file(RESULT_FULL_PATH(ash292 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ASIC_100ks)
{
    std::string input_file(MATRIX_FULL_PATH(ASIC_100ks ));
    std::string output_file(RESULT_FULL_PATH(ASIC_100ks ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ASIC_100k)
{
    std::string input_file(MATRIX_FULL_PATH(ASIC_100k ));
    std::string output_file(RESULT_FULL_PATH(ASIC_100k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ASIC_320ks)
{
    std::string input_file(MATRIX_FULL_PATH(ASIC_320ks ));
    std::string output_file(RESULT_FULL_PATH(ASIC_320ks ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ASIC_320k)
{
    std::string input_file(MATRIX_FULL_PATH(ASIC_320k ));
    std::string output_file(RESULT_FULL_PATH(ASIC_320k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, astro_ph)
{
    std::string input_file(MATRIX_FULL_PATH(astro-ph ));
    std::string output_file(RESULT_FULL_PATH(astro-ph ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, aug2dc)
{
    std::string input_file(MATRIX_FULL_PATH(aug2dc ));
    std::string output_file(RESULT_FULL_PATH(aug2dc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, aug2d)
{
    std::string input_file(MATRIX_FULL_PATH(aug2d ));
    std::string output_file(RESULT_FULL_PATH(aug2d ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, aug3dcqp)
{
    std::string input_file(MATRIX_FULL_PATH(aug3dcqp ));
    std::string output_file(RESULT_FULL_PATH(aug3dcqp ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, aug3d)
{
    std::string input_file(MATRIX_FULL_PATH(aug3d ));
    std::string output_file(RESULT_FULL_PATH(aug3d ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, auto)
{
    std::string input_file(MATRIX_FULL_PATH(auto ));
    std::string output_file(RESULT_FULL_PATH(auto ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, av41092)
{
    std::string input_file(MATRIX_FULL_PATH(av41092 ));
    std::string output_file(RESULT_FULL_PATH(av41092 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, b2_ss)
{
    std::string input_file(MATRIX_FULL_PATH(b2_ss ));
    std::string output_file(RESULT_FULL_PATH(b2_ss ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barrier2_10)
{
    std::string input_file(MATRIX_FULL_PATH(barrier2-10 ));
    std::string output_file(RESULT_FULL_PATH(barrier2-10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barrier2_11)
{
    std::string input_file(MATRIX_FULL_PATH(barrier2-11 ));
    std::string output_file(RESULT_FULL_PATH(barrier2-11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barrier2_12)
{
    std::string input_file(MATRIX_FULL_PATH(barrier2-12 ));
    std::string output_file(RESULT_FULL_PATH(barrier2-12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barrier2_1)
{
    std::string input_file(MATRIX_FULL_PATH(barrier2-1 ));
    std::string output_file(RESULT_FULL_PATH(barrier2-1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barrier2_2)
{
    std::string input_file(MATRIX_FULL_PATH(barrier2-2 ));
    std::string output_file(RESULT_FULL_PATH(barrier2-2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barrier2_3)
{
    std::string input_file(MATRIX_FULL_PATH(barrier2-3 ));
    std::string output_file(RESULT_FULL_PATH(barrier2-3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barrier2_4)
{
    std::string input_file(MATRIX_FULL_PATH(barrier2-4 ));
    std::string output_file(RESULT_FULL_PATH(barrier2-4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barrier2_9)
{
    std::string input_file(MATRIX_FULL_PATH(barrier2-9 ));
    std::string output_file(RESULT_FULL_PATH(barrier2-9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barth4)
{
    std::string input_file(MATRIX_FULL_PATH(barth4 ));
    std::string output_file(RESULT_FULL_PATH(barth4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barth5)
{
    std::string input_file(MATRIX_FULL_PATH(barth5 ));
    std::string output_file(RESULT_FULL_PATH(barth5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, barth)
{
    std::string input_file(MATRIX_FULL_PATH(barth ));
    std::string output_file(RESULT_FULL_PATH(barth ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Baumann)
{
    std::string input_file(MATRIX_FULL_PATH(Baumann ));
    std::string output_file(RESULT_FULL_PATH(Baumann ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bauru5727)
{
    std::string input_file(MATRIX_FULL_PATH(bauru5727 ));
    std::string output_file(RESULT_FULL_PATH(bauru5727 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bayer01)
{
    std::string input_file(MATRIX_FULL_PATH(bayer01 ));
    std::string output_file(RESULT_FULL_PATH(bayer01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bayer02)
{
    std::string input_file(MATRIX_FULL_PATH(bayer02 ));
    std::string output_file(RESULT_FULL_PATH(bayer02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bayer03)
{
    std::string input_file(MATRIX_FULL_PATH(bayer03 ));
    std::string output_file(RESULT_FULL_PATH(bayer03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bayer04)
{
    std::string input_file(MATRIX_FULL_PATH(bayer04 ));
    std::string output_file(RESULT_FULL_PATH(bayer04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bayer05)
{
    std::string input_file(MATRIX_FULL_PATH(bayer05 ));
    std::string output_file(RESULT_FULL_PATH(bayer05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bayer06)
{
    std::string input_file(MATRIX_FULL_PATH(bayer06 ));
    std::string output_file(RESULT_FULL_PATH(bayer06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bayer07)
{
    std::string input_file(MATRIX_FULL_PATH(bayer07 ));
    std::string output_file(RESULT_FULL_PATH(bayer07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bayer08)
{
    std::string input_file(MATRIX_FULL_PATH(bayer08 ));
    std::string output_file(RESULT_FULL_PATH(bayer08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bayer09)
{
    std::string input_file(MATRIX_FULL_PATH(bayer09 ));
    std::string output_file(RESULT_FULL_PATH(bayer09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bayer10)
{
    std::string input_file(MATRIX_FULL_PATH(bayer10 ));
    std::string output_file(RESULT_FULL_PATH(bayer10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bbmat)
{
    std::string input_file(MATRIX_FULL_PATH(bbmat ));
    std::string output_file(RESULT_FULL_PATH(bbmat ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcircuit)
{
    std::string input_file(MATRIX_FULL_PATH(bcircuit ));
    std::string output_file(RESULT_FULL_PATH(bcircuit ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcspwr04)
{
    std::string input_file(MATRIX_FULL_PATH(bcspwr04 ));
    std::string output_file(RESULT_FULL_PATH(bcspwr04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcspwr05)
{
    std::string input_file(MATRIX_FULL_PATH(bcspwr05 ));
    std::string output_file(RESULT_FULL_PATH(bcspwr05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcspwr06)
{
    std::string input_file(MATRIX_FULL_PATH(bcspwr06 ));
    std::string output_file(RESULT_FULL_PATH(bcspwr06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcspwr07)
{
    std::string input_file(MATRIX_FULL_PATH(bcspwr07 ));
    std::string output_file(RESULT_FULL_PATH(bcspwr07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcspwr08)
{
    std::string input_file(MATRIX_FULL_PATH(bcspwr08 ));
    std::string output_file(RESULT_FULL_PATH(bcspwr08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcspwr09)
{
    std::string input_file(MATRIX_FULL_PATH(bcspwr09 ));
    std::string output_file(RESULT_FULL_PATH(bcspwr09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcspwr10)
{
    std::string input_file(MATRIX_FULL_PATH(bcspwr10 ));
    std::string output_file(RESULT_FULL_PATH(bcspwr10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk04)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk04 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk05)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk05 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk06)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk06 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk07)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk07 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk08)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk08 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk09)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk09 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk10)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk10 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk11)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk11 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk12)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk12 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk13)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk13 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk14)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk14 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk15)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk15 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk16)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk16 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk17)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk17 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk18)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk18 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk19)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk19 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk20)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk20 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk21)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk21 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk23)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk23 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk24)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk24 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk25)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk25 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk26)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk26 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk27)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk27 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk27 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk28)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk28 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk28 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk29)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk29 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk29 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk30)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk30 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk31)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk31 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk31 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk32)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk32 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk32 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk33)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk33 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk33 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk34)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk34 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk34 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk35)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk35 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk35 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk36)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk36 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk36 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk37)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk37 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk37 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk38)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk38 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk38 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstk39)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstk39 ));
    std::string output_file(RESULT_FULL_PATH(bcsstk39 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm07)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm07 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm08)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm08 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm09)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm09 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm10)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm10 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm11)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm11 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm12)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm12 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm13)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm13 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm21)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm21 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm22)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm22 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm23)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm23 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm24)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm24 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm25)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm25 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm26)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm26 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm27)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm27 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm27 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm34)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm34 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm34 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm35)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm35 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm35 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm36)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm36 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm36 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm37)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm37 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm37 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm38)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm38 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm38 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bcsstm39)
{
    std::string input_file(MATRIX_FULL_PATH(bcsstm39 ));
    std::string output_file(RESULT_FULL_PATH(bcsstm39 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, b_dyn)
{
    std::string input_file(MATRIX_FULL_PATH(b_dyn ));
    std::string output_file(RESULT_FULL_PATH(b_dyn ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, BenElechi1)
{
    std::string input_file(MATRIX_FULL_PATH(BenElechi1 ));
    std::string output_file(RESULT_FULL_PATH(BenElechi1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, benzene)
{
    std::string input_file(MATRIX_FULL_PATH(benzene ));
    std::string output_file(RESULT_FULL_PATH(benzene ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bfly)
{
    std::string input_file(MATRIX_FULL_PATH(bfly ));
    std::string output_file(RESULT_FULL_PATH(bfly ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bfwa398)
{
    std::string input_file(MATRIX_FULL_PATH(bfwa398 ));
    std::string output_file(RESULT_FULL_PATH(bfwa398 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bfwa782)
{
    std::string input_file(MATRIX_FULL_PATH(bfwa782 ));
    std::string output_file(RESULT_FULL_PATH(bfwa782 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bfwb398)
{
    std::string input_file(MATRIX_FULL_PATH(bfwb398 ));
    std::string output_file(RESULT_FULL_PATH(bfwb398 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bfwb782)
{
    std::string input_file(MATRIX_FULL_PATH(bfwb782 ));
    std::string output_file(RESULT_FULL_PATH(bfwb782 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bibd_81_2)
{
    std::string input_file(MATRIX_FULL_PATH(bibd_81_2 ));
    std::string output_file(RESULT_FULL_PATH(bibd_81_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, big_dual)
{
    std::string input_file(MATRIX_FULL_PATH(big_dual ));
    std::string output_file(RESULT_FULL_PATH(big_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, big)
{
    std::string input_file(MATRIX_FULL_PATH(big ));
    std::string output_file(RESULT_FULL_PATH(big ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, biplane_9)
{
    std::string input_file(MATRIX_FULL_PATH(biplane-9 ));
    std::string output_file(RESULT_FULL_PATH(biplane-9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bips07_1693)
{
    std::string input_file(MATRIX_FULL_PATH(bips07_1693 ));
    std::string output_file(RESULT_FULL_PATH(bips07_1693 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bips07_1998)
{
    std::string input_file(MATRIX_FULL_PATH(bips07_1998 ));
    std::string output_file(RESULT_FULL_PATH(bips07_1998 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bips07_2476)
{
    std::string input_file(MATRIX_FULL_PATH(bips07_2476 ));
    std::string output_file(RESULT_FULL_PATH(bips07_2476 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bips07_3078)
{
    std::string input_file(MATRIX_FULL_PATH(bips07_3078 ));
    std::string output_file(RESULT_FULL_PATH(bips07_3078 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bips98_1142)
{
    std::string input_file(MATRIX_FULL_PATH(bips98_1142 ));
    std::string output_file(RESULT_FULL_PATH(bips98_1142 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bips98_1450)
{
    std::string input_file(MATRIX_FULL_PATH(bips98_1450 ));
    std::string output_file(RESULT_FULL_PATH(bips98_1450 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bips98_606)
{
    std::string input_file(MATRIX_FULL_PATH(bips98_606 ));
    std::string output_file(RESULT_FULL_PATH(bips98_606 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, blckhole)
{
    std::string input_file(MATRIX_FULL_PATH(blckhole ));
    std::string output_file(RESULT_FULL_PATH(blckhole ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, blockqp1)
{
    std::string input_file(MATRIX_FULL_PATH(blockqp1 ));
    std::string output_file(RESULT_FULL_PATH(blockqp1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bloweya)
{
    std::string input_file(MATRIX_FULL_PATH(bloweya ));
    std::string output_file(RESULT_FULL_PATH(bloweya ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bloweybl)
{
    std::string input_file(MATRIX_FULL_PATH(bloweybl ));
    std::string output_file(RESULT_FULL_PATH(bloweybl ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bloweybq)
{
    std::string input_file(MATRIX_FULL_PATH(bloweybq ));
    std::string output_file(RESULT_FULL_PATH(bloweybq ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bmw3_2)
{
    std::string input_file(MATRIX_FULL_PATH(bmw3_2 ));
    std::string output_file(RESULT_FULL_PATH(bmw3_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bmw7st_1)
{
    std::string input_file(MATRIX_FULL_PATH(bmw7st_1 ));
    std::string output_file(RESULT_FULL_PATH(bmw7st_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bmwcra_1)
{
    std::string input_file(MATRIX_FULL_PATH(bmwcra_1 ));
    std::string output_file(RESULT_FULL_PATH(bmwcra_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bodyy4)
{
    std::string input_file(MATRIX_FULL_PATH(bodyy4 ));
    std::string output_file(RESULT_FULL_PATH(bodyy4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bodyy5)
{
    std::string input_file(MATRIX_FULL_PATH(bodyy5 ));
    std::string output_file(RESULT_FULL_PATH(bodyy5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bodyy6)
{
    std::string input_file(MATRIX_FULL_PATH(bodyy6 ));
    std::string output_file(RESULT_FULL_PATH(bodyy6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, boneS01)
{
    std::string input_file(MATRIX_FULL_PATH(boneS01 ));
    std::string output_file(RESULT_FULL_PATH(boneS01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, boyd1)
{
    std::string input_file(MATRIX_FULL_PATH(boyd1 ));
    std::string output_file(RESULT_FULL_PATH(boyd1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, boyd2)
{
    std::string input_file(MATRIX_FULL_PATH(boyd2 ));
    std::string output_file(RESULT_FULL_PATH(boyd2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bp_0)
{
    std::string input_file(MATRIX_FULL_PATH(bp_0 ));
    std::string output_file(RESULT_FULL_PATH(bp_0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bp_1000)
{
    std::string input_file(MATRIX_FULL_PATH(bp_1000 ));
    std::string output_file(RESULT_FULL_PATH(bp_1000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bp_1200)
{
    std::string input_file(MATRIX_FULL_PATH(bp_1200 ));
    std::string output_file(RESULT_FULL_PATH(bp_1200 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bp_1400)
{
    std::string input_file(MATRIX_FULL_PATH(bp_1400 ));
    std::string output_file(RESULT_FULL_PATH(bp_1400 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bp_1600)
{
    std::string input_file(MATRIX_FULL_PATH(bp_1600 ));
    std::string output_file(RESULT_FULL_PATH(bp_1600 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bp_200)
{
    std::string input_file(MATRIX_FULL_PATH(bp_200 ));
    std::string output_file(RESULT_FULL_PATH(bp_200 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bp_400)
{
    std::string input_file(MATRIX_FULL_PATH(bp_400 ));
    std::string output_file(RESULT_FULL_PATH(bp_400 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bp_600)
{
    std::string input_file(MATRIX_FULL_PATH(bp_600 ));
    std::string output_file(RESULT_FULL_PATH(bp_600 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bp_800)
{
    std::string input_file(MATRIX_FULL_PATH(bp_800 ));
    std::string output_file(RESULT_FULL_PATH(bp_800 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, brack2)
{
    std::string input_file(MATRIX_FULL_PATH(brack2 ));
    std::string output_file(RESULT_FULL_PATH(brack2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, brainpc2)
{
    std::string input_file(MATRIX_FULL_PATH(brainpc2 ));
    std::string output_file(RESULT_FULL_PATH(brainpc2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bratu3d)
{
    std::string input_file(MATRIX_FULL_PATH(bratu3d ));
    std::string output_file(RESULT_FULL_PATH(bratu3d ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bundle1)
{
    std::string input_file(MATRIX_FULL_PATH(bundle1 ));
    std::string output_file(RESULT_FULL_PATH(bundle1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, bwm2000)
{
    std::string input_file(MATRIX_FULL_PATH(bwm2000 ));
    std::string output_file(RESULT_FULL_PATH(bwm2000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_18)
{
    std::string input_file(MATRIX_FULL_PATH(c-18 ));
    std::string output_file(RESULT_FULL_PATH(c-18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_19)
{
    std::string input_file(MATRIX_FULL_PATH(c-19 ));
    std::string output_file(RESULT_FULL_PATH(c-19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_20)
{
    std::string input_file(MATRIX_FULL_PATH(c-20 ));
    std::string output_file(RESULT_FULL_PATH(c-20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_21)
{
    std::string input_file(MATRIX_FULL_PATH(c-21 ));
    std::string output_file(RESULT_FULL_PATH(c-21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_22)
{
    std::string input_file(MATRIX_FULL_PATH(c-22 ));
    std::string output_file(RESULT_FULL_PATH(c-22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_23)
{
    std::string input_file(MATRIX_FULL_PATH(c-23 ));
    std::string output_file(RESULT_FULL_PATH(c-23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_24)
{
    std::string input_file(MATRIX_FULL_PATH(c-24 ));
    std::string output_file(RESULT_FULL_PATH(c-24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_25)
{
    std::string input_file(MATRIX_FULL_PATH(c-25 ));
    std::string output_file(RESULT_FULL_PATH(c-25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_26)
{
    std::string input_file(MATRIX_FULL_PATH(c-26 ));
    std::string output_file(RESULT_FULL_PATH(c-26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_27)
{
    std::string input_file(MATRIX_FULL_PATH(c-27 ));
    std::string output_file(RESULT_FULL_PATH(c-27 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_28)
{
    std::string input_file(MATRIX_FULL_PATH(c-28 ));
    std::string output_file(RESULT_FULL_PATH(c-28 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_29)
{
    std::string input_file(MATRIX_FULL_PATH(c-29 ));
    std::string output_file(RESULT_FULL_PATH(c-29 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_30)
{
    std::string input_file(MATRIX_FULL_PATH(c-30 ));
    std::string output_file(RESULT_FULL_PATH(c-30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_31)
{
    std::string input_file(MATRIX_FULL_PATH(c-31 ));
    std::string output_file(RESULT_FULL_PATH(c-31 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_32)
{
    std::string input_file(MATRIX_FULL_PATH(c-32 ));
    std::string output_file(RESULT_FULL_PATH(c-32 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_33)
{
    std::string input_file(MATRIX_FULL_PATH(c-33 ));
    std::string output_file(RESULT_FULL_PATH(c-33 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_34)
{
    std::string input_file(MATRIX_FULL_PATH(c-34 ));
    std::string output_file(RESULT_FULL_PATH(c-34 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_35)
{
    std::string input_file(MATRIX_FULL_PATH(c-35 ));
    std::string output_file(RESULT_FULL_PATH(c-35 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_36)
{
    std::string input_file(MATRIX_FULL_PATH(c-36 ));
    std::string output_file(RESULT_FULL_PATH(c-36 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_37)
{
    std::string input_file(MATRIX_FULL_PATH(c-37 ));
    std::string output_file(RESULT_FULL_PATH(c-37 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_38)
{
    std::string input_file(MATRIX_FULL_PATH(c-38 ));
    std::string output_file(RESULT_FULL_PATH(c-38 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_39)
{
    std::string input_file(MATRIX_FULL_PATH(c-39 ));
    std::string output_file(RESULT_FULL_PATH(c-39 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_40)
{
    std::string input_file(MATRIX_FULL_PATH(c-40 ));
    std::string output_file(RESULT_FULL_PATH(c-40 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_41)
{
    std::string input_file(MATRIX_FULL_PATH(c-41 ));
    std::string output_file(RESULT_FULL_PATH(c-41 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_42)
{
    std::string input_file(MATRIX_FULL_PATH(c-42 ));
    std::string output_file(RESULT_FULL_PATH(c-42 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_43)
{
    std::string input_file(MATRIX_FULL_PATH(c-43 ));
    std::string output_file(RESULT_FULL_PATH(c-43 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_44)
{
    std::string input_file(MATRIX_FULL_PATH(c-44 ));
    std::string output_file(RESULT_FULL_PATH(c-44 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_45)
{
    std::string input_file(MATRIX_FULL_PATH(c-45 ));
    std::string output_file(RESULT_FULL_PATH(c-45 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_46)
{
    std::string input_file(MATRIX_FULL_PATH(c-46 ));
    std::string output_file(RESULT_FULL_PATH(c-46 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_47)
{
    std::string input_file(MATRIX_FULL_PATH(c-47 ));
    std::string output_file(RESULT_FULL_PATH(c-47 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_48)
{
    std::string input_file(MATRIX_FULL_PATH(c-48 ));
    std::string output_file(RESULT_FULL_PATH(c-48 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_49)
{
    std::string input_file(MATRIX_FULL_PATH(c-49 ));
    std::string output_file(RESULT_FULL_PATH(c-49 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_50)
{
    std::string input_file(MATRIX_FULL_PATH(c-50 ));
    std::string output_file(RESULT_FULL_PATH(c-50 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_51)
{
    std::string input_file(MATRIX_FULL_PATH(c-51 ));
    std::string output_file(RESULT_FULL_PATH(c-51 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_52)
{
    std::string input_file(MATRIX_FULL_PATH(c-52 ));
    std::string output_file(RESULT_FULL_PATH(c-52 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_53)
{
    std::string input_file(MATRIX_FULL_PATH(c-53 ));
    std::string output_file(RESULT_FULL_PATH(c-53 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_54)
{
    std::string input_file(MATRIX_FULL_PATH(c-54 ));
    std::string output_file(RESULT_FULL_PATH(c-54 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_55)
{
    std::string input_file(MATRIX_FULL_PATH(c-55 ));
    std::string output_file(RESULT_FULL_PATH(c-55 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_56)
{
    std::string input_file(MATRIX_FULL_PATH(c-56 ));
    std::string output_file(RESULT_FULL_PATH(c-56 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_57)
{
    std::string input_file(MATRIX_FULL_PATH(c-57 ));
    std::string output_file(RESULT_FULL_PATH(c-57 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_58)
{
    std::string input_file(MATRIX_FULL_PATH(c-58 ));
    std::string output_file(RESULT_FULL_PATH(c-58 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_59)
{
    std::string input_file(MATRIX_FULL_PATH(c-59 ));
    std::string output_file(RESULT_FULL_PATH(c-59 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_60)
{
    std::string input_file(MATRIX_FULL_PATH(c-60 ));
    std::string output_file(RESULT_FULL_PATH(c-60 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_61)
{
    std::string input_file(MATRIX_FULL_PATH(c-61 ));
    std::string output_file(RESULT_FULL_PATH(c-61 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_62ghs)
{
    std::string input_file(MATRIX_FULL_PATH(c-62ghs ));
    std::string output_file(RESULT_FULL_PATH(c-62ghs ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_62)
{
    std::string input_file(MATRIX_FULL_PATH(c-62 ));
    std::string output_file(RESULT_FULL_PATH(c-62 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_63)
{
    std::string input_file(MATRIX_FULL_PATH(c-63 ));
    std::string output_file(RESULT_FULL_PATH(c-63 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_64b)
{
    std::string input_file(MATRIX_FULL_PATH(c-64b ));
    std::string output_file(RESULT_FULL_PATH(c-64b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_64)
{
    std::string input_file(MATRIX_FULL_PATH(c-64 ));
    std::string output_file(RESULT_FULL_PATH(c-64 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_65)
{
    std::string input_file(MATRIX_FULL_PATH(c-65 ));
    std::string output_file(RESULT_FULL_PATH(c-65 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_66b)
{
    std::string input_file(MATRIX_FULL_PATH(c-66b ));
    std::string output_file(RESULT_FULL_PATH(c-66b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_66)
{
    std::string input_file(MATRIX_FULL_PATH(c-66 ));
    std::string output_file(RESULT_FULL_PATH(c-66 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_67b)
{
    std::string input_file(MATRIX_FULL_PATH(c-67b ));
    std::string output_file(RESULT_FULL_PATH(c-67b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_67)
{
    std::string input_file(MATRIX_FULL_PATH(c-67 ));
    std::string output_file(RESULT_FULL_PATH(c-67 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_68)
{
    std::string input_file(MATRIX_FULL_PATH(c-68 ));
    std::string output_file(RESULT_FULL_PATH(c-68 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_69)
{
    std::string input_file(MATRIX_FULL_PATH(c-69 ));
    std::string output_file(RESULT_FULL_PATH(c-69 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_70)
{
    std::string input_file(MATRIX_FULL_PATH(c-70 ));
    std::string output_file(RESULT_FULL_PATH(c-70 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_71)
{
    std::string input_file(MATRIX_FULL_PATH(c-71 ));
    std::string output_file(RESULT_FULL_PATH(c-71 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_72)
{
    std::string input_file(MATRIX_FULL_PATH(c-72 ));
    std::string output_file(RESULT_FULL_PATH(c-72 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_73b)
{
    std::string input_file(MATRIX_FULL_PATH(c-73b ));
    std::string output_file(RESULT_FULL_PATH(c-73b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_73)
{
    std::string input_file(MATRIX_FULL_PATH(c-73 ));
    std::string output_file(RESULT_FULL_PATH(c-73 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ca_AstroPh)
{
    std::string input_file(MATRIX_FULL_PATH(ca-AstroPh ));
    std::string output_file(RESULT_FULL_PATH(ca-AstroPh ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ca_CondMat)
{
    std::string input_file(MATRIX_FULL_PATH(ca-CondMat ));
    std::string output_file(RESULT_FULL_PATH(ca-CondMat ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cage10)
{
    std::string input_file(MATRIX_FULL_PATH(cage10 ));
    std::string output_file(RESULT_FULL_PATH(cage10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cage11)
{
    std::string input_file(MATRIX_FULL_PATH(cage11 ));
    std::string output_file(RESULT_FULL_PATH(cage11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cage12)
{
    std::string input_file(MATRIX_FULL_PATH(cage12 ));
    std::string output_file(RESULT_FULL_PATH(cage12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cage13)
{
    std::string input_file(MATRIX_FULL_PATH(cage13 ));
    std::string output_file(RESULT_FULL_PATH(cage13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cage7)
{
    std::string input_file(MATRIX_FULL_PATH(cage7 ));
    std::string output_file(RESULT_FULL_PATH(cage7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cage8)
{
    std::string input_file(MATRIX_FULL_PATH(cage8 ));
    std::string output_file(RESULT_FULL_PATH(cage8 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cage9)
{
    std::string input_file(MATRIX_FULL_PATH(cage9 ));
    std::string output_file(RESULT_FULL_PATH(cage9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cage)
{
    std::string input_file(MATRIX_FULL_PATH(cage ));
    std::string output_file(RESULT_FULL_PATH(cage ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, CAG_mat1916)
{
    std::string input_file(MATRIX_FULL_PATH(CAG_mat1916 ));
    std::string output_file(RESULT_FULL_PATH(CAG_mat1916 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, CAG_mat364)
{
    std::string input_file(MATRIX_FULL_PATH(CAG_mat364 ));
    std::string output_file(RESULT_FULL_PATH(CAG_mat364 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ca_GrQc)
{
    std::string input_file(MATRIX_FULL_PATH(ca-GrQc ));
    std::string output_file(RESULT_FULL_PATH(ca-GrQc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ca_HepPh)
{
    std::string input_file(MATRIX_FULL_PATH(ca-HepPh ));
    std::string output_file(RESULT_FULL_PATH(ca-HepPh ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ca_HepTh)
{
    std::string input_file(MATRIX_FULL_PATH(ca-HepTh ));
    std::string output_file(RESULT_FULL_PATH(ca-HepTh ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, caidaRouterLevel)
{
    std::string input_file(MATRIX_FULL_PATH(caidaRouterLevel ));
    std::string output_file(RESULT_FULL_PATH(caidaRouterLevel ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, California)
{
    std::string input_file(MATRIX_FULL_PATH(California ));
    std::string output_file(RESULT_FULL_PATH(California ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_1054)
{
    std::string input_file(MATRIX_FULL_PATH(can_1054 ));
    std::string output_file(RESULT_FULL_PATH(can_1054 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_1072)
{
    std::string input_file(MATRIX_FULL_PATH(can_1072 ));
    std::string output_file(RESULT_FULL_PATH(can_1072 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_144)
{
    std::string input_file(MATRIX_FULL_PATH(can_144 ));
    std::string output_file(RESULT_FULL_PATH(can_144 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_161)
{
    std::string input_file(MATRIX_FULL_PATH(can_161 ));
    std::string output_file(RESULT_FULL_PATH(can_161 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_187)
{
    std::string input_file(MATRIX_FULL_PATH(can_187 ));
    std::string output_file(RESULT_FULL_PATH(can_187 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_229)
{
    std::string input_file(MATRIX_FULL_PATH(can_229 ));
    std::string output_file(RESULT_FULL_PATH(can_229 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_256)
{
    std::string input_file(MATRIX_FULL_PATH(can_256 ));
    std::string output_file(RESULT_FULL_PATH(can_256 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_268)
{
    std::string input_file(MATRIX_FULL_PATH(can_268 ));
    std::string output_file(RESULT_FULL_PATH(can_268 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_292)
{
    std::string input_file(MATRIX_FULL_PATH(can_292 ));
    std::string output_file(RESULT_FULL_PATH(can_292 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_445)
{
    std::string input_file(MATRIX_FULL_PATH(can_445 ));
    std::string output_file(RESULT_FULL_PATH(can_445 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_634)
{
    std::string input_file(MATRIX_FULL_PATH(can_634 ));
    std::string output_file(RESULT_FULL_PATH(can_634 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_715)
{
    std::string input_file(MATRIX_FULL_PATH(can_715 ));
    std::string output_file(RESULT_FULL_PATH(can_715 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, can_838)
{
    std::string input_file(MATRIX_FULL_PATH(can_838 ));
    std::string output_file(RESULT_FULL_PATH(can_838 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cant)
{
    std::string input_file(MATRIX_FULL_PATH(cant ));
    std::string output_file(RESULT_FULL_PATH(cant ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, case39)
{
    std::string input_file(MATRIX_FULL_PATH(case39 ));
    std::string output_file(RESULT_FULL_PATH(case39 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, case9)
{
    std::string input_file(MATRIX_FULL_PATH(case9 ));
    std::string output_file(RESULT_FULL_PATH(case9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity01)
{
    std::string input_file(MATRIX_FULL_PATH(cavity01 ));
    std::string output_file(RESULT_FULL_PATH(cavity01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity02)
{
    std::string input_file(MATRIX_FULL_PATH(cavity02 ));
    std::string output_file(RESULT_FULL_PATH(cavity02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity03)
{
    std::string input_file(MATRIX_FULL_PATH(cavity03 ));
    std::string output_file(RESULT_FULL_PATH(cavity03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity04)
{
    std::string input_file(MATRIX_FULL_PATH(cavity04 ));
    std::string output_file(RESULT_FULL_PATH(cavity04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity05)
{
    std::string input_file(MATRIX_FULL_PATH(cavity05 ));
    std::string output_file(RESULT_FULL_PATH(cavity05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity06)
{
    std::string input_file(MATRIX_FULL_PATH(cavity06 ));
    std::string output_file(RESULT_FULL_PATH(cavity06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity07)
{
    std::string input_file(MATRIX_FULL_PATH(cavity07 ));
    std::string output_file(RESULT_FULL_PATH(cavity07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity08)
{
    std::string input_file(MATRIX_FULL_PATH(cavity08 ));
    std::string output_file(RESULT_FULL_PATH(cavity08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity09)
{
    std::string input_file(MATRIX_FULL_PATH(cavity09 ));
    std::string output_file(RESULT_FULL_PATH(cavity09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity10)
{
    std::string input_file(MATRIX_FULL_PATH(cavity10 ));
    std::string output_file(RESULT_FULL_PATH(cavity10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity11)
{
    std::string input_file(MATRIX_FULL_PATH(cavity11 ));
    std::string output_file(RESULT_FULL_PATH(cavity11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity12)
{
    std::string input_file(MATRIX_FULL_PATH(cavity12 ));
    std::string output_file(RESULT_FULL_PATH(cavity12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity13)
{
    std::string input_file(MATRIX_FULL_PATH(cavity13 ));
    std::string output_file(RESULT_FULL_PATH(cavity13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity14)
{
    std::string input_file(MATRIX_FULL_PATH(cavity14 ));
    std::string output_file(RESULT_FULL_PATH(cavity14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity15)
{
    std::string input_file(MATRIX_FULL_PATH(cavity15 ));
    std::string output_file(RESULT_FULL_PATH(cavity15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity16)
{
    std::string input_file(MATRIX_FULL_PATH(cavity16 ));
    std::string output_file(RESULT_FULL_PATH(cavity16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity17)
{
    std::string input_file(MATRIX_FULL_PATH(cavity17 ));
    std::string output_file(RESULT_FULL_PATH(cavity17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity18)
{
    std::string input_file(MATRIX_FULL_PATH(cavity18 ));
    std::string output_file(RESULT_FULL_PATH(cavity18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity19)
{
    std::string input_file(MATRIX_FULL_PATH(cavity19 ));
    std::string output_file(RESULT_FULL_PATH(cavity19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity20)
{
    std::string input_file(MATRIX_FULL_PATH(cavity20 ));
    std::string output_file(RESULT_FULL_PATH(cavity20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity21)
{
    std::string input_file(MATRIX_FULL_PATH(cavity21 ));
    std::string output_file(RESULT_FULL_PATH(cavity21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity22)
{
    std::string input_file(MATRIX_FULL_PATH(cavity22 ));
    std::string output_file(RESULT_FULL_PATH(cavity22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity23)
{
    std::string input_file(MATRIX_FULL_PATH(cavity23 ));
    std::string output_file(RESULT_FULL_PATH(cavity23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity24)
{
    std::string input_file(MATRIX_FULL_PATH(cavity24 ));
    std::string output_file(RESULT_FULL_PATH(cavity24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity25)
{
    std::string input_file(MATRIX_FULL_PATH(cavity25 ));
    std::string output_file(RESULT_FULL_PATH(cavity25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cavity26)
{
    std::string input_file(MATRIX_FULL_PATH(cavity26 ));
    std::string output_file(RESULT_FULL_PATH(cavity26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, c_big)
{
    std::string input_file(MATRIX_FULL_PATH(c-big ));
    std::string output_file(RESULT_FULL_PATH(c-big ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cbuckle)
{
    std::string input_file(MATRIX_FULL_PATH(cbuckle ));
    std::string output_file(RESULT_FULL_PATH(cbuckle ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cca)
{
    std::string input_file(MATRIX_FULL_PATH(cca ));
    std::string output_file(RESULT_FULL_PATH(cca ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ccc)
{
    std::string input_file(MATRIX_FULL_PATH(ccc ));
    std::string output_file(RESULT_FULL_PATH(ccc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cdde1)
{
    std::string input_file(MATRIX_FULL_PATH(cdde1 ));
    std::string output_file(RESULT_FULL_PATH(cdde1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cdde2)
{
    std::string input_file(MATRIX_FULL_PATH(cdde2 ));
    std::string output_file(RESULT_FULL_PATH(cdde2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cdde3)
{
    std::string input_file(MATRIX_FULL_PATH(cdde3 ));
    std::string output_file(RESULT_FULL_PATH(cdde3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cdde4)
{
    std::string input_file(MATRIX_FULL_PATH(cdde4 ));
    std::string output_file(RESULT_FULL_PATH(cdde4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cdde5)
{
    std::string input_file(MATRIX_FULL_PATH(cdde5 ));
    std::string output_file(RESULT_FULL_PATH(cdde5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cdde6)
{
    std::string input_file(MATRIX_FULL_PATH(cdde6 ));
    std::string output_file(RESULT_FULL_PATH(cdde6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cegb2802)
{
    std::string input_file(MATRIX_FULL_PATH(cegb2802 ));
    std::string output_file(RESULT_FULL_PATH(cegb2802 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cegb2919)
{
    std::string input_file(MATRIX_FULL_PATH(cegb2919 ));
    std::string output_file(RESULT_FULL_PATH(cegb2919 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cegb3024)
{
    std::string input_file(MATRIX_FULL_PATH(cegb3024 ));
    std::string output_file(RESULT_FULL_PATH(cegb3024 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cegb3306)
{
    std::string input_file(MATRIX_FULL_PATH(cegb3306 ));
    std::string output_file(RESULT_FULL_PATH(cegb3306 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, celegans_metabolic)
{
    std::string input_file(MATRIX_FULL_PATH(celegans_metabolic ));
    std::string output_file(RESULT_FULL_PATH(celegans_metabolic ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, celegansneural)
{
    std::string input_file(MATRIX_FULL_PATH(celegansneural ));
    std::string output_file(RESULT_FULL_PATH(celegansneural ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cell1)
{
    std::string input_file(MATRIX_FULL_PATH(cell1 ));
    std::string output_file(RESULT_FULL_PATH(cell1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cell2)
{
    std::string input_file(MATRIX_FULL_PATH(cell2 ));
    std::string output_file(RESULT_FULL_PATH(cell2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cfd1)
{
    std::string input_file(MATRIX_FULL_PATH(cfd1 ));
    std::string output_file(RESULT_FULL_PATH(cfd1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cfd2)
{
    std::string input_file(MATRIX_FULL_PATH(cfd2 ));
    std::string output_file(RESULT_FULL_PATH(cfd2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ch5_5_b3)
{
    std::string input_file(MATRIX_FULL_PATH(ch5-5-b3 ));
    std::string output_file(RESULT_FULL_PATH(ch5-5-b3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ch7_8_b5)
{
    std::string input_file(MATRIX_FULL_PATH(ch7-8-b5 ));
    std::string output_file(RESULT_FULL_PATH(ch7-8-b5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Chebyshev1)
{
    std::string input_file(MATRIX_FULL_PATH(Chebyshev1 ));
    std::string output_file(RESULT_FULL_PATH(Chebyshev1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Chebyshev2)
{
    std::string input_file(MATRIX_FULL_PATH(Chebyshev2 ));
    std::string output_file(RESULT_FULL_PATH(Chebyshev2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Chebyshev3)
{
    std::string input_file(MATRIX_FULL_PATH(Chebyshev3 ));
    std::string output_file(RESULT_FULL_PATH(Chebyshev3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Chebyshev4)
{
    std::string input_file(MATRIX_FULL_PATH(Chebyshev4 ));
    std::string output_file(RESULT_FULL_PATH(Chebyshev4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Chem97ZtZ)
{
    std::string input_file(MATRIX_FULL_PATH(Chem97ZtZ ));
    std::string output_file(RESULT_FULL_PATH(Chem97ZtZ ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, chem_master1)
{
    std::string input_file(MATRIX_FULL_PATH(chem_master1 ));
    std::string output_file(RESULT_FULL_PATH(chem_master1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, chipcool0)
{
    std::string input_file(MATRIX_FULL_PATH(chipcool0 ));
    std::string output_file(RESULT_FULL_PATH(chipcool0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, chipcool1)
{
    std::string input_file(MATRIX_FULL_PATH(chipcool1 ));
    std::string output_file(RESULT_FULL_PATH(chipcool1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, circuit_1)
{
    std::string input_file(MATRIX_FULL_PATH(circuit_1 ));
    std::string output_file(RESULT_FULL_PATH(circuit_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, circuit204)
{
    std::string input_file(MATRIX_FULL_PATH(circuit204 ));
    std::string output_file(RESULT_FULL_PATH(circuit204 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, circuit_2)
{
    std::string input_file(MATRIX_FULL_PATH(circuit_2 ));
    std::string output_file(RESULT_FULL_PATH(circuit_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, circuit_3)
{
    std::string input_file(MATRIX_FULL_PATH(circuit_3 ));
    std::string output_file(RESULT_FULL_PATH(circuit_3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, circuit_4)
{
    std::string input_file(MATRIX_FULL_PATH(circuit_4 ));
    std::string output_file(RESULT_FULL_PATH(circuit_4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, citationCiteseer)
{
    std::string input_file(MATRIX_FULL_PATH(citationCiteseer ));
    std::string output_file(RESULT_FULL_PATH(citationCiteseer ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cit_HepPh)
{
    std::string input_file(MATRIX_FULL_PATH(cit-HepPh ));
    std::string output_file(RESULT_FULL_PATH(cit-HepPh ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cit_HepTh)
{
    std::string input_file(MATRIX_FULL_PATH(cit-HepTh ));
    std::string output_file(RESULT_FULL_PATH(cit-HepTh ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ck400)
{
    std::string input_file(MATRIX_FULL_PATH(ck400 ));
    std::string output_file(RESULT_FULL_PATH(ck400 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ck656)
{
    std::string input_file(MATRIX_FULL_PATH(ck656 ));
    std::string output_file(RESULT_FULL_PATH(ck656 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ckt11752_dc_1)
{
    std::string input_file(MATRIX_FULL_PATH(ckt11752_dc_1 ));
    std::string output_file(RESULT_FULL_PATH(ckt11752_dc_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ckt11752_tr_0)
{
    std::string input_file(MATRIX_FULL_PATH(ckt11752_tr_0 ));
    std::string output_file(RESULT_FULL_PATH(ckt11752_tr_0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cnr_2000)
{
    std::string input_file(MATRIX_FULL_PATH(cnr-2000 ));
    std::string output_file(RESULT_FULL_PATH(cnr-2000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, coater1)
{
    std::string input_file(MATRIX_FULL_PATH(coater1 ));
    std::string output_file(RESULT_FULL_PATH(coater1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, coater2)
{
    std::string input_file(MATRIX_FULL_PATH(coater2 ));
    std::string output_file(RESULT_FULL_PATH(coater2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, coAuthorsCiteseer)
{
    std::string input_file(MATRIX_FULL_PATH(coAuthorsCiteseer ));
    std::string output_file(RESULT_FULL_PATH(coAuthorsCiteseer ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, coAuthorsDBLP)
{
    std::string input_file(MATRIX_FULL_PATH(coAuthorsDBLP ));
    std::string output_file(RESULT_FULL_PATH(coAuthorsDBLP ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, commanche_dual)
{
    std::string input_file(MATRIX_FULL_PATH(commanche_dual ));
    std::string output_file(RESULT_FULL_PATH(commanche_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, comsol)
{
    std::string input_file(MATRIX_FULL_PATH(comsol ));
    std::string output_file(RESULT_FULL_PATH(comsol ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cond_mat_2003)
{
    std::string input_file(MATRIX_FULL_PATH(cond-mat-2003 ));
    std::string output_file(RESULT_FULL_PATH(cond-mat-2003 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cond_mat_2005)
{
    std::string input_file(MATRIX_FULL_PATH(cond-mat-2005 ));
    std::string output_file(RESULT_FULL_PATH(cond-mat-2005 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cond_mat)
{
    std::string input_file(MATRIX_FULL_PATH(cond-mat ));
    std::string output_file(RESULT_FULL_PATH(cond-mat ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf5_0_4x4_10)
{
    std::string input_file(MATRIX_FULL_PATH(conf5_0-4x4-10 ));
    std::string output_file(RESULT_FULL_PATH(conf5_0-4x4-10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf5_0_4x4_14)
{
    std::string input_file(MATRIX_FULL_PATH(conf5_0-4x4-14 ));
    std::string output_file(RESULT_FULL_PATH(conf5_0-4x4-14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf5_0_4x4_18)
{
    std::string input_file(MATRIX_FULL_PATH(conf5_0-4x4-18 ));
    std::string output_file(RESULT_FULL_PATH(conf5_0-4x4-18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf5_0_4x4_22)
{
    std::string input_file(MATRIX_FULL_PATH(conf5_0-4x4-22 ));
    std::string output_file(RESULT_FULL_PATH(conf5_0-4x4-22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf5_0_4x4_26)
{
    std::string input_file(MATRIX_FULL_PATH(conf5_0-4x4-26 ));
    std::string output_file(RESULT_FULL_PATH(conf5_0-4x4-26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf5_4_8x8_05)
{
    std::string input_file(MATRIX_FULL_PATH(conf5_4-8x8-05 ));
    std::string output_file(RESULT_FULL_PATH(conf5_4-8x8-05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf5_4_8x8_10)
{
    std::string input_file(MATRIX_FULL_PATH(conf5_4-8x8-10 ));
    std::string output_file(RESULT_FULL_PATH(conf5_4-8x8-10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf5_4_8x8_15)
{
    std::string input_file(MATRIX_FULL_PATH(conf5_4-8x8-15 ));
    std::string output_file(RESULT_FULL_PATH(conf5_4-8x8-15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf5_4_8x8_20)
{
    std::string input_file(MATRIX_FULL_PATH(conf5_4-8x8-20 ));
    std::string output_file(RESULT_FULL_PATH(conf5_4-8x8-20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf6_0_4x4_20)
{
    std::string input_file(MATRIX_FULL_PATH(conf6_0-4x4-20 ));
    std::string output_file(RESULT_FULL_PATH(conf6_0-4x4-20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf6_0_4x4_30)
{
    std::string input_file(MATRIX_FULL_PATH(conf6_0-4x4-30 ));
    std::string output_file(RESULT_FULL_PATH(conf6_0-4x4-30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf6_0_8x8_20)
{
    std::string input_file(MATRIX_FULL_PATH(conf6_0-8x8-20 ));
    std::string output_file(RESULT_FULL_PATH(conf6_0-8x8-20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf6_0_8x8_30)
{
    std::string input_file(MATRIX_FULL_PATH(conf6_0-8x8-30 ));
    std::string output_file(RESULT_FULL_PATH(conf6_0-8x8-30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, conf6_0_8x8_80)
{
    std::string input_file(MATRIX_FULL_PATH(conf6_0-8x8-80 ));
    std::string output_file(RESULT_FULL_PATH(conf6_0-8x8-80 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, consph)
{
    std::string input_file(MATRIX_FULL_PATH(consph ));
    std::string output_file(RESULT_FULL_PATH(consph ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cont_201)
{
    std::string input_file(MATRIX_FULL_PATH(cont-201 ));
    std::string output_file(RESULT_FULL_PATH(cont-201 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cont_300)
{
    std::string input_file(MATRIX_FULL_PATH(cont-300 ));
    std::string output_file(RESULT_FULL_PATH(cont-300 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cop20k_A)
{
    std::string input_file(MATRIX_FULL_PATH(cop20k_A ));
    std::string output_file(RESULT_FULL_PATH(cop20k_A ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, coPapersCiteseer)
{
    std::string input_file(MATRIX_FULL_PATH(coPapersCiteseer ));
    std::string output_file(RESULT_FULL_PATH(coPapersCiteseer ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, copter1)
{
    std::string input_file(MATRIX_FULL_PATH(copter1 ));
    std::string output_file(RESULT_FULL_PATH(copter1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, copter2)
{
    std::string input_file(MATRIX_FULL_PATH(copter2 ));
    std::string output_file(RESULT_FULL_PATH(copter2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, CO)
{
    std::string input_file(MATRIX_FULL_PATH(CO ));
    std::string output_file(RESULT_FULL_PATH(CO ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, coupled)
{
    std::string input_file(MATRIX_FULL_PATH(coupled ));
    std::string output_file(RESULT_FULL_PATH(coupled ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crack_dual)
{
    std::string input_file(MATRIX_FULL_PATH(crack_dual ));
    std::string output_file(RESULT_FULL_PATH(crack_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crack)
{
    std::string input_file(MATRIX_FULL_PATH(crack ));
    std::string output_file(RESULT_FULL_PATH(crack ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crankseg_1)
{
    std::string input_file(MATRIX_FULL_PATH(crankseg_1 ));
    std::string output_file(RESULT_FULL_PATH(crankseg_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crankseg_2)
{
    std::string input_file(MATRIX_FULL_PATH(crankseg_2 ));
    std::string output_file(RESULT_FULL_PATH(crankseg_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crashbasis)
{
    std::string input_file(MATRIX_FULL_PATH(crashbasis ));
    std::string output_file(RESULT_FULL_PATH(crashbasis ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crplat2)
{
    std::string input_file(MATRIX_FULL_PATH(crplat2 ));
    std::string output_file(RESULT_FULL_PATH(crplat2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cryg10000)
{
    std::string input_file(MATRIX_FULL_PATH(cryg10000 ));
    std::string output_file(RESULT_FULL_PATH(cryg10000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cryg2500)
{
    std::string input_file(MATRIX_FULL_PATH(cryg2500 ));
    std::string output_file(RESULT_FULL_PATH(cryg2500 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crystk01)
{
    std::string input_file(MATRIX_FULL_PATH(crystk01 ));
    std::string output_file(RESULT_FULL_PATH(crystk01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crystk02)
{
    std::string input_file(MATRIX_FULL_PATH(crystk02 ));
    std::string output_file(RESULT_FULL_PATH(crystk02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crystk03)
{
    std::string input_file(MATRIX_FULL_PATH(crystk03 ));
    std::string output_file(RESULT_FULL_PATH(crystk03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crystm01)
{
    std::string input_file(MATRIX_FULL_PATH(crystm01 ));
    std::string output_file(RESULT_FULL_PATH(crystm01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crystm02)
{
    std::string input_file(MATRIX_FULL_PATH(crystm02 ));
    std::string output_file(RESULT_FULL_PATH(crystm02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, crystm03)
{
    std::string input_file(MATRIX_FULL_PATH(crystm03 ));
    std::string output_file(RESULT_FULL_PATH(crystm03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cs4)
{
    std::string input_file(MATRIX_FULL_PATH(cs4 ));
    std::string output_file(RESULT_FULL_PATH(cs4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, CSphd)
{
    std::string input_file(MATRIX_FULL_PATH(CSphd ));
    std::string output_file(RESULT_FULL_PATH(CSphd ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ct20stif)
{
    std::string input_file(MATRIX_FULL_PATH(ct20stif ));
    std::string output_file(RESULT_FULL_PATH(ct20stif ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cti)
{
    std::string input_file(MATRIX_FULL_PATH(cti ));
    std::string output_file(RESULT_FULL_PATH(cti ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cvxbqp1)
{
    std::string input_file(MATRIX_FULL_PATH(cvxbqp1 ));
    std::string output_file(RESULT_FULL_PATH(cvxbqp1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cvxqp3)
{
    std::string input_file(MATRIX_FULL_PATH(cvxqp3 ));
    std::string output_file(RESULT_FULL_PATH(cvxqp3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, cyl6)
{
    std::string input_file(MATRIX_FULL_PATH(cyl6 ));
    std::string output_file(RESULT_FULL_PATH(cyl6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, darcy003)
{
    std::string input_file(MATRIX_FULL_PATH(darcy003 ));
    std::string output_file(RESULT_FULL_PATH(darcy003 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, data)
{
    std::string input_file(MATRIX_FULL_PATH(data ));
    std::string output_file(RESULT_FULL_PATH(data ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dawson5)
{
    std::string input_file(MATRIX_FULL_PATH(dawson5 ));
    std::string output_file(RESULT_FULL_PATH(dawson5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dblp_2010)
{
    std::string input_file(MATRIX_FULL_PATH(dblp-2010 ));
    std::string output_file(RESULT_FULL_PATH(dblp-2010 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dc1)
{
    std::string input_file(MATRIX_FULL_PATH(dc1 ));
    std::string output_file(RESULT_FULL_PATH(dc1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dc2)
{
    std::string input_file(MATRIX_FULL_PATH(dc2 ));
    std::string output_file(RESULT_FULL_PATH(dc2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dc3)
{
    std::string input_file(MATRIX_FULL_PATH(dc3 ));
    std::string output_file(RESULT_FULL_PATH(dc3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n10)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n10 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n11)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n11 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n12)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n12 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n13)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n13 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n14)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n14 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n15)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n15 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n16)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n16 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n17)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n17 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n18)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n18 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, delaunay_n19)
{
    std::string input_file(MATRIX_FULL_PATH(delaunay_n19 ));
    std::string output_file(RESULT_FULL_PATH(delaunay_n19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, denormal)
{
    std::string input_file(MATRIX_FULL_PATH(denormal ));
    std::string output_file(RESULT_FULL_PATH(denormal ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, descriptor_xingo6u)
{
    std::string input_file(MATRIX_FULL_PATH(descriptor_xingo6u ));
    std::string output_file(RESULT_FULL_PATH(descriptor_xingo6u ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, diag)
{
    std::string input_file(MATRIX_FULL_PATH(diag ));
    std::string output_file(RESULT_FULL_PATH(diag ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dictionary28)
{
    std::string input_file(MATRIX_FULL_PATH(dictionary28 ));
    std::string output_file(RESULT_FULL_PATH(dictionary28 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dielFilterV3clx)
{
    std::string input_file(MATRIX_FULL_PATH(dielFilterV3clx ));
    std::string output_file(RESULT_FULL_PATH(dielFilterV3clx ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dixmaanl)
{
    std::string input_file(MATRIX_FULL_PATH(dixmaanl ));
    std::string output_file(RESULT_FULL_PATH(dixmaanl ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, DK01R)
{
    std::string input_file(MATRIX_FULL_PATH(DK01R ));
    std::string output_file(RESULT_FULL_PATH(DK01R ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, d_pretok)
{
    std::string input_file(MATRIX_FULL_PATH(d_pretok ));
    std::string output_file(RESULT_FULL_PATH(d_pretok ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dtoc)
{
    std::string input_file(MATRIX_FULL_PATH(dtoc ));
    std::string output_file(RESULT_FULL_PATH(dtoc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Dubcova1)
{
    std::string input_file(MATRIX_FULL_PATH(Dubcova1 ));
    std::string output_file(RESULT_FULL_PATH(Dubcova1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Dubcova2)
{
    std::string input_file(MATRIX_FULL_PATH(Dubcova2 ));
    std::string output_file(RESULT_FULL_PATH(Dubcova2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Dubcova3)
{
    std::string input_file(MATRIX_FULL_PATH(Dubcova3 ));
    std::string output_file(RESULT_FULL_PATH(Dubcova3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dw1024)
{
    std::string input_file(MATRIX_FULL_PATH(dw1024 ));
    std::string output_file(RESULT_FULL_PATH(dw1024 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dw2048)
{
    std::string input_file(MATRIX_FULL_PATH(dw2048 ));
    std::string output_file(RESULT_FULL_PATH(dw2048 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dw256A)
{
    std::string input_file(MATRIX_FULL_PATH(dw256A ));
    std::string output_file(RESULT_FULL_PATH(dw256A ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dw256B)
{
    std::string input_file(MATRIX_FULL_PATH(dw256B ));
    std::string output_file(RESULT_FULL_PATH(dw256B ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dw4096)
{
    std::string input_file(MATRIX_FULL_PATH(dw4096 ));
    std::string output_file(RESULT_FULL_PATH(dw4096 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dw8192)
{
    std::string input_file(MATRIX_FULL_PATH(dw8192 ));
    std::string output_file(RESULT_FULL_PATH(dw8192 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwa512)
{
    std::string input_file(MATRIX_FULL_PATH(dwa512 ));
    std::string output_file(RESULT_FULL_PATH(dwa512 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwb512)
{
    std::string input_file(MATRIX_FULL_PATH(dwb512 ));
    std::string output_file(RESULT_FULL_PATH(dwb512 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwg961a)
{
    std::string input_file(MATRIX_FULL_PATH(dwg961a ));
    std::string output_file(RESULT_FULL_PATH(dwg961a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwg961b)
{
    std::string input_file(MATRIX_FULL_PATH(dwg961b ));
    std::string output_file(RESULT_FULL_PATH(dwg961b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_1005)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_1005 ));
    std::string output_file(RESULT_FULL_PATH(dwt_1005 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_1007)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_1007 ));
    std::string output_file(RESULT_FULL_PATH(dwt_1007 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_1242)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_1242 ));
    std::string output_file(RESULT_FULL_PATH(dwt_1242 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_162)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_162 ));
    std::string output_file(RESULT_FULL_PATH(dwt_162 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_193)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_193 ));
    std::string output_file(RESULT_FULL_PATH(dwt_193 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_198)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_198 ));
    std::string output_file(RESULT_FULL_PATH(dwt_198 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_209)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_209 ));
    std::string output_file(RESULT_FULL_PATH(dwt_209 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_221)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_221 ));
    std::string output_file(RESULT_FULL_PATH(dwt_221 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_245)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_245 ));
    std::string output_file(RESULT_FULL_PATH(dwt_245 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_2680)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_2680 ));
    std::string output_file(RESULT_FULL_PATH(dwt_2680 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_307)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_307 ));
    std::string output_file(RESULT_FULL_PATH(dwt_307 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_310)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_310 ));
    std::string output_file(RESULT_FULL_PATH(dwt_310 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_346)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_346 ));
    std::string output_file(RESULT_FULL_PATH(dwt_346 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_361)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_361 ));
    std::string output_file(RESULT_FULL_PATH(dwt_361 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_419)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_419 ));
    std::string output_file(RESULT_FULL_PATH(dwt_419 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_492)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_492 ));
    std::string output_file(RESULT_FULL_PATH(dwt_492 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_503)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_503 ));
    std::string output_file(RESULT_FULL_PATH(dwt_503 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_512)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_512 ));
    std::string output_file(RESULT_FULL_PATH(dwt_512 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_592)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_592 ));
    std::string output_file(RESULT_FULL_PATH(dwt_592 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_607)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_607 ));
    std::string output_file(RESULT_FULL_PATH(dwt_607 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_758)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_758 ));
    std::string output_file(RESULT_FULL_PATH(dwt_758 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_869)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_869 ));
    std::string output_file(RESULT_FULL_PATH(dwt_869 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_878)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_878 ));
    std::string output_file(RESULT_FULL_PATH(dwt_878 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_918)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_918 ));
    std::string output_file(RESULT_FULL_PATH(dwt_918 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, dwt_992)
{
    std::string input_file(MATRIX_FULL_PATH(dwt_992 ));
    std::string output_file(RESULT_FULL_PATH(dwt_992 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, e40r0100)
{
    std::string input_file(MATRIX_FULL_PATH(e40r0100 ));
    std::string output_file(RESULT_FULL_PATH(e40r0100 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, EAT_RS)
{
    std::string input_file(MATRIX_FULL_PATH(EAT_RS ));
    std::string output_file(RESULT_FULL_PATH(EAT_RS ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, EAT_SR)
{
    std::string input_file(MATRIX_FULL_PATH(EAT_SR ));
    std::string output_file(RESULT_FULL_PATH(EAT_SR ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ecl32)
{
    std::string input_file(MATRIX_FULL_PATH(ecl32 ));
    std::string output_file(RESULT_FULL_PATH(ecl32 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, email_Enron)
{
    std::string input_file(MATRIX_FULL_PATH(email-Enron ));
    std::string output_file(RESULT_FULL_PATH(email-Enron ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, email_EuAll)
{
    std::string input_file(MATRIX_FULL_PATH(email-EuAll ));
    std::string output_file(RESULT_FULL_PATH(email-EuAll ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, email)
{
    std::string input_file(MATRIX_FULL_PATH(email ));
    std::string output_file(RESULT_FULL_PATH(email ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, engine)
{
    std::string input_file(MATRIX_FULL_PATH(engine ));
    std::string output_file(RESULT_FULL_PATH(engine ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, enron)
{
    std::string input_file(MATRIX_FULL_PATH(enron ));
    std::string output_file(RESULT_FULL_PATH(enron ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, EPA)
{
    std::string input_file(MATRIX_FULL_PATH(EPA ));
    std::string output_file(RESULT_FULL_PATH(EPA ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, epb0)
{
    std::string input_file(MATRIX_FULL_PATH(epb0 ));
    std::string output_file(RESULT_FULL_PATH(epb0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, epb1)
{
    std::string input_file(MATRIX_FULL_PATH(epb1 ));
    std::string output_file(RESULT_FULL_PATH(epb1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, epb2)
{
    std::string input_file(MATRIX_FULL_PATH(epb2 ));
    std::string output_file(RESULT_FULL_PATH(epb2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, epb3)
{
    std::string input_file(MATRIX_FULL_PATH(epb3 ));
    std::string output_file(RESULT_FULL_PATH(epb3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Erdos02)
{
    std::string input_file(MATRIX_FULL_PATH(Erdos02 ));
    std::string output_file(RESULT_FULL_PATH(Erdos02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Erdos971)
{
    std::string input_file(MATRIX_FULL_PATH(Erdos971 ));
    std::string output_file(RESULT_FULL_PATH(Erdos971 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Erdos972)
{
    std::string input_file(MATRIX_FULL_PATH(Erdos972 ));
    std::string output_file(RESULT_FULL_PATH(Erdos972 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Erdos981)
{
    std::string input_file(MATRIX_FULL_PATH(Erdos981 ));
    std::string output_file(RESULT_FULL_PATH(Erdos981 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Erdos982)
{
    std::string input_file(MATRIX_FULL_PATH(Erdos982 ));
    std::string output_file(RESULT_FULL_PATH(Erdos982 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Erdos991)
{
    std::string input_file(MATRIX_FULL_PATH(Erdos991 ));
    std::string output_file(RESULT_FULL_PATH(Erdos991 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Erdos992)
{
    std::string input_file(MATRIX_FULL_PATH(Erdos992 ));
    std::string output_file(RESULT_FULL_PATH(Erdos992 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, eris1176)
{
    std::string input_file(MATRIX_FULL_PATH(eris1176 ));
    std::string output_file(RESULT_FULL_PATH(eris1176 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, eurqsa)
{
    std::string input_file(MATRIX_FULL_PATH(eurqsa ));
    std::string output_file(RESULT_FULL_PATH(eurqsa ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, EVA)
{
    std::string input_file(MATRIX_FULL_PATH(EVA ));
    std::string output_file(RESULT_FULL_PATH(EVA ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex10hs)
{
    std::string input_file(MATRIX_FULL_PATH(ex10hs ));
    std::string output_file(RESULT_FULL_PATH(ex10hs ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex10)
{
    std::string input_file(MATRIX_FULL_PATH(ex10 ));
    std::string output_file(RESULT_FULL_PATH(ex10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex11)
{
    std::string input_file(MATRIX_FULL_PATH(ex11 ));
    std::string output_file(RESULT_FULL_PATH(ex11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex12)
{
    std::string input_file(MATRIX_FULL_PATH(ex12 ));
    std::string output_file(RESULT_FULL_PATH(ex12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex13)
{
    std::string input_file(MATRIX_FULL_PATH(ex13 ));
    std::string output_file(RESULT_FULL_PATH(ex13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex14)
{
    std::string input_file(MATRIX_FULL_PATH(ex14 ));
    std::string output_file(RESULT_FULL_PATH(ex14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex15)
{
    std::string input_file(MATRIX_FULL_PATH(ex15 ));
    std::string output_file(RESULT_FULL_PATH(ex15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex18)
{
    std::string input_file(MATRIX_FULL_PATH(ex18 ));
    std::string output_file(RESULT_FULL_PATH(ex18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex19)
{
    std::string input_file(MATRIX_FULL_PATH(ex19 ));
    std::string output_file(RESULT_FULL_PATH(ex19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex1)
{
    std::string input_file(MATRIX_FULL_PATH(ex1 ));
    std::string output_file(RESULT_FULL_PATH(ex1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex20)
{
    std::string input_file(MATRIX_FULL_PATH(ex20 ));
    std::string output_file(RESULT_FULL_PATH(ex20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex21)
{
    std::string input_file(MATRIX_FULL_PATH(ex21 ));
    std::string output_file(RESULT_FULL_PATH(ex21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex22)
{
    std::string input_file(MATRIX_FULL_PATH(ex22 ));
    std::string output_file(RESULT_FULL_PATH(ex22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex23)
{
    std::string input_file(MATRIX_FULL_PATH(ex23 ));
    std::string output_file(RESULT_FULL_PATH(ex23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex24)
{
    std::string input_file(MATRIX_FULL_PATH(ex24 ));
    std::string output_file(RESULT_FULL_PATH(ex24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex25)
{
    std::string input_file(MATRIX_FULL_PATH(ex25 ));
    std::string output_file(RESULT_FULL_PATH(ex25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex26)
{
    std::string input_file(MATRIX_FULL_PATH(ex26 ));
    std::string output_file(RESULT_FULL_PATH(ex26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex27)
{
    std::string input_file(MATRIX_FULL_PATH(ex27 ));
    std::string output_file(RESULT_FULL_PATH(ex27 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex28)
{
    std::string input_file(MATRIX_FULL_PATH(ex28 ));
    std::string output_file(RESULT_FULL_PATH(ex28 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex29)
{
    std::string input_file(MATRIX_FULL_PATH(ex29 ));
    std::string output_file(RESULT_FULL_PATH(ex29 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex2)
{
    std::string input_file(MATRIX_FULL_PATH(ex2 ));
    std::string output_file(RESULT_FULL_PATH(ex2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex31)
{
    std::string input_file(MATRIX_FULL_PATH(ex31 ));
    std::string output_file(RESULT_FULL_PATH(ex31 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex32)
{
    std::string input_file(MATRIX_FULL_PATH(ex32 ));
    std::string output_file(RESULT_FULL_PATH(ex32 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex33)
{
    std::string input_file(MATRIX_FULL_PATH(ex33 ));
    std::string output_file(RESULT_FULL_PATH(ex33 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex35)
{
    std::string input_file(MATRIX_FULL_PATH(ex35 ));
    std::string output_file(RESULT_FULL_PATH(ex35 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex36)
{
    std::string input_file(MATRIX_FULL_PATH(ex36 ));
    std::string output_file(RESULT_FULL_PATH(ex36 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex37)
{
    std::string input_file(MATRIX_FULL_PATH(ex37 ));
    std::string output_file(RESULT_FULL_PATH(ex37 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex3sta1)
{
    std::string input_file(MATRIX_FULL_PATH(ex3sta1 ));
    std::string output_file(RESULT_FULL_PATH(ex3sta1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex3)
{
    std::string input_file(MATRIX_FULL_PATH(ex3 ));
    std::string output_file(RESULT_FULL_PATH(ex3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex40)
{
    std::string input_file(MATRIX_FULL_PATH(ex40 ));
    std::string output_file(RESULT_FULL_PATH(ex40 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex4)
{
    std::string input_file(MATRIX_FULL_PATH(ex4 ));
    std::string output_file(RESULT_FULL_PATH(ex4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, EX5)
{
    std::string input_file(MATRIX_FULL_PATH(EX5 ));
    std::string output_file(RESULT_FULL_PATH(EX5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex6)
{
    std::string input_file(MATRIX_FULL_PATH(ex6 ));
    std::string output_file(RESULT_FULL_PATH(ex6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex7)
{
    std::string input_file(MATRIX_FULL_PATH(ex7 ));
    std::string output_file(RESULT_FULL_PATH(ex7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex8)
{
    std::string input_file(MATRIX_FULL_PATH(ex8 ));
    std::string output_file(RESULT_FULL_PATH(ex8 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ex9)
{
    std::string input_file(MATRIX_FULL_PATH(ex9 ));
    std::string output_file(RESULT_FULL_PATH(ex9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, exdata_1)
{
    std::string input_file(MATRIX_FULL_PATH(exdata_1 ));
    std::string output_file(RESULT_FULL_PATH(exdata_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, extr1b)
{
    std::string input_file(MATRIX_FULL_PATH(extr1b ));
    std::string output_file(RESULT_FULL_PATH(extr1b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, extr1)
{
    std::string input_file(MATRIX_FULL_PATH(extr1 ));
    std::string output_file(RESULT_FULL_PATH(extr1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, F1)
{
    std::string input_file(MATRIX_FULL_PATH(F1 ));
    std::string output_file(RESULT_FULL_PATH(F1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, F2)
{
    std::string input_file(MATRIX_FULL_PATH(F2 ));
    std::string output_file(RESULT_FULL_PATH(F2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, FA)
{
    std::string input_file(MATRIX_FULL_PATH(FA ));
    std::string output_file(RESULT_FULL_PATH(FA ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fcondp2)
{
    std::string input_file(MATRIX_FULL_PATH(fcondp2 ));
    std::string output_file(RESULT_FULL_PATH(fcondp2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fd12)
{
    std::string input_file(MATRIX_FULL_PATH(fd12 ));
    std::string output_file(RESULT_FULL_PATH(fd12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fd15)
{
    std::string input_file(MATRIX_FULL_PATH(fd15 ));
    std::string output_file(RESULT_FULL_PATH(fd15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fd18)
{
    std::string input_file(MATRIX_FULL_PATH(fd18 ));
    std::string output_file(RESULT_FULL_PATH(fd18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fe_4elt2)
{
    std::string input_file(MATRIX_FULL_PATH(fe_4elt2 ));
    std::string output_file(RESULT_FULL_PATH(fe_4elt2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fe_body)
{
    std::string input_file(MATRIX_FULL_PATH(fe_body ));
    std::string output_file(RESULT_FULL_PATH(fe_body ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, FEM_3D_thermal1)
{
    std::string input_file(MATRIX_FULL_PATH(FEM_3D_thermal1 ));
    std::string output_file(RESULT_FULL_PATH(FEM_3D_thermal1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, FEM_3D_thermal2)
{
    std::string input_file(MATRIX_FULL_PATH(FEM_3D_thermal2 ));
    std::string output_file(RESULT_FULL_PATH(FEM_3D_thermal2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fem_filter)
{
    std::string input_file(MATRIX_FULL_PATH(fem_filter ));
    std::string output_file(RESULT_FULL_PATH(fem_filter ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fem_hifreq_circuit)
{
    std::string input_file(MATRIX_FULL_PATH(fem_hifreq_circuit ));
    std::string output_file(RESULT_FULL_PATH(fem_hifreq_circuit ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fe_ocean)
{
    std::string input_file(MATRIX_FULL_PATH(fe_ocean ));
    std::string output_file(RESULT_FULL_PATH(fe_ocean ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fe_rotor)
{
    std::string input_file(MATRIX_FULL_PATH(fe_rotor ));
    std::string output_file(RESULT_FULL_PATH(fe_rotor ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fe_sphere)
{
    std::string input_file(MATRIX_FULL_PATH(fe_sphere ));
    std::string output_file(RESULT_FULL_PATH(fe_sphere ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fe_tooth)
{
    std::string input_file(MATRIX_FULL_PATH(fe_tooth ));
    std::string output_file(RESULT_FULL_PATH(fe_tooth ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, filter2D)
{
    std::string input_file(MATRIX_FULL_PATH(filter2D ));
    std::string output_file(RESULT_FULL_PATH(filter2D ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, filter3D)
{
    std::string input_file(MATRIX_FULL_PATH(filter3D ));
    std::string output_file(RESULT_FULL_PATH(filter3D ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, finan512)
{
    std::string input_file(MATRIX_FULL_PATH(finan512 ));
    std::string output_file(RESULT_FULL_PATH(finan512 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, finance256)
{
    std::string input_file(MATRIX_FULL_PATH(finance256 ));
    std::string output_file(RESULT_FULL_PATH(finance256 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, flowmeter0)
{
    std::string input_file(MATRIX_FULL_PATH(flowmeter0 ));
    std::string output_file(RESULT_FULL_PATH(flowmeter0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, flowmeter5)
{
    std::string input_file(MATRIX_FULL_PATH(flowmeter5 ));
    std::string output_file(RESULT_FULL_PATH(flowmeter5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, foldoc)
{
    std::string input_file(MATRIX_FULL_PATH(foldoc ));
    std::string output_file(RESULT_FULL_PATH(foldoc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ford1)
{
    std::string input_file(MATRIX_FULL_PATH(ford1 ));
    std::string output_file(RESULT_FULL_PATH(ford1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ford2)
{
    std::string input_file(MATRIX_FULL_PATH(ford2 ));
    std::string output_file(RESULT_FULL_PATH(ford2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_01)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_01 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_02)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_02 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_03)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_03 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_04)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_04 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_05)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_05 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_06)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_06 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_07)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_07 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_08)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_08 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_09)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_09 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_10)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_10 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_11)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_11 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_12)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_12 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_13)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_13 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_14)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_14 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_15)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_15 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_16)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_16 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_17)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_17 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_18)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_18 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_19)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_19 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_20)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_20 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_21)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_21 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_22)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_22 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_23)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_23 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_24)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_24 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_25)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_25 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_26)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_26 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_27)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_27 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_27 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_28)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_28 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_28 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_29)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_29 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_29 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_30)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_30 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_31)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_31 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_31 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_32)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_32 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_32 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_33)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_33 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_33 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_34)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_34 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_34 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_35)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_35 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_35 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_36)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_36 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_36 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_37)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_37 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_37 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_38)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_38 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_38 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_39)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_39 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_39 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_40)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_40 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_40 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_41)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_41 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_41 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_42)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_42 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_42 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_43)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_43 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_43 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_44)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_44 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_44 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_45)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_45 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_45 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_46)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_46 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_46 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_47)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_47 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_47 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_48)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_48 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_48 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_49)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_49 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_49 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_50)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_50 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_50 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_dcop_51)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_dcop_51 ));
    std::string output_file(RESULT_FULL_PATH(fpga_dcop_51 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_trans_01)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_trans_01 ));
    std::string output_file(RESULT_FULL_PATH(fpga_trans_01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fpga_trans_02)
{
    std::string input_file(MATRIX_FULL_PATH(fpga_trans_02 ));
    std::string output_file(RESULT_FULL_PATH(fpga_trans_02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fp)
{
    std::string input_file(MATRIX_FULL_PATH(fp ));
    std::string output_file(RESULT_FULL_PATH(fp ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_183_3)
{
    std::string input_file(MATRIX_FULL_PATH(fs_183_3 ));
    std::string output_file(RESULT_FULL_PATH(fs_183_3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_183_4)
{
    std::string input_file(MATRIX_FULL_PATH(fs_183_4 ));
    std::string output_file(RESULT_FULL_PATH(fs_183_4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_541_1)
{
    std::string input_file(MATRIX_FULL_PATH(fs_541_1 ));
    std::string output_file(RESULT_FULL_PATH(fs_541_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_541_2)
{
    std::string input_file(MATRIX_FULL_PATH(fs_541_2 ));
    std::string output_file(RESULT_FULL_PATH(fs_541_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_541_3)
{
    std::string input_file(MATRIX_FULL_PATH(fs_541_3 ));
    std::string output_file(RESULT_FULL_PATH(fs_541_3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_541_4)
{
    std::string input_file(MATRIX_FULL_PATH(fs_541_4 ));
    std::string output_file(RESULT_FULL_PATH(fs_541_4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_680_1)
{
    std::string input_file(MATRIX_FULL_PATH(fs_680_1 ));
    std::string output_file(RESULT_FULL_PATH(fs_680_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_680_2)
{
    std::string input_file(MATRIX_FULL_PATH(fs_680_2 ));
    std::string output_file(RESULT_FULL_PATH(fs_680_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_680_3)
{
    std::string input_file(MATRIX_FULL_PATH(fs_680_3 ));
    std::string output_file(RESULT_FULL_PATH(fs_680_3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_760_1)
{
    std::string input_file(MATRIX_FULL_PATH(fs_760_1 ));
    std::string output_file(RESULT_FULL_PATH(fs_760_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_760_2)
{
    std::string input_file(MATRIX_FULL_PATH(fs_760_2 ));
    std::string output_file(RESULT_FULL_PATH(fs_760_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fs_760_3)
{
    std::string input_file(MATRIX_FULL_PATH(fs_760_3 ));
    std::string output_file(RESULT_FULL_PATH(fs_760_3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fullb)
{
    std::string input_file(MATRIX_FULL_PATH(fullb ));
    std::string output_file(RESULT_FULL_PATH(fullb ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fv1)
{
    std::string input_file(MATRIX_FULL_PATH(fv1 ));
    std::string output_file(RESULT_FULL_PATH(fv1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fv2)
{
    std::string input_file(MATRIX_FULL_PATH(fv2 ));
    std::string output_file(RESULT_FULL_PATH(fv2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fv3)
{
    std::string input_file(MATRIX_FULL_PATH(fv3 ));
    std::string output_file(RESULT_FULL_PATH(fv3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fxm3_6)
{
    std::string input_file(MATRIX_FULL_PATH(fxm3_6 ));
    std::string output_file(RESULT_FULL_PATH(fxm3_6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, fxm4_6)
{
    std::string input_file(MATRIX_FULL_PATH(fxm4_6 ));
    std::string output_file(RESULT_FULL_PATH(fxm4_6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G10)
{
    std::string input_file(MATRIX_FULL_PATH(G10 ));
    std::string output_file(RESULT_FULL_PATH(G10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G11)
{
    std::string input_file(MATRIX_FULL_PATH(G11 ));
    std::string output_file(RESULT_FULL_PATH(G11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G12)
{
    std::string input_file(MATRIX_FULL_PATH(G12 ));
    std::string output_file(RESULT_FULL_PATH(G12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G13)
{
    std::string input_file(MATRIX_FULL_PATH(G13 ));
    std::string output_file(RESULT_FULL_PATH(G13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G14)
{
    std::string input_file(MATRIX_FULL_PATH(G14 ));
    std::string output_file(RESULT_FULL_PATH(G14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G15)
{
    std::string input_file(MATRIX_FULL_PATH(G15 ));
    std::string output_file(RESULT_FULL_PATH(G15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G16)
{
    std::string input_file(MATRIX_FULL_PATH(G16 ));
    std::string output_file(RESULT_FULL_PATH(G16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G17)
{
    std::string input_file(MATRIX_FULL_PATH(G17 ));
    std::string output_file(RESULT_FULL_PATH(G17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G18)
{
    std::string input_file(MATRIX_FULL_PATH(G18 ));
    std::string output_file(RESULT_FULL_PATH(G18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G19)
{
    std::string input_file(MATRIX_FULL_PATH(G19 ));
    std::string output_file(RESULT_FULL_PATH(G19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G1)
{
    std::string input_file(MATRIX_FULL_PATH(G1 ));
    std::string output_file(RESULT_FULL_PATH(G1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G20)
{
    std::string input_file(MATRIX_FULL_PATH(G20 ));
    std::string output_file(RESULT_FULL_PATH(G20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G21)
{
    std::string input_file(MATRIX_FULL_PATH(G21 ));
    std::string output_file(RESULT_FULL_PATH(G21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G22)
{
    std::string input_file(MATRIX_FULL_PATH(G22 ));
    std::string output_file(RESULT_FULL_PATH(G22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G23)
{
    std::string input_file(MATRIX_FULL_PATH(G23 ));
    std::string output_file(RESULT_FULL_PATH(G23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G24)
{
    std::string input_file(MATRIX_FULL_PATH(G24 ));
    std::string output_file(RESULT_FULL_PATH(G24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G25)
{
    std::string input_file(MATRIX_FULL_PATH(G25 ));
    std::string output_file(RESULT_FULL_PATH(G25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G26)
{
    std::string input_file(MATRIX_FULL_PATH(G26 ));
    std::string output_file(RESULT_FULL_PATH(G26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G27)
{
    std::string input_file(MATRIX_FULL_PATH(G27 ));
    std::string output_file(RESULT_FULL_PATH(G27 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G28)
{
    std::string input_file(MATRIX_FULL_PATH(G28 ));
    std::string output_file(RESULT_FULL_PATH(G28 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G29)
{
    std::string input_file(MATRIX_FULL_PATH(G29 ));
    std::string output_file(RESULT_FULL_PATH(G29 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G2_circuit)
{
    std::string input_file(MATRIX_FULL_PATH(G2_circuit ));
    std::string output_file(RESULT_FULL_PATH(G2_circuit ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G2)
{
    std::string input_file(MATRIX_FULL_PATH(G2 ));
    std::string output_file(RESULT_FULL_PATH(G2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G30)
{
    std::string input_file(MATRIX_FULL_PATH(G30 ));
    std::string output_file(RESULT_FULL_PATH(G30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G31)
{
    std::string input_file(MATRIX_FULL_PATH(G31 ));
    std::string output_file(RESULT_FULL_PATH(G31 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G32)
{
    std::string input_file(MATRIX_FULL_PATH(G32 ));
    std::string output_file(RESULT_FULL_PATH(G32 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G33)
{
    std::string input_file(MATRIX_FULL_PATH(G33 ));
    std::string output_file(RESULT_FULL_PATH(G33 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G34)
{
    std::string input_file(MATRIX_FULL_PATH(G34 ));
    std::string output_file(RESULT_FULL_PATH(G34 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G35)
{
    std::string input_file(MATRIX_FULL_PATH(G35 ));
    std::string output_file(RESULT_FULL_PATH(G35 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G36)
{
    std::string input_file(MATRIX_FULL_PATH(G36 ));
    std::string output_file(RESULT_FULL_PATH(G36 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G37)
{
    std::string input_file(MATRIX_FULL_PATH(G37 ));
    std::string output_file(RESULT_FULL_PATH(G37 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G38)
{
    std::string input_file(MATRIX_FULL_PATH(G38 ));
    std::string output_file(RESULT_FULL_PATH(G38 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G39)
{
    std::string input_file(MATRIX_FULL_PATH(G39 ));
    std::string output_file(RESULT_FULL_PATH(G39 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g3rmt3m3)
{
    std::string input_file(MATRIX_FULL_PATH(g3rmt3m3 ));
    std::string output_file(RESULT_FULL_PATH(g3rmt3m3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G3)
{
    std::string input_file(MATRIX_FULL_PATH(G3 ));
    std::string output_file(RESULT_FULL_PATH(G3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G40)
{
    std::string input_file(MATRIX_FULL_PATH(G40 ));
    std::string output_file(RESULT_FULL_PATH(G40 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G41)
{
    std::string input_file(MATRIX_FULL_PATH(G41 ));
    std::string output_file(RESULT_FULL_PATH(G41 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G42)
{
    std::string input_file(MATRIX_FULL_PATH(G42 ));
    std::string output_file(RESULT_FULL_PATH(G42 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G43)
{
    std::string input_file(MATRIX_FULL_PATH(G43 ));
    std::string output_file(RESULT_FULL_PATH(G43 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G44)
{
    std::string input_file(MATRIX_FULL_PATH(G44 ));
    std::string output_file(RESULT_FULL_PATH(G44 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G45)
{
    std::string input_file(MATRIX_FULL_PATH(G45 ));
    std::string output_file(RESULT_FULL_PATH(G45 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G46)
{
    std::string input_file(MATRIX_FULL_PATH(G46 ));
    std::string output_file(RESULT_FULL_PATH(G46 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G47)
{
    std::string input_file(MATRIX_FULL_PATH(G47 ));
    std::string output_file(RESULT_FULL_PATH(G47 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G48)
{
    std::string input_file(MATRIX_FULL_PATH(G48 ));
    std::string output_file(RESULT_FULL_PATH(G48 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G49)
{
    std::string input_file(MATRIX_FULL_PATH(G49 ));
    std::string output_file(RESULT_FULL_PATH(G49 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G4)
{
    std::string input_file(MATRIX_FULL_PATH(G4 ));
    std::string output_file(RESULT_FULL_PATH(G4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G50)
{
    std::string input_file(MATRIX_FULL_PATH(G50 ));
    std::string output_file(RESULT_FULL_PATH(G50 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G51)
{
    std::string input_file(MATRIX_FULL_PATH(G51 ));
    std::string output_file(RESULT_FULL_PATH(G51 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G52)
{
    std::string input_file(MATRIX_FULL_PATH(G52 ));
    std::string output_file(RESULT_FULL_PATH(G52 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G53)
{
    std::string input_file(MATRIX_FULL_PATH(G53 ));
    std::string output_file(RESULT_FULL_PATH(G53 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G54)
{
    std::string input_file(MATRIX_FULL_PATH(G54 ));
    std::string output_file(RESULT_FULL_PATH(G54 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G55)
{
    std::string input_file(MATRIX_FULL_PATH(G55 ));
    std::string output_file(RESULT_FULL_PATH(G55 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G56)
{
    std::string input_file(MATRIX_FULL_PATH(G56 ));
    std::string output_file(RESULT_FULL_PATH(G56 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G57)
{
    std::string input_file(MATRIX_FULL_PATH(G57 ));
    std::string output_file(RESULT_FULL_PATH(G57 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G58)
{
    std::string input_file(MATRIX_FULL_PATH(G58 ));
    std::string output_file(RESULT_FULL_PATH(G58 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G59)
{
    std::string input_file(MATRIX_FULL_PATH(G59 ));
    std::string output_file(RESULT_FULL_PATH(G59 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G5)
{
    std::string input_file(MATRIX_FULL_PATH(G5 ));
    std::string output_file(RESULT_FULL_PATH(G5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G60)
{
    std::string input_file(MATRIX_FULL_PATH(G60 ));
    std::string output_file(RESULT_FULL_PATH(G60 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G61)
{
    std::string input_file(MATRIX_FULL_PATH(G61 ));
    std::string output_file(RESULT_FULL_PATH(G61 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G62)
{
    std::string input_file(MATRIX_FULL_PATH(G62 ));
    std::string output_file(RESULT_FULL_PATH(G62 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G63)
{
    std::string input_file(MATRIX_FULL_PATH(G63 ));
    std::string output_file(RESULT_FULL_PATH(G63 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G64)
{
    std::string input_file(MATRIX_FULL_PATH(G64 ));
    std::string output_file(RESULT_FULL_PATH(G64 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G65)
{
    std::string input_file(MATRIX_FULL_PATH(G65 ));
    std::string output_file(RESULT_FULL_PATH(G65 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G66)
{
    std::string input_file(MATRIX_FULL_PATH(G66 ));
    std::string output_file(RESULT_FULL_PATH(G66 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G67)
{
    std::string input_file(MATRIX_FULL_PATH(G67 ));
    std::string output_file(RESULT_FULL_PATH(G67 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G6)
{
    std::string input_file(MATRIX_FULL_PATH(G6 ));
    std::string output_file(RESULT_FULL_PATH(G6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac010sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac010sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac010sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac010)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac010 ));
    std::string output_file(RESULT_FULL_PATH(g7jac010 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac020sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac020sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac020sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac020)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac020 ));
    std::string output_file(RESULT_FULL_PATH(g7jac020 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac040sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac040sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac040sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac040)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac040 ));
    std::string output_file(RESULT_FULL_PATH(g7jac040 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac050sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac050sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac050sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac060sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac060sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac060sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac060)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac060 ));
    std::string output_file(RESULT_FULL_PATH(g7jac060 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac080sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac080sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac080sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac080)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac080 ));
    std::string output_file(RESULT_FULL_PATH(g7jac080 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac100sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac100sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac100sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac100)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac100 ));
    std::string output_file(RESULT_FULL_PATH(g7jac100 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac120sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac120sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac120sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac120)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac120 ));
    std::string output_file(RESULT_FULL_PATH(g7jac120 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac140sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac140sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac140sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac140)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac140 ));
    std::string output_file(RESULT_FULL_PATH(g7jac140 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac160sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac160sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac160sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac160)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac160 ));
    std::string output_file(RESULT_FULL_PATH(g7jac160 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac180sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac180sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac180sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac180)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac180 ));
    std::string output_file(RESULT_FULL_PATH(g7jac180 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac200sc)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac200sc ));
    std::string output_file(RESULT_FULL_PATH(g7jac200sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, g7jac200)
{
    std::string input_file(MATRIX_FULL_PATH(g7jac200 ));
    std::string output_file(RESULT_FULL_PATH(g7jac200 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G7)
{
    std::string input_file(MATRIX_FULL_PATH(G7 ));
    std::string output_file(RESULT_FULL_PATH(G7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G8)
{
    std::string input_file(MATRIX_FULL_PATH(G8 ));
    std::string output_file(RESULT_FULL_PATH(G8 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, G9)
{
    std::string input_file(MATRIX_FULL_PATH(G9 ));
    std::string output_file(RESULT_FULL_PATH(G9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Ga10As10H30)
{
    std::string input_file(MATRIX_FULL_PATH(Ga10As10H30 ));
    std::string output_file(RESULT_FULL_PATH(Ga10As10H30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Ga19As19H42)
{
    std::string input_file(MATRIX_FULL_PATH(Ga19As19H42 ));
    std::string output_file(RESULT_FULL_PATH(Ga19As19H42 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Ga3As3H12)
{
    std::string input_file(MATRIX_FULL_PATH(Ga3As3H12 ));
    std::string output_file(RESULT_FULL_PATH(Ga3As3H12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Ga41As41H72)
{
    std::string input_file(MATRIX_FULL_PATH(Ga41As41H72 ));
    std::string output_file(RESULT_FULL_PATH(Ga41As41H72 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, GaAsH6)
{
    std::string input_file(MATRIX_FULL_PATH(GaAsH6 ));
    std::string output_file(RESULT_FULL_PATH(GaAsH6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, garon1)
{
    std::string input_file(MATRIX_FULL_PATH(garon1 ));
    std::string output_file(RESULT_FULL_PATH(garon1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, garon2)
{
    std::string input_file(MATRIX_FULL_PATH(garon2 ));
    std::string output_file(RESULT_FULL_PATH(garon2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gas_sensor)
{
    std::string input_file(MATRIX_FULL_PATH(gas_sensor ));
    std::string output_file(RESULT_FULL_PATH(gas_sensor ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, GD00_c)
{
    std::string input_file(MATRIX_FULL_PATH(GD00_c ));
    std::string output_file(RESULT_FULL_PATH(GD00_c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, GD06_Java)
{
    std::string input_file(MATRIX_FULL_PATH(GD06_Java ));
    std::string output_file(RESULT_FULL_PATH(GD06_Java ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, GD96_a)
{
    std::string input_file(MATRIX_FULL_PATH(GD96_a ));
    std::string output_file(RESULT_FULL_PATH(GD96_a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Ge87H76)
{
    std::string input_file(MATRIX_FULL_PATH(Ge87H76 ));
    std::string output_file(RESULT_FULL_PATH(Ge87H76 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Ge99H100)
{
    std::string input_file(MATRIX_FULL_PATH(Ge99H100 ));
    std::string output_file(RESULT_FULL_PATH(Ge99H100 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gearbox)
{
    std::string input_file(MATRIX_FULL_PATH(gearbox ));
    std::string output_file(RESULT_FULL_PATH(gearbox ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gemat11)
{
    std::string input_file(MATRIX_FULL_PATH(gemat11 ));
    std::string output_file(RESULT_FULL_PATH(gemat11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gemat12)
{
    std::string input_file(MATRIX_FULL_PATH(gemat12 ));
    std::string output_file(RESULT_FULL_PATH(gemat12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, geom)
{
    std::string input_file(MATRIX_FULL_PATH(geom ));
    std::string output_file(RESULT_FULL_PATH(geom ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, goodwin)
{
    std::string input_file(MATRIX_FULL_PATH(goodwin ));
    std::string output_file(RESULT_FULL_PATH(goodwin ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gr_30_30)
{
    std::string input_file(MATRIX_FULL_PATH(gr_30_30 ));
    std::string output_file(RESULT_FULL_PATH(gr_30_30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, graham1)
{
    std::string input_file(MATRIX_FULL_PATH(graham1 ));
    std::string output_file(RESULT_FULL_PATH(graham1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gre_1107)
{
    std::string input_file(MATRIX_FULL_PATH(gre_1107 ));
    std::string output_file(RESULT_FULL_PATH(gre_1107 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gre_343)
{
    std::string input_file(MATRIX_FULL_PATH(gre_343 ));
    std::string output_file(RESULT_FULL_PATH(gre_343 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gre_512)
{
    std::string input_file(MATRIX_FULL_PATH(gre_512 ));
    std::string output_file(RESULT_FULL_PATH(gre_512 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, grid2_dual)
{
    std::string input_file(MATRIX_FULL_PATH(grid2_dual ));
    std::string output_file(RESULT_FULL_PATH(grid2_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, grid2)
{
    std::string input_file(MATRIX_FULL_PATH(grid2 ));
    std::string output_file(RESULT_FULL_PATH(grid2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gridgena)
{
    std::string input_file(MATRIX_FULL_PATH(gridgena ));
    std::string output_file(RESULT_FULL_PATH(gridgena ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, GT01R)
{
    std::string input_file(MATRIX_FULL_PATH(GT01R ));
    std::string output_file(RESULT_FULL_PATH(GT01R ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gupta1)
{
    std::string input_file(MATRIX_FULL_PATH(gupta1 ));
    std::string output_file(RESULT_FULL_PATH(gupta1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gupta2)
{
    std::string input_file(MATRIX_FULL_PATH(gupta2 ));
    std::string output_file(RESULT_FULL_PATH(gupta2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gupta3)
{
    std::string input_file(MATRIX_FULL_PATH(gupta3 ));
    std::string output_file(RESULT_FULL_PATH(gupta3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gyro_k)
{
    std::string input_file(MATRIX_FULL_PATH(gyro_k ));
    std::string output_file(RESULT_FULL_PATH(gyro_k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gyro_m)
{
    std::string input_file(MATRIX_FULL_PATH(gyro_m ));
    std::string output_file(RESULT_FULL_PATH(gyro_m ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, gyro)
{
    std::string input_file(MATRIX_FULL_PATH(gyro ));
    std::string output_file(RESULT_FULL_PATH(gyro ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, H2O)
{
    std::string input_file(MATRIX_FULL_PATH(H2O ));
    std::string output_file(RESULT_FULL_PATH(H2O ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, halfb)
{
    std::string input_file(MATRIX_FULL_PATH(halfb ));
    std::string output_file(RESULT_FULL_PATH(halfb ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Hamrle2)
{
    std::string input_file(MATRIX_FULL_PATH(Hamrle2 ));
    std::string output_file(RESULT_FULL_PATH(Hamrle2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Harvard500)
{
    std::string input_file(MATRIX_FULL_PATH(Harvard500 ));
    std::string output_file(RESULT_FULL_PATH(Harvard500 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hcircuit)
{
    std::string input_file(MATRIX_FULL_PATH(hcircuit ));
    std::string output_file(RESULT_FULL_PATH(hcircuit ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, heart1)
{
    std::string input_file(MATRIX_FULL_PATH(heart1 ));
    std::string output_file(RESULT_FULL_PATH(heart1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, heart2)
{
    std::string input_file(MATRIX_FULL_PATH(heart2 ));
    std::string output_file(RESULT_FULL_PATH(heart2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, heart3)
{
    std::string input_file(MATRIX_FULL_PATH(heart3 ));
    std::string output_file(RESULT_FULL_PATH(heart3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, helm2d03)
{
    std::string input_file(MATRIX_FULL_PATH(helm2d03 ));
    std::string output_file(RESULT_FULL_PATH(helm2d03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, helm3d01)
{
    std::string input_file(MATRIX_FULL_PATH(helm3d01 ));
    std::string output_file(RESULT_FULL_PATH(helm3d01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, HEP_th_new)
{
    std::string input_file(MATRIX_FULL_PATH(HEP-th-new ));
    std::string output_file(RESULT_FULL_PATH(HEP-th-new ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hep_th)
{
    std::string input_file(MATRIX_FULL_PATH(hep-th ));
    std::string output_file(RESULT_FULL_PATH(hep-th ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hood)
{
    std::string input_file(MATRIX_FULL_PATH(hood ));
    std::string output_file(RESULT_FULL_PATH(hood ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hor_131)
{
    std::string input_file(MATRIX_FULL_PATH(hor_131 ));
    std::string output_file(RESULT_FULL_PATH(hor_131 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, HTC_336_4438)
{
    std::string input_file(MATRIX_FULL_PATH(HTC_336_4438 ));
    std::string output_file(RESULT_FULL_PATH(HTC_336_4438 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, HTC_336_9129)
{
    std::string input_file(MATRIX_FULL_PATH(HTC_336_9129 ));
    std::string output_file(RESULT_FULL_PATH(HTC_336_9129 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, human_gene1)
{
    std::string input_file(MATRIX_FULL_PATH(human_gene1 ));
    std::string output_file(RESULT_FULL_PATH(human_gene1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, human_gene2)
{
    std::string input_file(MATRIX_FULL_PATH(human_gene2 ));
    std::string output_file(RESULT_FULL_PATH(human_gene2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hvdc1)
{
    std::string input_file(MATRIX_FULL_PATH(hvdc1 ));
    std::string output_file(RESULT_FULL_PATH(hvdc1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hvdc2)
{
    std::string input_file(MATRIX_FULL_PATH(hvdc2 ));
    std::string output_file(RESULT_FULL_PATH(hvdc2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hydr1c)
{
    std::string input_file(MATRIX_FULL_PATH(hydr1c ));
    std::string output_file(RESULT_FULL_PATH(hydr1c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, hydr1)
{
    std::string input_file(MATRIX_FULL_PATH(hydr1 ));
    std::string output_file(RESULT_FULL_PATH(hydr1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ibm_matrix_2)
{
    std::string input_file(MATRIX_FULL_PATH(ibm_matrix_2 ));
    std::string output_file(RESULT_FULL_PATH(ibm_matrix_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, igbt3)
{
    std::string input_file(MATRIX_FULL_PATH(igbt3 ));
    std::string output_file(RESULT_FULL_PATH(igbt3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Ill_Stokes)
{
    std::string input_file(MATRIX_FULL_PATH(Ill_Stokes ));
    std::string output_file(RESULT_FULL_PATH(Ill_Stokes ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, impcol_c)
{
    std::string input_file(MATRIX_FULL_PATH(impcol_c ));
    std::string output_file(RESULT_FULL_PATH(impcol_c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, impcol_d)
{
    std::string input_file(MATRIX_FULL_PATH(impcol_d ));
    std::string output_file(RESULT_FULL_PATH(impcol_d ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, impcol_e)
{
    std::string input_file(MATRIX_FULL_PATH(impcol_e ));
    std::string output_file(RESULT_FULL_PATH(impcol_e ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, init_adder1)
{
    std::string input_file(MATRIX_FULL_PATH(init_adder1 ));
    std::string output_file(RESULT_FULL_PATH(init_adder1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, inlet)
{
    std::string input_file(MATRIX_FULL_PATH(inlet ));
    std::string output_file(RESULT_FULL_PATH(inlet ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, inline_1)
{
    std::string input_file(MATRIX_FULL_PATH(inline_1 ));
    std::string output_file(RESULT_FULL_PATH(inline_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ins2)
{
    std::string input_file(MATRIX_FULL_PATH(ins2 ));
    std::string output_file(RESULT_FULL_PATH(ins2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, internet)
{
    std::string input_file(MATRIX_FULL_PATH(internet ));
    std::string output_file(RESULT_FULL_PATH(internet ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, invextr1_new)
{
    std::string input_file(MATRIX_FULL_PATH(invextr1_new ));
    std::string output_file(RESULT_FULL_PATH(invextr1_new ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, iprob)
{
    std::string input_file(MATRIX_FULL_PATH(iprob ));
    std::string output_file(RESULT_FULL_PATH(iprob ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jagmesh1)
{
    std::string input_file(MATRIX_FULL_PATH(jagmesh1 ));
    std::string output_file(RESULT_FULL_PATH(jagmesh1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jagmesh2)
{
    std::string input_file(MATRIX_FULL_PATH(jagmesh2 ));
    std::string output_file(RESULT_FULL_PATH(jagmesh2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jagmesh3)
{
    std::string input_file(MATRIX_FULL_PATH(jagmesh3 ));
    std::string output_file(RESULT_FULL_PATH(jagmesh3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jagmesh4)
{
    std::string input_file(MATRIX_FULL_PATH(jagmesh4 ));
    std::string output_file(RESULT_FULL_PATH(jagmesh4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jagmesh5)
{
    std::string input_file(MATRIX_FULL_PATH(jagmesh5 ));
    std::string output_file(RESULT_FULL_PATH(jagmesh5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jagmesh6)
{
    std::string input_file(MATRIX_FULL_PATH(jagmesh6 ));
    std::string output_file(RESULT_FULL_PATH(jagmesh6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jagmesh7)
{
    std::string input_file(MATRIX_FULL_PATH(jagmesh7 ));
    std::string output_file(RESULT_FULL_PATH(jagmesh7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jagmesh8)
{
    std::string input_file(MATRIX_FULL_PATH(jagmesh8 ));
    std::string output_file(RESULT_FULL_PATH(jagmesh8 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jagmesh9)
{
    std::string input_file(MATRIX_FULL_PATH(jagmesh9 ));
    std::string output_file(RESULT_FULL_PATH(jagmesh9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac020sc)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac020sc ));
    std::string output_file(RESULT_FULL_PATH(jan99jac020sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac020)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac020 ));
    std::string output_file(RESULT_FULL_PATH(jan99jac020 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac040sc)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac040sc ));
    std::string output_file(RESULT_FULL_PATH(jan99jac040sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac040)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac040 ));
    std::string output_file(RESULT_FULL_PATH(jan99jac040 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac060sc)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac060sc ));
    std::string output_file(RESULT_FULL_PATH(jan99jac060sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac060)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac060 ));
    std::string output_file(RESULT_FULL_PATH(jan99jac060 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac080sc)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac080sc ));
    std::string output_file(RESULT_FULL_PATH(jan99jac080sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac080)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac080 ));
    std::string output_file(RESULT_FULL_PATH(jan99jac080 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac100sc)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac100sc ));
    std::string output_file(RESULT_FULL_PATH(jan99jac100sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac100)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac100 ));
    std::string output_file(RESULT_FULL_PATH(jan99jac100 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac120sc)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac120sc ));
    std::string output_file(RESULT_FULL_PATH(jan99jac120sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jan99jac120)
{
    std::string input_file(MATRIX_FULL_PATH(jan99jac120 ));
    std::string output_file(RESULT_FULL_PATH(jan99jac120 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jazz)
{
    std::string input_file(MATRIX_FULL_PATH(jazz ));
    std::string output_file(RESULT_FULL_PATH(jazz ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jnlbrng1)
{
    std::string input_file(MATRIX_FULL_PATH(jnlbrng1 ));
    std::string output_file(RESULT_FULL_PATH(jnlbrng1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, jpwh_991)
{
    std::string input_file(MATRIX_FULL_PATH(jpwh_991 ));
    std::string output_file(RESULT_FULL_PATH(jpwh_991 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, juba40k)
{
    std::string input_file(MATRIX_FULL_PATH(juba40k ));
    std::string output_file(RESULT_FULL_PATH(juba40k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, k1_san)
{
    std::string input_file(MATRIX_FULL_PATH(k1_san ));
    std::string output_file(RESULT_FULL_PATH(k1_san ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, k3plates)
{
    std::string input_file(MATRIX_FULL_PATH(k3plates ));
    std::string output_file(RESULT_FULL_PATH(k3plates ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Kaufhold)
{
    std::string input_file(MATRIX_FULL_PATH(Kaufhold ));
    std::string output_file(RESULT_FULL_PATH(Kaufhold ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, kim1)
{
    std::string input_file(MATRIX_FULL_PATH(kim1 ));
    std::string output_file(RESULT_FULL_PATH(kim1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, kim2)
{
    std::string input_file(MATRIX_FULL_PATH(kim2 ));
    std::string output_file(RESULT_FULL_PATH(kim2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Kohonen)
{
    std::string input_file(MATRIX_FULL_PATH(Kohonen ));
    std::string output_file(RESULT_FULL_PATH(Kohonen ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, kron_g500_logn16)
{
    std::string input_file(MATRIX_FULL_PATH(kron_g500-logn16 ));
    std::string output_file(RESULT_FULL_PATH(kron_g500-logn16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, kron_g500_logn17)
{
    std::string input_file(MATRIX_FULL_PATH(kron_g500-logn17 ));
    std::string output_file(RESULT_FULL_PATH(kron_g500-logn17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, kron_g500_logn18)
{
    std::string input_file(MATRIX_FULL_PATH(kron_g500-logn18 ));
    std::string output_file(RESULT_FULL_PATH(kron_g500-logn18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, kron_g500_logn19)
{
    std::string input_file(MATRIX_FULL_PATH(kron_g500-logn19 ));
    std::string output_file(RESULT_FULL_PATH(kron_g500-logn19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Kuu)
{
    std::string input_file(MATRIX_FULL_PATH(Kuu ));
    std::string output_file(RESULT_FULL_PATH(Kuu ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, L_9)
{
    std::string input_file(MATRIX_FULL_PATH(L-9 ));
    std::string output_file(RESULT_FULL_PATH(L-9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, laminar_duct3D)
{
    std::string input_file(MATRIX_FULL_PATH(laminar_duct3D ));
    std::string output_file(RESULT_FULL_PATH(laminar_duct3D ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, language)
{
    std::string input_file(MATRIX_FULL_PATH(language ));
    std::string output_file(RESULT_FULL_PATH(language ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, largebasis)
{
    std::string input_file(MATRIX_FULL_PATH(largebasis ));
    std::string output_file(RESULT_FULL_PATH(largebasis ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, laser)
{
    std::string input_file(MATRIX_FULL_PATH(laser ));
    std::string output_file(RESULT_FULL_PATH(laser ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Lederberg)
{
    std::string input_file(MATRIX_FULL_PATH(Lederberg ));
    std::string output_file(RESULT_FULL_PATH(Lederberg ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, LF10000)
{
    std::string input_file(MATRIX_FULL_PATH(LF10000 ));
    std::string output_file(RESULT_FULL_PATH(LF10000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, LFAT5000)
{
    std::string input_file(MATRIX_FULL_PATH(LFAT5000 ));
    std::string output_file(RESULT_FULL_PATH(LFAT5000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr01)
{
    std::string input_file(MATRIX_FULL_PATH(lhr01 ));
    std::string output_file(RESULT_FULL_PATH(lhr01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr02)
{
    std::string input_file(MATRIX_FULL_PATH(lhr02 ));
    std::string output_file(RESULT_FULL_PATH(lhr02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr04c)
{
    std::string input_file(MATRIX_FULL_PATH(lhr04c ));
    std::string output_file(RESULT_FULL_PATH(lhr04c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr04)
{
    std::string input_file(MATRIX_FULL_PATH(lhr04 ));
    std::string output_file(RESULT_FULL_PATH(lhr04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr07c)
{
    std::string input_file(MATRIX_FULL_PATH(lhr07c ));
    std::string output_file(RESULT_FULL_PATH(lhr07c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr07)
{
    std::string input_file(MATRIX_FULL_PATH(lhr07 ));
    std::string output_file(RESULT_FULL_PATH(lhr07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr10c)
{
    std::string input_file(MATRIX_FULL_PATH(lhr10c ));
    std::string output_file(RESULT_FULL_PATH(lhr10c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr10)
{
    std::string input_file(MATRIX_FULL_PATH(lhr10 ));
    std::string output_file(RESULT_FULL_PATH(lhr10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr11c)
{
    std::string input_file(MATRIX_FULL_PATH(lhr11c ));
    std::string output_file(RESULT_FULL_PATH(lhr11c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr11)
{
    std::string input_file(MATRIX_FULL_PATH(lhr11 ));
    std::string output_file(RESULT_FULL_PATH(lhr11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr14c)
{
    std::string input_file(MATRIX_FULL_PATH(lhr14c ));
    std::string output_file(RESULT_FULL_PATH(lhr14c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr17c)
{
    std::string input_file(MATRIX_FULL_PATH(lhr17c ));
    std::string output_file(RESULT_FULL_PATH(lhr17c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr17)
{
    std::string input_file(MATRIX_FULL_PATH(lhr17 ));
    std::string output_file(RESULT_FULL_PATH(lhr17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr34c)
{
    std::string input_file(MATRIX_FULL_PATH(lhr34c ));
    std::string output_file(RESULT_FULL_PATH(lhr34c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr34)
{
    std::string input_file(MATRIX_FULL_PATH(lhr34 ));
    std::string output_file(RESULT_FULL_PATH(lhr34 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr71c)
{
    std::string input_file(MATRIX_FULL_PATH(lhr71c ));
    std::string output_file(RESULT_FULL_PATH(lhr71c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lhr71)
{
    std::string input_file(MATRIX_FULL_PATH(lhr71 ));
    std::string output_file(RESULT_FULL_PATH(lhr71 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, light_in_tissue)
{
    std::string input_file(MATRIX_FULL_PATH(light_in_tissue ));
    std::string output_file(RESULT_FULL_PATH(light_in_tissue ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Lin)
{
    std::string input_file(MATRIX_FULL_PATH(Lin ));
    std::string output_file(RESULT_FULL_PATH(Lin ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, linverse)
{
    std::string input_file(MATRIX_FULL_PATH(linverse ));
    std::string output_file(RESULT_FULL_PATH(linverse ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, li)
{
    std::string input_file(MATRIX_FULL_PATH(li ));
    std::string output_file(RESULT_FULL_PATH(li ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lns_3937)
{
    std::string input_file(MATRIX_FULL_PATH(lns_3937 ));
    std::string output_file(RESULT_FULL_PATH(lns_3937 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lns_511)
{
    std::string input_file(MATRIX_FULL_PATH(lns_511 ));
    std::string output_file(RESULT_FULL_PATH(lns_511 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lnsp3937)
{
    std::string input_file(MATRIX_FULL_PATH(lnsp3937 ));
    std::string output_file(RESULT_FULL_PATH(lnsp3937 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lnsp_511)
{
    std::string input_file(MATRIX_FULL_PATH(lnsp_511 ));
    std::string output_file(RESULT_FULL_PATH(lnsp_511 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lock1074)
{
    std::string input_file(MATRIX_FULL_PATH(lock1074 ));
    std::string output_file(RESULT_FULL_PATH(lock1074 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lock2232)
{
    std::string input_file(MATRIX_FULL_PATH(lock2232 ));
    std::string output_file(RESULT_FULL_PATH(lock2232 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lock3491)
{
    std::string input_file(MATRIX_FULL_PATH(lock3491 ));
    std::string output_file(RESULT_FULL_PATH(lock3491 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lock_700)
{
    std::string input_file(MATRIX_FULL_PATH(lock_700 ));
    std::string output_file(RESULT_FULL_PATH(lock_700 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lpl1)
{
    std::string input_file(MATRIX_FULL_PATH(lpl1 ));
    std::string output_file(RESULT_FULL_PATH(lpl1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp1009)
{
    std::string input_file(MATRIX_FULL_PATH(lshp1009 ));
    std::string output_file(RESULT_FULL_PATH(lshp1009 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp1270)
{
    std::string input_file(MATRIX_FULL_PATH(lshp1270 ));
    std::string output_file(RESULT_FULL_PATH(lshp1270 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp1561)
{
    std::string input_file(MATRIX_FULL_PATH(lshp1561 ));
    std::string output_file(RESULT_FULL_PATH(lshp1561 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp1882)
{
    std::string input_file(MATRIX_FULL_PATH(lshp1882 ));
    std::string output_file(RESULT_FULL_PATH(lshp1882 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp2233)
{
    std::string input_file(MATRIX_FULL_PATH(lshp2233 ));
    std::string output_file(RESULT_FULL_PATH(lshp2233 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp2614)
{
    std::string input_file(MATRIX_FULL_PATH(lshp2614 ));
    std::string output_file(RESULT_FULL_PATH(lshp2614 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp_265)
{
    std::string input_file(MATRIX_FULL_PATH(lshp_265 ));
    std::string output_file(RESULT_FULL_PATH(lshp_265 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp3025)
{
    std::string input_file(MATRIX_FULL_PATH(lshp3025 ));
    std::string output_file(RESULT_FULL_PATH(lshp3025 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp3466)
{
    std::string input_file(MATRIX_FULL_PATH(lshp3466 ));
    std::string output_file(RESULT_FULL_PATH(lshp3466 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp_406)
{
    std::string input_file(MATRIX_FULL_PATH(lshp_406 ));
    std::string output_file(RESULT_FULL_PATH(lshp_406 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp_577)
{
    std::string input_file(MATRIX_FULL_PATH(lshp_577 ));
    std::string output_file(RESULT_FULL_PATH(lshp_577 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lshp_778)
{
    std::string input_file(MATRIX_FULL_PATH(lshp_778 ));
    std::string output_file(RESULT_FULL_PATH(lshp_778 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, L)
{
    std::string input_file(MATRIX_FULL_PATH(L ));
    std::string output_file(RESULT_FULL_PATH(L ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lund_a)
{
    std::string input_file(MATRIX_FULL_PATH(lund_a ));
    std::string output_file(RESULT_FULL_PATH(lund_a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lund_b)
{
    std::string input_file(MATRIX_FULL_PATH(lund_b ));
    std::string output_file(RESULT_FULL_PATH(lund_b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lung1)
{
    std::string input_file(MATRIX_FULL_PATH(lung1 ));
    std::string output_file(RESULT_FULL_PATH(lung1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, lung2)
{
    std::string input_file(MATRIX_FULL_PATH(lung2 ));
    std::string output_file(RESULT_FULL_PATH(lung2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, luxembourg_osm)
{
    std::string input_file(MATRIX_FULL_PATH(luxembourg_osm ));
    std::string output_file(RESULT_FULL_PATH(luxembourg_osm ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, M10PI_n1)
{
    std::string input_file(MATRIX_FULL_PATH(M10PI_n1 ));
    std::string output_file(RESULT_FULL_PATH(M10PI_n1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, M10PI_n)
{
    std::string input_file(MATRIX_FULL_PATH(M10PI_n ));
    std::string output_file(RESULT_FULL_PATH(M10PI_n ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, m133_b3)
{
    std::string input_file(MATRIX_FULL_PATH(m133-b3 ));
    std::string output_file(RESULT_FULL_PATH(m133-b3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, m14b)
{
    std::string input_file(MATRIX_FULL_PATH(m14b ));
    std::string output_file(RESULT_FULL_PATH(m14b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, M20PI_n1)
{
    std::string input_file(MATRIX_FULL_PATH(M20PI_n1 ));
    std::string output_file(RESULT_FULL_PATH(M20PI_n1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, M20PI_n)
{
    std::string input_file(MATRIX_FULL_PATH(M20PI_n ));
    std::string output_file(RESULT_FULL_PATH(M20PI_n ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, m3plates)
{
    std::string input_file(MATRIX_FULL_PATH(m3plates ));
    std::string output_file(RESULT_FULL_PATH(m3plates ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, M40PI_n1)
{
    std::string input_file(MATRIX_FULL_PATH(M40PI_n1 ));
    std::string output_file(RESULT_FULL_PATH(M40PI_n1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, M40PI_n)
{
    std::string input_file(MATRIX_FULL_PATH(M40PI_n ));
    std::string output_file(RESULT_FULL_PATH(M40PI_n ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, M80PI_n1)
{
    std::string input_file(MATRIX_FULL_PATH(M80PI_n1 ));
    std::string output_file(RESULT_FULL_PATH(M80PI_n1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, M80PI_n)
{
    std::string input_file(MATRIX_FULL_PATH(M80PI_n ));
    std::string output_file(RESULT_FULL_PATH(M80PI_n ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mac_econ_fwd500)
{
    std::string input_file(MATRIX_FULL_PATH(mac_econ_fwd500 ));
    std::string output_file(RESULT_FULL_PATH(mac_econ_fwd500 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mahindas)
{
    std::string input_file(MATRIX_FULL_PATH(mahindas ));
    std::string output_file(RESULT_FULL_PATH(mahindas ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, majorbasis)
{
    std::string input_file(MATRIX_FULL_PATH(majorbasis ));
    std::string output_file(RESULT_FULL_PATH(majorbasis ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, man_5976)
{
    std::string input_file(MATRIX_FULL_PATH(man_5976 ));
    std::string output_file(RESULT_FULL_PATH(man_5976 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mario001)
{
    std::string input_file(MATRIX_FULL_PATH(mario001 ));
    std::string output_file(RESULT_FULL_PATH(mario001 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mario002)
{
    std::string input_file(MATRIX_FULL_PATH(mario002 ));
    std::string output_file(RESULT_FULL_PATH(mario002 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac020sc)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac020sc ));
    std::string output_file(RESULT_FULL_PATH(mark3jac020sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac020)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac020 ));
    std::string output_file(RESULT_FULL_PATH(mark3jac020 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac040sc)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac040sc ));
    std::string output_file(RESULT_FULL_PATH(mark3jac040sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac040)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac040 ));
    std::string output_file(RESULT_FULL_PATH(mark3jac040 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac060sc)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac060sc ));
    std::string output_file(RESULT_FULL_PATH(mark3jac060sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac060)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac060 ));
    std::string output_file(RESULT_FULL_PATH(mark3jac060 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac080sc)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac080sc ));
    std::string output_file(RESULT_FULL_PATH(mark3jac080sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac080)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac080 ));
    std::string output_file(RESULT_FULL_PATH(mark3jac080 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac100sc)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac100sc ));
    std::string output_file(RESULT_FULL_PATH(mark3jac100sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac100)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac100 ));
    std::string output_file(RESULT_FULL_PATH(mark3jac100 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac120sc)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac120sc ));
    std::string output_file(RESULT_FULL_PATH(mark3jac120sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac120)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac120 ));
    std::string output_file(RESULT_FULL_PATH(mark3jac120 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac140sc)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac140sc ));
    std::string output_file(RESULT_FULL_PATH(mark3jac140sc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mark3jac140)
{
    std::string input_file(MATRIX_FULL_PATH(mark3jac140 ));
    std::string output_file(RESULT_FULL_PATH(mark3jac140 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, matrix_9)
{
    std::string input_file(MATRIX_FULL_PATH(matrix_9 ));
    std::string output_file(RESULT_FULL_PATH(matrix_9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, matrix_new_3)
{
    std::string input_file(MATRIX_FULL_PATH(matrix-new_3 ));
    std::string output_file(RESULT_FULL_PATH(matrix-new_3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mbeacxc)
{
    std::string input_file(MATRIX_FULL_PATH(mbeacxc ));
    std::string output_file(RESULT_FULL_PATH(mbeacxc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mbeaflw)
{
    std::string input_file(MATRIX_FULL_PATH(mbeaflw ));
    std::string output_file(RESULT_FULL_PATH(mbeaflw ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mbeause)
{
    std::string input_file(MATRIX_FULL_PATH(mbeause ));
    std::string output_file(RESULT_FULL_PATH(mbeause ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mc2depi)
{
    std::string input_file(MATRIX_FULL_PATH(mc2depi ));
    std::string output_file(RESULT_FULL_PATH(mc2depi ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mcca)
{
    std::string input_file(MATRIX_FULL_PATH(mcca ));
    std::string output_file(RESULT_FULL_PATH(mcca ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mcfe)
{
    std::string input_file(MATRIX_FULL_PATH(mcfe ));
    std::string output_file(RESULT_FULL_PATH(mcfe ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, meg1)
{
    std::string input_file(MATRIX_FULL_PATH(meg1 ));
    std::string output_file(RESULT_FULL_PATH(meg1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, meg4)
{
    std::string input_file(MATRIX_FULL_PATH(meg4 ));
    std::string output_file(RESULT_FULL_PATH(meg4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, memplus)
{
    std::string input_file(MATRIX_FULL_PATH(memplus ));
    std::string output_file(RESULT_FULL_PATH(memplus ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mesh2e1)
{
    std::string input_file(MATRIX_FULL_PATH(mesh2e1 ));
    std::string output_file(RESULT_FULL_PATH(mesh2e1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mesh2em5)
{
    std::string input_file(MATRIX_FULL_PATH(mesh2em5 ));
    std::string output_file(RESULT_FULL_PATH(mesh2em5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mesh3e1)
{
    std::string input_file(MATRIX_FULL_PATH(mesh3e1 ));
    std::string output_file(RESULT_FULL_PATH(mesh3e1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mesh3em5)
{
    std::string input_file(MATRIX_FULL_PATH(mesh3em5 ));
    std::string output_file(RESULT_FULL_PATH(mesh3em5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mhd1280a)
{
    std::string input_file(MATRIX_FULL_PATH(mhd1280a ));
    std::string output_file(RESULT_FULL_PATH(mhd1280a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mhd1280b)
{
    std::string input_file(MATRIX_FULL_PATH(mhd1280b ));
    std::string output_file(RESULT_FULL_PATH(mhd1280b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mhd3200a)
{
    std::string input_file(MATRIX_FULL_PATH(mhd3200a ));
    std::string output_file(RESULT_FULL_PATH(mhd3200a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mhd3200b)
{
    std::string input_file(MATRIX_FULL_PATH(mhd3200b ));
    std::string output_file(RESULT_FULL_PATH(mhd3200b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mhd4800a)
{
    std::string input_file(MATRIX_FULL_PATH(mhd4800a ));
    std::string output_file(RESULT_FULL_PATH(mhd4800a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mhd4800b)
{
    std::string input_file(MATRIX_FULL_PATH(mhd4800b ));
    std::string output_file(RESULT_FULL_PATH(mhd4800b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mhda416)
{
    std::string input_file(MATRIX_FULL_PATH(mhda416 ));
    std::string output_file(RESULT_FULL_PATH(mhda416 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mhdb416)
{
    std::string input_file(MATRIX_FULL_PATH(mhdb416 ));
    std::string output_file(RESULT_FULL_PATH(mhdb416 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mimo28x28_system)
{
    std::string input_file(MATRIX_FULL_PATH(mimo28x28_system ));
    std::string output_file(RESULT_FULL_PATH(mimo28x28_system ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mimo46x46_system)
{
    std::string input_file(MATRIX_FULL_PATH(mimo46x46_system ));
    std::string output_file(RESULT_FULL_PATH(mimo46x46_system ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mimo8x8_system)
{
    std::string input_file(MATRIX_FULL_PATH(mimo8x8_system ));
    std::string output_file(RESULT_FULL_PATH(mimo8x8_system ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, minnesota)
{
    std::string input_file(MATRIX_FULL_PATH(minnesota ));
    std::string output_file(RESULT_FULL_PATH(minnesota ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, minsurfo)
{
    std::string input_file(MATRIX_FULL_PATH(minsurfo ));
    std::string output_file(RESULT_FULL_PATH(minsurfo ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mip1)
{
    std::string input_file(MATRIX_FULL_PATH(mip1 ));
    std::string output_file(RESULT_FULL_PATH(mip1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mixtank_new)
{
    std::string input_file(MATRIX_FULL_PATH(mixtank_new ));
    std::string output_file(RESULT_FULL_PATH(mixtank_new ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mono_500Hz)
{
    std::string input_file(MATRIX_FULL_PATH(mono_500Hz ));
    std::string output_file(RESULT_FULL_PATH(mono_500Hz ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mouse_gene)
{
    std::string input_file(MATRIX_FULL_PATH(mouse_gene ));
    std::string output_file(RESULT_FULL_PATH(mouse_gene ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mplate)
{
    std::string input_file(MATRIX_FULL_PATH(mplate ));
    std::string output_file(RESULT_FULL_PATH(mplate ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, msc00726)
{
    std::string input_file(MATRIX_FULL_PATH(msc00726 ));
    std::string output_file(RESULT_FULL_PATH(msc00726 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, msc01050)
{
    std::string input_file(MATRIX_FULL_PATH(msc01050 ));
    std::string output_file(RESULT_FULL_PATH(msc01050 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, msc01440)
{
    std::string input_file(MATRIX_FULL_PATH(msc01440 ));
    std::string output_file(RESULT_FULL_PATH(msc01440 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, msc04515)
{
    std::string input_file(MATRIX_FULL_PATH(msc04515 ));
    std::string output_file(RESULT_FULL_PATH(msc04515 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, msc10848)
{
    std::string input_file(MATRIX_FULL_PATH(msc10848 ));
    std::string output_file(RESULT_FULL_PATH(msc10848 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, msc23052)
{
    std::string input_file(MATRIX_FULL_PATH(msc23052 ));
    std::string output_file(RESULT_FULL_PATH(msc23052 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, msdoor)
{
    std::string input_file(MATRIX_FULL_PATH(msdoor ));
    std::string output_file(RESULT_FULL_PATH(msdoor ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, m_t1)
{
    std::string input_file(MATRIX_FULL_PATH(m_t1 ));
    std::string output_file(RESULT_FULL_PATH(m_t1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mult_dcop_01)
{
    std::string input_file(MATRIX_FULL_PATH(mult_dcop_01 ));
    std::string output_file(RESULT_FULL_PATH(mult_dcop_01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mult_dcop_02)
{
    std::string input_file(MATRIX_FULL_PATH(mult_dcop_02 ));
    std::string output_file(RESULT_FULL_PATH(mult_dcop_02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, mult_dcop_03)
{
    std::string input_file(MATRIX_FULL_PATH(mult_dcop_03 ));
    std::string output_file(RESULT_FULL_PATH(mult_dcop_03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Muu)
{
    std::string input_file(MATRIX_FULL_PATH(Muu ));
    std::string output_file(RESULT_FULL_PATH(Muu ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, n3c6_b7)
{
    std::string input_file(MATRIX_FULL_PATH(n3c6-b7 ));
    std::string output_file(RESULT_FULL_PATH(n3c6-b7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Na5)
{
    std::string input_file(MATRIX_FULL_PATH(Na5 ));
    std::string output_file(RESULT_FULL_PATH(Na5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nasa1824)
{
    std::string input_file(MATRIX_FULL_PATH(nasa1824 ));
    std::string output_file(RESULT_FULL_PATH(nasa1824 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nasa2146)
{
    std::string input_file(MATRIX_FULL_PATH(nasa2146 ));
    std::string output_file(RESULT_FULL_PATH(nasa2146 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nasa2910)
{
    std::string input_file(MATRIX_FULL_PATH(nasa2910 ));
    std::string output_file(RESULT_FULL_PATH(nasa2910 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nasa4704)
{
    std::string input_file(MATRIX_FULL_PATH(nasa4704 ));
    std::string output_file(RESULT_FULL_PATH(nasa4704 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nasasrb)
{
    std::string input_file(MATRIX_FULL_PATH(nasasrb ));
    std::string output_file(RESULT_FULL_PATH(nasasrb ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ncvxbqp1)
{
    std::string input_file(MATRIX_FULL_PATH(ncvxbqp1 ));
    std::string output_file(RESULT_FULL_PATH(ncvxbqp1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ncvxqp1)
{
    std::string input_file(MATRIX_FULL_PATH(ncvxqp1 ));
    std::string output_file(RESULT_FULL_PATH(ncvxqp1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ncvxqp3)
{
    std::string input_file(MATRIX_FULL_PATH(ncvxqp3 ));
    std::string output_file(RESULT_FULL_PATH(ncvxqp3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ncvxqp5)
{
    std::string input_file(MATRIX_FULL_PATH(ncvxqp5 ));
    std::string output_file(RESULT_FULL_PATH(ncvxqp5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ncvxqp7)
{
    std::string input_file(MATRIX_FULL_PATH(ncvxqp7 ));
    std::string output_file(RESULT_FULL_PATH(ncvxqp7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ncvxqp9)
{
    std::string input_file(MATRIX_FULL_PATH(ncvxqp9 ));
    std::string output_file(RESULT_FULL_PATH(ncvxqp9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nd12k)
{
    std::string input_file(MATRIX_FULL_PATH(nd12k ));
    std::string output_file(RESULT_FULL_PATH(nd12k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nd24k)
{
    std::string input_file(MATRIX_FULL_PATH(nd24k ));
    std::string output_file(RESULT_FULL_PATH(nd24k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nd3k)
{
    std::string input_file(MATRIX_FULL_PATH(nd3k ));
    std::string output_file(RESULT_FULL_PATH(nd3k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nd6k)
{
    std::string input_file(MATRIX_FULL_PATH(nd6k ));
    std::string output_file(RESULT_FULL_PATH(nd6k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth01)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth01 ));
    std::string output_file(RESULT_FULL_PATH(nemeth01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth02)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth02 ));
    std::string output_file(RESULT_FULL_PATH(nemeth02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth03)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth03 ));
    std::string output_file(RESULT_FULL_PATH(nemeth03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth04)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth04 ));
    std::string output_file(RESULT_FULL_PATH(nemeth04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth05)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth05 ));
    std::string output_file(RESULT_FULL_PATH(nemeth05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth06)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth06 ));
    std::string output_file(RESULT_FULL_PATH(nemeth06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth07)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth07 ));
    std::string output_file(RESULT_FULL_PATH(nemeth07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth08)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth08 ));
    std::string output_file(RESULT_FULL_PATH(nemeth08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth09)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth09 ));
    std::string output_file(RESULT_FULL_PATH(nemeth09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth10)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth10 ));
    std::string output_file(RESULT_FULL_PATH(nemeth10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth11)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth11 ));
    std::string output_file(RESULT_FULL_PATH(nemeth11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth12)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth12 ));
    std::string output_file(RESULT_FULL_PATH(nemeth12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth13)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth13 ));
    std::string output_file(RESULT_FULL_PATH(nemeth13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth14)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth14 ));
    std::string output_file(RESULT_FULL_PATH(nemeth14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth15)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth15 ));
    std::string output_file(RESULT_FULL_PATH(nemeth15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth16)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth16 ));
    std::string output_file(RESULT_FULL_PATH(nemeth16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth17)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth17 ));
    std::string output_file(RESULT_FULL_PATH(nemeth17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth18)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth18 ));
    std::string output_file(RESULT_FULL_PATH(nemeth18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth19)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth19 ));
    std::string output_file(RESULT_FULL_PATH(nemeth19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth20)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth20 ));
    std::string output_file(RESULT_FULL_PATH(nemeth20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth21)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth21 ));
    std::string output_file(RESULT_FULL_PATH(nemeth21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth22)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth22 ));
    std::string output_file(RESULT_FULL_PATH(nemeth22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth23)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth23 ));
    std::string output_file(RESULT_FULL_PATH(nemeth23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth24)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth24 ));
    std::string output_file(RESULT_FULL_PATH(nemeth24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth25)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth25 ));
    std::string output_file(RESULT_FULL_PATH(nemeth25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nemeth26)
{
    std::string input_file(MATRIX_FULL_PATH(nemeth26 ));
    std::string output_file(RESULT_FULL_PATH(nemeth26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, net100)
{
    std::string input_file(MATRIX_FULL_PATH(net100 ));
    std::string output_file(RESULT_FULL_PATH(net100 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, net125)
{
    std::string input_file(MATRIX_FULL_PATH(net125 ));
    std::string output_file(RESULT_FULL_PATH(net125 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, net150)
{
    std::string input_file(MATRIX_FULL_PATH(net150 ));
    std::string output_file(RESULT_FULL_PATH(net150 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, net25)
{
    std::string input_file(MATRIX_FULL_PATH(net25 ));
    std::string output_file(RESULT_FULL_PATH(net25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, net4_1)
{
    std::string input_file(MATRIX_FULL_PATH(net4-1 ));
    std::string output_file(RESULT_FULL_PATH(net4-1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, net50)
{
    std::string input_file(MATRIX_FULL_PATH(net50 ));
    std::string output_file(RESULT_FULL_PATH(net50 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, net75)
{
    std::string input_file(MATRIX_FULL_PATH(net75 ));
    std::string output_file(RESULT_FULL_PATH(net75 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, netscience)
{
    std::string input_file(MATRIX_FULL_PATH(netscience ));
    std::string output_file(RESULT_FULL_PATH(netscience ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, netz4504_dual)
{
    std::string input_file(MATRIX_FULL_PATH(netz4504_dual ));
    std::string output_file(RESULT_FULL_PATH(netz4504_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, netz4504)
{
    std::string input_file(MATRIX_FULL_PATH(netz4504 ));
    std::string output_file(RESULT_FULL_PATH(netz4504 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nmos3)
{
    std::string input_file(MATRIX_FULL_PATH(nmos3 ));
    std::string output_file(RESULT_FULL_PATH(nmos3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nnc1374)
{
    std::string input_file(MATRIX_FULL_PATH(nnc1374 ));
    std::string output_file(RESULT_FULL_PATH(nnc1374 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nnc261)
{
    std::string input_file(MATRIX_FULL_PATH(nnc261 ));
    std::string output_file(RESULT_FULL_PATH(nnc261 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nnc666)
{
    std::string input_file(MATRIX_FULL_PATH(nnc666 ));
    std::string output_file(RESULT_FULL_PATH(nnc666 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nopoly)
{
    std::string input_file(MATRIX_FULL_PATH(nopoly ));
    std::string output_file(RESULT_FULL_PATH(nopoly ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nopss_11k)
{
    std::string input_file(MATRIX_FULL_PATH(nopss_11k ));
    std::string output_file(RESULT_FULL_PATH(nopss_11k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nos2)
{
    std::string input_file(MATRIX_FULL_PATH(nos2 ));
    std::string output_file(RESULT_FULL_PATH(nos2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nos3)
{
    std::string input_file(MATRIX_FULL_PATH(nos3 ));
    std::string output_file(RESULT_FULL_PATH(nos3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nos5)
{
    std::string input_file(MATRIX_FULL_PATH(nos5 ));
    std::string output_file(RESULT_FULL_PATH(nos5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nos6)
{
    std::string input_file(MATRIX_FULL_PATH(nos6 ));
    std::string output_file(RESULT_FULL_PATH(nos6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, nos7)
{
    std::string input_file(MATRIX_FULL_PATH(nos7 ));
    std::string output_file(RESULT_FULL_PATH(nos7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, NotreDame_www)
{
    std::string input_file(MATRIX_FULL_PATH(NotreDame_www ));
    std::string output_file(RESULT_FULL_PATH(NotreDame_www ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, NotreDame_yeast)
{
    std::string input_file(MATRIX_FULL_PATH(NotreDame_yeast ));
    std::string output_file(RESULT_FULL_PATH(NotreDame_yeast ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ns3Da)
{
    std::string input_file(MATRIX_FULL_PATH(ns3Da ));
    std::string output_file(RESULT_FULL_PATH(ns3Da ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, obstclae)
{
    std::string input_file(MATRIX_FULL_PATH(obstclae ));
    std::string output_file(RESULT_FULL_PATH(obstclae ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, odepa400)
{
    std::string input_file(MATRIX_FULL_PATH(odepa400 ));
    std::string output_file(RESULT_FULL_PATH(odepa400 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ODLIS)
{
    std::string input_file(MATRIX_FULL_PATH(ODLIS ));
    std::string output_file(RESULT_FULL_PATH(ODLIS ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, offshore)
{
    std::string input_file(MATRIX_FULL_PATH(offshore ));
    std::string output_file(RESULT_FULL_PATH(offshore ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ohne2)
{
    std::string input_file(MATRIX_FULL_PATH(ohne2 ));
    std::string output_file(RESULT_FULL_PATH(ohne2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oilpan)
{
    std::string input_file(MATRIX_FULL_PATH(oilpan ));
    std::string output_file(RESULT_FULL_PATH(oilpan ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, olafu)
{
    std::string input_file(MATRIX_FULL_PATH(olafu ));
    std::string output_file(RESULT_FULL_PATH(olafu ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, olesnik0)
{
    std::string input_file(MATRIX_FULL_PATH(olesnik0 ));
    std::string output_file(RESULT_FULL_PATH(olesnik0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, olm1000)
{
    std::string input_file(MATRIX_FULL_PATH(olm1000 ));
    std::string output_file(RESULT_FULL_PATH(olm1000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, olm2000)
{
    std::string input_file(MATRIX_FULL_PATH(olm2000 ));
    std::string output_file(RESULT_FULL_PATH(olm2000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, olm5000)
{
    std::string input_file(MATRIX_FULL_PATH(olm5000 ));
    std::string output_file(RESULT_FULL_PATH(olm5000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, olm500)
{
    std::string input_file(MATRIX_FULL_PATH(olm500 ));
    std::string output_file(RESULT_FULL_PATH(olm500 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, onera_dual)
{
    std::string input_file(MATRIX_FULL_PATH(onera_dual ));
    std::string output_file(RESULT_FULL_PATH(onera_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, onetone1)
{
    std::string input_file(MATRIX_FULL_PATH(onetone1 ));
    std::string output_file(RESULT_FULL_PATH(onetone1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, onetone2)
{
    std::string input_file(MATRIX_FULL_PATH(onetone2 ));
    std::string output_file(RESULT_FULL_PATH(onetone2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, OPF_10000)
{
    std::string input_file(MATRIX_FULL_PATH(OPF_10000 ));
    std::string output_file(RESULT_FULL_PATH(OPF_10000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, OPF_3754)
{
    std::string input_file(MATRIX_FULL_PATH(OPF_3754 ));
    std::string output_file(RESULT_FULL_PATH(OPF_3754 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, OPF_6000)
{
    std::string input_file(MATRIX_FULL_PATH(OPF_6000 ));
    std::string output_file(RESULT_FULL_PATH(OPF_6000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, opt1)
{
    std::string input_file(MATRIX_FULL_PATH(opt1 ));
    std::string output_file(RESULT_FULL_PATH(opt1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, orani678)
{
    std::string input_file(MATRIX_FULL_PATH(orani678 ));
    std::string output_file(RESULT_FULL_PATH(orani678 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Oregon_1)
{
    std::string input_file(MATRIX_FULL_PATH(Oregon-1 ));
    std::string output_file(RESULT_FULL_PATH(Oregon-1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Oregon_2)
{
    std::string input_file(MATRIX_FULL_PATH(Oregon-2 ));
    std::string output_file(RESULT_FULL_PATH(Oregon-2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, orsirr_1)
{
    std::string input_file(MATRIX_FULL_PATH(orsirr_1 ));
    std::string output_file(RESULT_FULL_PATH(orsirr_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, orsirr_2)
{
    std::string input_file(MATRIX_FULL_PATH(orsirr_2 ));
    std::string output_file(RESULT_FULL_PATH(orsirr_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, orsreg_1)
{
    std::string input_file(MATRIX_FULL_PATH(orsreg_1 ));
    std::string output_file(RESULT_FULL_PATH(orsreg_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_01)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_01 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_02)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_02 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_03)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_03 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_04)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_04 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_05)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_05 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_06)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_06 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_07)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_07 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_08)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_08 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_09)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_09 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_10)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_10 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_11)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_11 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_12)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_12 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_13)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_13 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_14)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_14 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_15)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_15 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_16)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_16 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_17)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_17 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_18)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_18 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_19)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_19 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_20)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_20 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_21)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_21 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_22)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_22 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_23)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_23 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_24)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_24 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_25)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_25 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_26)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_26 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_27)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_27 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_27 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_28)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_28 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_28 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_29)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_29 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_29 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_30)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_30 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_31)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_31 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_31 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_32)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_32 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_32 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_33)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_33 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_33 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_34)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_34 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_34 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_35)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_35 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_35 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_36)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_36 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_36 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_37)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_37 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_37 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_38)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_38 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_38 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_39)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_39 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_39 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_40)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_40 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_40 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_41)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_41 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_41 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_42)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_42 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_42 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_43)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_43 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_43 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_44)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_44 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_44 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_45)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_45 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_45 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_46)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_46 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_46 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_47)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_47 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_47 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_48)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_48 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_48 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_49)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_49 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_49 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_50)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_50 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_50 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_51)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_51 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_51 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_52)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_52 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_52 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_53)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_53 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_53 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_54)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_54 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_54 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_55)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_55 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_55 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_56)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_56 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_56 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_dcop_57)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_dcop_57 ));
    std::string output_file(RESULT_FULL_PATH(oscil_dcop_57 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, oscil_trans_01)
{
    std::string input_file(MATRIX_FULL_PATH(oscil_trans_01 ));
    std::string output_file(RESULT_FULL_PATH(oscil_trans_01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, p2p_Gnutella04)
{
    std::string input_file(MATRIX_FULL_PATH(p2p-Gnutella04 ));
    std::string output_file(RESULT_FULL_PATH(p2p-Gnutella04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, p2p_Gnutella05)
{
    std::string input_file(MATRIX_FULL_PATH(p2p-Gnutella05 ));
    std::string output_file(RESULT_FULL_PATH(p2p-Gnutella05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, p2p_Gnutella06)
{
    std::string input_file(MATRIX_FULL_PATH(p2p-Gnutella06 ));
    std::string output_file(RESULT_FULL_PATH(p2p-Gnutella06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, p2p_Gnutella08)
{
    std::string input_file(MATRIX_FULL_PATH(p2p-Gnutella08 ));
    std::string output_file(RESULT_FULL_PATH(p2p-Gnutella08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, p2p_Gnutella09)
{
    std::string input_file(MATRIX_FULL_PATH(p2p-Gnutella09 ));
    std::string output_file(RESULT_FULL_PATH(p2p-Gnutella09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, p2p_Gnutella24)
{
    std::string input_file(MATRIX_FULL_PATH(p2p-Gnutella24 ));
    std::string output_file(RESULT_FULL_PATH(p2p-Gnutella24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, p2p_Gnutella25)
{
    std::string input_file(MATRIX_FULL_PATH(p2p-Gnutella25 ));
    std::string output_file(RESULT_FULL_PATH(p2p-Gnutella25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, p2p_Gnutella30)
{
    std::string input_file(MATRIX_FULL_PATH(p2p-Gnutella30 ));
    std::string output_file(RESULT_FULL_PATH(p2p-Gnutella30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, p2p_Gnutella31)
{
    std::string input_file(MATRIX_FULL_PATH(p2p-Gnutella31 ));
    std::string output_file(RESULT_FULL_PATH(p2p-Gnutella31 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, para_10)
{
    std::string input_file(MATRIX_FULL_PATH(para-10 ));
    std::string output_file(RESULT_FULL_PATH(para-10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, para_4)
{
    std::string input_file(MATRIX_FULL_PATH(para-4 ));
    std::string output_file(RESULT_FULL_PATH(para-4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, para_5)
{
    std::string input_file(MATRIX_FULL_PATH(para-5 ));
    std::string output_file(RESULT_FULL_PATH(para-5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, para_6)
{
    std::string input_file(MATRIX_FULL_PATH(para-6 ));
    std::string output_file(RESULT_FULL_PATH(para-6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, para_7)
{
    std::string input_file(MATRIX_FULL_PATH(para-7 ));
    std::string output_file(RESULT_FULL_PATH(para-7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, para_8)
{
    std::string input_file(MATRIX_FULL_PATH(para-8 ));
    std::string output_file(RESULT_FULL_PATH(para-8 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, para_9)
{
    std::string input_file(MATRIX_FULL_PATH(para-9 ));
    std::string output_file(RESULT_FULL_PATH(para-9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, patents_main)
{
    std::string input_file(MATRIX_FULL_PATH(patents_main ));
    std::string output_file(RESULT_FULL_PATH(patents_main ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pattern1)
{
    std::string input_file(MATRIX_FULL_PATH(pattern1 ));
    std::string output_file(RESULT_FULL_PATH(pattern1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pcrystk02)
{
    std::string input_file(MATRIX_FULL_PATH(pcrystk02 ));
    std::string output_file(RESULT_FULL_PATH(pcrystk02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pcrystk03)
{
    std::string input_file(MATRIX_FULL_PATH(pcrystk03 ));
    std::string output_file(RESULT_FULL_PATH(pcrystk03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pct20stif)
{
    std::string input_file(MATRIX_FULL_PATH(pct20stif ));
    std::string output_file(RESULT_FULL_PATH(pct20stif ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pdb1HYS)
{
    std::string input_file(MATRIX_FULL_PATH(pdb1HYS ));
    std::string output_file(RESULT_FULL_PATH(pdb1HYS ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pde225)
{
    std::string input_file(MATRIX_FULL_PATH(pde225 ));
    std::string output_file(RESULT_FULL_PATH(pde225 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pde2961)
{
    std::string input_file(MATRIX_FULL_PATH(pde2961 ));
    std::string output_file(RESULT_FULL_PATH(pde2961 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pde900)
{
    std::string input_file(MATRIX_FULL_PATH(pde900 ));
    std::string output_file(RESULT_FULL_PATH(pde900 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pds10)
{
    std::string input_file(MATRIX_FULL_PATH(pds10 ));
    std::string output_file(RESULT_FULL_PATH(pds10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Pd)
{
    std::string input_file(MATRIX_FULL_PATH(Pd ));
    std::string output_file(RESULT_FULL_PATH(Pd ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pesa)
{
    std::string input_file(MATRIX_FULL_PATH(pesa ));
    std::string output_file(RESULT_FULL_PATH(pesa ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pf2177)
{
    std::string input_file(MATRIX_FULL_PATH(pf2177 ));
    std::string output_file(RESULT_FULL_PATH(pf2177 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pfinan512)
{
    std::string input_file(MATRIX_FULL_PATH(pfinan512 ));
    std::string output_file(RESULT_FULL_PATH(pfinan512 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, PGPgiantcompo)
{
    std::string input_file(MATRIX_FULL_PATH(PGPgiantcompo ));
    std::string output_file(RESULT_FULL_PATH(PGPgiantcompo ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, piston)
{
    std::string input_file(MATRIX_FULL_PATH(piston ));
    std::string output_file(RESULT_FULL_PATH(piston ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk01)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk01 ));
    std::string output_file(RESULT_FULL_PATH(pkustk01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk02)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk02 ));
    std::string output_file(RESULT_FULL_PATH(pkustk02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk03)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk03 ));
    std::string output_file(RESULT_FULL_PATH(pkustk03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk04)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk04 ));
    std::string output_file(RESULT_FULL_PATH(pkustk04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk05)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk05 ));
    std::string output_file(RESULT_FULL_PATH(pkustk05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk06)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk06 ));
    std::string output_file(RESULT_FULL_PATH(pkustk06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk07)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk07 ));
    std::string output_file(RESULT_FULL_PATH(pkustk07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk08)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk08 ));
    std::string output_file(RESULT_FULL_PATH(pkustk08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk09)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk09 ));
    std::string output_file(RESULT_FULL_PATH(pkustk09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk10)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk10 ));
    std::string output_file(RESULT_FULL_PATH(pkustk10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk11)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk11 ));
    std::string output_file(RESULT_FULL_PATH(pkustk11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk12)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk12 ));
    std::string output_file(RESULT_FULL_PATH(pkustk12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk13)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk13 ));
    std::string output_file(RESULT_FULL_PATH(pkustk13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pkustk14)
{
    std::string input_file(MATRIX_FULL_PATH(pkustk14 ));
    std::string output_file(RESULT_FULL_PATH(pkustk14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, plat1919)
{
    std::string input_file(MATRIX_FULL_PATH(plat1919 ));
    std::string output_file(RESULT_FULL_PATH(plat1919 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, plat362)
{
    std::string input_file(MATRIX_FULL_PATH(plat362 ));
    std::string output_file(RESULT_FULL_PATH(plat362 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, plbuckle)
{
    std::string input_file(MATRIX_FULL_PATH(plbuckle ));
    std::string output_file(RESULT_FULL_PATH(plbuckle ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pli)
{
    std::string input_file(MATRIX_FULL_PATH(pli ));
    std::string output_file(RESULT_FULL_PATH(pli ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, plsk1919)
{
    std::string input_file(MATRIX_FULL_PATH(plsk1919 ));
    std::string output_file(RESULT_FULL_PATH(plsk1919 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, plskz362)
{
    std::string input_file(MATRIX_FULL_PATH(plskz362 ));
    std::string output_file(RESULT_FULL_PATH(plskz362 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, poisson2D)
{
    std::string input_file(MATRIX_FULL_PATH(poisson2D ));
    std::string output_file(RESULT_FULL_PATH(poisson2D ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, poisson3Da)
{
    std::string input_file(MATRIX_FULL_PATH(poisson3Da ));
    std::string output_file(RESULT_FULL_PATH(poisson3Da ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, poisson3Db)
{
    std::string input_file(MATRIX_FULL_PATH(poisson3Db ));
    std::string output_file(RESULT_FULL_PATH(poisson3Db ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, polblogs)
{
    std::string input_file(MATRIX_FULL_PATH(polblogs ));
    std::string output_file(RESULT_FULL_PATH(polblogs ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, poli3)
{
    std::string input_file(MATRIX_FULL_PATH(poli3 ));
    std::string output_file(RESULT_FULL_PATH(poli3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, poli4)
{
    std::string input_file(MATRIX_FULL_PATH(poli4 ));
    std::string output_file(RESULT_FULL_PATH(poli4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, poli_large)
{
    std::string input_file(MATRIX_FULL_PATH(poli_large ));
    std::string output_file(RESULT_FULL_PATH(poli_large ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, poli)
{
    std::string input_file(MATRIX_FULL_PATH(poli ));
    std::string output_file(RESULT_FULL_PATH(poli ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pores_2)
{
    std::string input_file(MATRIX_FULL_PATH(pores_2 ));
    std::string output_file(RESULT_FULL_PATH(pores_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pores_3)
{
    std::string input_file(MATRIX_FULL_PATH(pores_3 ));
    std::string output_file(RESULT_FULL_PATH(pores_3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, powersim)
{
    std::string input_file(MATRIX_FULL_PATH(powersim ));
    std::string output_file(RESULT_FULL_PATH(powersim ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, power)
{
    std::string input_file(MATRIX_FULL_PATH(power ));
    std::string output_file(RESULT_FULL_PATH(power ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, PR02R)
{
    std::string input_file(MATRIX_FULL_PATH(PR02R ));
    std::string output_file(RESULT_FULL_PATH(PR02R ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Pres_Poisson)
{
    std::string input_file(MATRIX_FULL_PATH(Pres_Poisson ));
    std::string output_file(RESULT_FULL_PATH(Pres_Poisson ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, problem1)
{
    std::string input_file(MATRIX_FULL_PATH(problem1 ));
    std::string output_file(RESULT_FULL_PATH(problem1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, psmigr_1)
{
    std::string input_file(MATRIX_FULL_PATH(psmigr_1 ));
    std::string output_file(RESULT_FULL_PATH(psmigr_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, psmigr_2)
{
    std::string input_file(MATRIX_FULL_PATH(psmigr_2 ));
    std::string output_file(RESULT_FULL_PATH(psmigr_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, psmigr_3)
{
    std::string input_file(MATRIX_FULL_PATH(psmigr_3 ));
    std::string output_file(RESULT_FULL_PATH(psmigr_3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pwtk)
{
    std::string input_file(MATRIX_FULL_PATH(pwtk ));
    std::string output_file(RESULT_FULL_PATH(pwtk ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, pwt)
{
    std::string input_file(MATRIX_FULL_PATH(pwt ));
    std::string output_file(RESULT_FULL_PATH(pwt ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, qa8fk)
{
    std::string input_file(MATRIX_FULL_PATH(qa8fk ));
    std::string output_file(RESULT_FULL_PATH(qa8fk ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, qa8fm)
{
    std::string input_file(MATRIX_FULL_PATH(qa8fm ));
    std::string output_file(RESULT_FULL_PATH(qa8fm ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, qc2534)
{
    std::string input_file(MATRIX_FULL_PATH(qc2534 ));
    std::string output_file(RESULT_FULL_PATH(qc2534 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, qc324)
{
    std::string input_file(MATRIX_FULL_PATH(qc324 ));
    std::string output_file(RESULT_FULL_PATH(qc324 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, qh1484)
{
    std::string input_file(MATRIX_FULL_PATH(qh1484 ));
    std::string output_file(RESULT_FULL_PATH(qh1484 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, qh768)
{
    std::string input_file(MATRIX_FULL_PATH(qh768 ));
    std::string output_file(RESULT_FULL_PATH(qh768 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, qh882)
{
    std::string input_file(MATRIX_FULL_PATH(qh882 ));
    std::string output_file(RESULT_FULL_PATH(qh882 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, qpband)
{
    std::string input_file(MATRIX_FULL_PATH(qpband ));
    std::string output_file(RESULT_FULL_PATH(qpband ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, radfr1)
{
    std::string input_file(MATRIX_FULL_PATH(radfr1 ));
    std::string output_file(RESULT_FULL_PATH(radfr1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, raefsky1)
{
    std::string input_file(MATRIX_FULL_PATH(raefsky1 ));
    std::string output_file(RESULT_FULL_PATH(raefsky1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, raefsky2)
{
    std::string input_file(MATRIX_FULL_PATH(raefsky2 ));
    std::string output_file(RESULT_FULL_PATH(raefsky2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, raefsky3)
{
    std::string input_file(MATRIX_FULL_PATH(raefsky3 ));
    std::string output_file(RESULT_FULL_PATH(raefsky3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, raefsky4)
{
    std::string input_file(MATRIX_FULL_PATH(raefsky4 ));
    std::string output_file(RESULT_FULL_PATH(raefsky4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, raefsky5)
{
    std::string input_file(MATRIX_FULL_PATH(raefsky5 ));
    std::string output_file(RESULT_FULL_PATH(raefsky5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, raefsky6)
{
    std::string input_file(MATRIX_FULL_PATH(raefsky6 ));
    std::string output_file(RESULT_FULL_PATH(raefsky6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rail_1357)
{
    std::string input_file(MATRIX_FULL_PATH(rail_1357 ));
    std::string output_file(RESULT_FULL_PATH(rail_1357 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rail_20209)
{
    std::string input_file(MATRIX_FULL_PATH(rail_20209 ));
    std::string output_file(RESULT_FULL_PATH(rail_20209 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rail_5177)
{
    std::string input_file(MATRIX_FULL_PATH(rail_5177 ));
    std::string output_file(RESULT_FULL_PATH(rail_5177 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rail_79841)
{
    std::string input_file(MATRIX_FULL_PATH(rail_79841 ));
    std::string output_file(RESULT_FULL_PATH(rail_79841 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Raj1)
{
    std::string input_file(MATRIX_FULL_PATH(Raj1 ));
    std::string output_file(RESULT_FULL_PATH(Raj1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat01)
{
    std::string input_file(MATRIX_FULL_PATH(rajat01 ));
    std::string output_file(RESULT_FULL_PATH(rajat01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat02)
{
    std::string input_file(MATRIX_FULL_PATH(rajat02 ));
    std::string output_file(RESULT_FULL_PATH(rajat02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat03)
{
    std::string input_file(MATRIX_FULL_PATH(rajat03 ));
    std::string output_file(RESULT_FULL_PATH(rajat03 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat04)
{
    std::string input_file(MATRIX_FULL_PATH(rajat04 ));
    std::string output_file(RESULT_FULL_PATH(rajat04 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat05)
{
    std::string input_file(MATRIX_FULL_PATH(rajat05 ));
    std::string output_file(RESULT_FULL_PATH(rajat05 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat06)
{
    std::string input_file(MATRIX_FULL_PATH(rajat06 ));
    std::string output_file(RESULT_FULL_PATH(rajat06 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat07)
{
    std::string input_file(MATRIX_FULL_PATH(rajat07 ));
    std::string output_file(RESULT_FULL_PATH(rajat07 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat08)
{
    std::string input_file(MATRIX_FULL_PATH(rajat08 ));
    std::string output_file(RESULT_FULL_PATH(rajat08 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat09)
{
    std::string input_file(MATRIX_FULL_PATH(rajat09 ));
    std::string output_file(RESULT_FULL_PATH(rajat09 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat10)
{
    std::string input_file(MATRIX_FULL_PATH(rajat10 ));
    std::string output_file(RESULT_FULL_PATH(rajat10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat11)
{
    std::string input_file(MATRIX_FULL_PATH(rajat11 ));
    std::string output_file(RESULT_FULL_PATH(rajat11 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat12)
{
    std::string input_file(MATRIX_FULL_PATH(rajat12 ));
    std::string output_file(RESULT_FULL_PATH(rajat12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat13)
{
    std::string input_file(MATRIX_FULL_PATH(rajat13 ));
    std::string output_file(RESULT_FULL_PATH(rajat13 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat14)
{
    std::string input_file(MATRIX_FULL_PATH(rajat14 ));
    std::string output_file(RESULT_FULL_PATH(rajat14 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat15)
{
    std::string input_file(MATRIX_FULL_PATH(rajat15 ));
    std::string output_file(RESULT_FULL_PATH(rajat15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat16)
{
    std::string input_file(MATRIX_FULL_PATH(rajat16 ));
    std::string output_file(RESULT_FULL_PATH(rajat16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat17)
{
    std::string input_file(MATRIX_FULL_PATH(rajat17 ));
    std::string output_file(RESULT_FULL_PATH(rajat17 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat18)
{
    std::string input_file(MATRIX_FULL_PATH(rajat18 ));
    std::string output_file(RESULT_FULL_PATH(rajat18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat19)
{
    std::string input_file(MATRIX_FULL_PATH(rajat19 ));
    std::string output_file(RESULT_FULL_PATH(rajat19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat20)
{
    std::string input_file(MATRIX_FULL_PATH(rajat20 ));
    std::string output_file(RESULT_FULL_PATH(rajat20 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat21)
{
    std::string input_file(MATRIX_FULL_PATH(rajat21 ));
    std::string output_file(RESULT_FULL_PATH(rajat21 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat22)
{
    std::string input_file(MATRIX_FULL_PATH(rajat22 ));
    std::string output_file(RESULT_FULL_PATH(rajat22 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat23)
{
    std::string input_file(MATRIX_FULL_PATH(rajat23 ));
    std::string output_file(RESULT_FULL_PATH(rajat23 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat24)
{
    std::string input_file(MATRIX_FULL_PATH(rajat24 ));
    std::string output_file(RESULT_FULL_PATH(rajat24 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat25)
{
    std::string input_file(MATRIX_FULL_PATH(rajat25 ));
    std::string output_file(RESULT_FULL_PATH(rajat25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat26)
{
    std::string input_file(MATRIX_FULL_PATH(rajat26 ));
    std::string output_file(RESULT_FULL_PATH(rajat26 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat27)
{
    std::string input_file(MATRIX_FULL_PATH(rajat27 ));
    std::string output_file(RESULT_FULL_PATH(rajat27 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rajat28)
{
    std::string input_file(MATRIX_FULL_PATH(rajat28 ));
    std::string output_file(RESULT_FULL_PATH(rajat28 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ramage02)
{
    std::string input_file(MATRIX_FULL_PATH(ramage02 ));
    std::string output_file(RESULT_FULL_PATH(ramage02 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rbsa480)
{
    std::string input_file(MATRIX_FULL_PATH(rbsa480 ));
    std::string output_file(RESULT_FULL_PATH(rbsa480 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rbsb480)
{
    std::string input_file(MATRIX_FULL_PATH(rbsb480 ));
    std::string output_file(RESULT_FULL_PATH(rbsb480 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb1250l)
{
    std::string input_file(MATRIX_FULL_PATH(rdb1250l ));
    std::string output_file(RESULT_FULL_PATH(rdb1250l ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb1250)
{
    std::string input_file(MATRIX_FULL_PATH(rdb1250 ));
    std::string output_file(RESULT_FULL_PATH(rdb1250 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb200l)
{
    std::string input_file(MATRIX_FULL_PATH(rdb200l ));
    std::string output_file(RESULT_FULL_PATH(rdb200l ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb200)
{
    std::string input_file(MATRIX_FULL_PATH(rdb200 ));
    std::string output_file(RESULT_FULL_PATH(rdb200 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb2048_noL)
{
    std::string input_file(MATRIX_FULL_PATH(rdb2048_noL ));
    std::string output_file(RESULT_FULL_PATH(rdb2048_noL ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb2048)
{
    std::string input_file(MATRIX_FULL_PATH(rdb2048 ));
    std::string output_file(RESULT_FULL_PATH(rdb2048 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb3200l)
{
    std::string input_file(MATRIX_FULL_PATH(rdb3200l ));
    std::string output_file(RESULT_FULL_PATH(rdb3200l ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb450l)
{
    std::string input_file(MATRIX_FULL_PATH(rdb450l ));
    std::string output_file(RESULT_FULL_PATH(rdb450l ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb450)
{
    std::string input_file(MATRIX_FULL_PATH(rdb450 ));
    std::string output_file(RESULT_FULL_PATH(rdb450 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb5000)
{
    std::string input_file(MATRIX_FULL_PATH(rdb5000 ));
    std::string output_file(RESULT_FULL_PATH(rdb5000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb800l)
{
    std::string input_file(MATRIX_FULL_PATH(rdb800l ));
    std::string output_file(RESULT_FULL_PATH(rdb800l ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdb968)
{
    std::string input_file(MATRIX_FULL_PATH(rdb968 ));
    std::string output_file(RESULT_FULL_PATH(rdb968 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdist1)
{
    std::string input_file(MATRIX_FULL_PATH(rdist1 ));
    std::string output_file(RESULT_FULL_PATH(rdist1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdist2)
{
    std::string input_file(MATRIX_FULL_PATH(rdist2 ));
    std::string output_file(RESULT_FULL_PATH(rdist2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rdist3a)
{
    std::string input_file(MATRIX_FULL_PATH(rdist3a ));
    std::string output_file(RESULT_FULL_PATH(rdist3a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Reuters911)
{
    std::string input_file(MATRIX_FULL_PATH(Reuters911 ));
    std::string output_file(RESULT_FULL_PATH(Reuters911 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, RFdevice)
{
    std::string input_file(MATRIX_FULL_PATH(RFdevice ));
    std::string output_file(RESULT_FULL_PATH(RFdevice ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rgg_n_2_15_s0)
{
    std::string input_file(MATRIX_FULL_PATH(rgg_n_2_15_s0 ));
    std::string output_file(RESULT_FULL_PATH(rgg_n_2_15_s0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rgg_n_2_16_s0)
{
    std::string input_file(MATRIX_FULL_PATH(rgg_n_2_16_s0 ));
    std::string output_file(RESULT_FULL_PATH(rgg_n_2_16_s0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rgg_n_2_17_s0)
{
    std::string input_file(MATRIX_FULL_PATH(rgg_n_2_17_s0 ));
    std::string output_file(RESULT_FULL_PATH(rgg_n_2_17_s0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rgg_n_2_18_s0)
{
    std::string input_file(MATRIX_FULL_PATH(rgg_n_2_18_s0 ));
    std::string output_file(RESULT_FULL_PATH(rgg_n_2_18_s0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rgg_n_2_19_s0)
{
    std::string input_file(MATRIX_FULL_PATH(rgg_n_2_19_s0 ));
    std::string output_file(RESULT_FULL_PATH(rgg_n_2_19_s0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rim)
{
    std::string input_file(MATRIX_FULL_PATH(rim ));
    std::string output_file(RESULT_FULL_PATH(rim ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, RM07R)
{
    std::string input_file(MATRIX_FULL_PATH(RM07R ));
    std::string output_file(RESULT_FULL_PATH(RM07R ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rma10)
{
    std::string input_file(MATRIX_FULL_PATH(rma10 ));
    std::string output_file(RESULT_FULL_PATH(rma10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Roget)
{
    std::string input_file(MATRIX_FULL_PATH(Roget ));
    std::string output_file(RESULT_FULL_PATH(Roget ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rotor2)
{
    std::string input_file(MATRIX_FULL_PATH(rotor2 ));
    std::string output_file(RESULT_FULL_PATH(rotor2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rw136)
{
    std::string input_file(MATRIX_FULL_PATH(rw136 ));
    std::string output_file(RESULT_FULL_PATH(rw136 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rw496)
{
    std::string input_file(MATRIX_FULL_PATH(rw496 ));
    std::string output_file(RESULT_FULL_PATH(rw496 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, rw5151)
{
    std::string input_file(MATRIX_FULL_PATH(rw5151 ));
    std::string output_file(RESULT_FULL_PATH(rw5151 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, S10PI_n1)
{
    std::string input_file(MATRIX_FULL_PATH(S10PI_n1 ));
    std::string output_file(RESULT_FULL_PATH(S10PI_n1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, S10PI_n)
{
    std::string input_file(MATRIX_FULL_PATH(S10PI_n ));
    std::string output_file(RESULT_FULL_PATH(S10PI_n ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, s1rmq4m1)
{
    std::string input_file(MATRIX_FULL_PATH(s1rmq4m1 ));
    std::string output_file(RESULT_FULL_PATH(s1rmq4m1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, s1rmt3m1)
{
    std::string input_file(MATRIX_FULL_PATH(s1rmt3m1 ));
    std::string output_file(RESULT_FULL_PATH(s1rmt3m1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, S20PI_n1)
{
    std::string input_file(MATRIX_FULL_PATH(S20PI_n1 ));
    std::string output_file(RESULT_FULL_PATH(S20PI_n1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, S20PI_n)
{
    std::string input_file(MATRIX_FULL_PATH(S20PI_n ));
    std::string output_file(RESULT_FULL_PATH(S20PI_n ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, s2rmq4m1)
{
    std::string input_file(MATRIX_FULL_PATH(s2rmq4m1 ));
    std::string output_file(RESULT_FULL_PATH(s2rmq4m1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, s2rmt3m1)
{
    std::string input_file(MATRIX_FULL_PATH(s2rmt3m1 ));
    std::string output_file(RESULT_FULL_PATH(s2rmt3m1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, s3dkq4m2)
{
    std::string input_file(MATRIX_FULL_PATH(s3dkq4m2 ));
    std::string output_file(RESULT_FULL_PATH(s3dkq4m2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, s3dkt3m2)
{
    std::string input_file(MATRIX_FULL_PATH(s3dkt3m2 ));
    std::string output_file(RESULT_FULL_PATH(s3dkt3m2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, s3rmq4m1)
{
    std::string input_file(MATRIX_FULL_PATH(s3rmq4m1 ));
    std::string output_file(RESULT_FULL_PATH(s3rmq4m1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, s3rmt3m1)
{
    std::string input_file(MATRIX_FULL_PATH(s3rmt3m1 ));
    std::string output_file(RESULT_FULL_PATH(s3rmt3m1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, s3rmt3m3)
{
    std::string input_file(MATRIX_FULL_PATH(s3rmt3m3 ));
    std::string output_file(RESULT_FULL_PATH(s3rmt3m3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, S40PI_n1)
{
    std::string input_file(MATRIX_FULL_PATH(S40PI_n1 ));
    std::string output_file(RESULT_FULL_PATH(S40PI_n1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, S40PI_n)
{
    std::string input_file(MATRIX_FULL_PATH(S40PI_n ));
    std::string output_file(RESULT_FULL_PATH(S40PI_n ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, s4dkt3m2)
{
    std::string input_file(MATRIX_FULL_PATH(s4dkt3m2 ));
    std::string output_file(RESULT_FULL_PATH(s4dkt3m2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, S80PI_n1)
{
    std::string input_file(MATRIX_FULL_PATH(S80PI_n1 ));
    std::string output_file(RESULT_FULL_PATH(S80PI_n1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, S80PI_n)
{
    std::string input_file(MATRIX_FULL_PATH(S80PI_n ));
    std::string output_file(RESULT_FULL_PATH(S80PI_n ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, saylr1)
{
    std::string input_file(MATRIX_FULL_PATH(saylr1 ));
    std::string output_file(RESULT_FULL_PATH(saylr1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, saylr3)
{
    std::string input_file(MATRIX_FULL_PATH(saylr3 ));
    std::string output_file(RESULT_FULL_PATH(saylr3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, saylr4)
{
    std::string input_file(MATRIX_FULL_PATH(saylr4 ));
    std::string output_file(RESULT_FULL_PATH(saylr4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, SciMet)
{
    std::string input_file(MATRIX_FULL_PATH(SciMet ));
    std::string output_file(RESULT_FULL_PATH(SciMet ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, scircuit)
{
    std::string input_file(MATRIX_FULL_PATH(scircuit ));
    std::string output_file(RESULT_FULL_PATH(scircuit ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, se)
{
    std::string input_file(MATRIX_FULL_PATH(se ));
    std::string output_file(RESULT_FULL_PATH(se ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shallow_water1)
{
    std::string input_file(MATRIX_FULL_PATH(shallow_water1 ));
    std::string output_file(RESULT_FULL_PATH(shallow_water1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shallow_water2)
{
    std::string input_file(MATRIX_FULL_PATH(shallow_water2 ));
    std::string output_file(RESULT_FULL_PATH(shallow_water2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shar_te2_b3)
{
    std::string input_file(MATRIX_FULL_PATH(shar_te2-b3 ));
    std::string output_file(RESULT_FULL_PATH(shar_te2-b3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sherman1)
{
    std::string input_file(MATRIX_FULL_PATH(sherman1 ));
    std::string output_file(RESULT_FULL_PATH(sherman1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sherman2)
{
    std::string input_file(MATRIX_FULL_PATH(sherman2 ));
    std::string output_file(RESULT_FULL_PATH(sherman2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sherman3)
{
    std::string input_file(MATRIX_FULL_PATH(sherman3 ));
    std::string output_file(RESULT_FULL_PATH(sherman3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sherman4)
{
    std::string input_file(MATRIX_FULL_PATH(sherman4 ));
    std::string output_file(RESULT_FULL_PATH(sherman4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sherman5)
{
    std::string input_file(MATRIX_FULL_PATH(sherman5 ));
    std::string output_file(RESULT_FULL_PATH(sherman5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shermanACa)
{
    std::string input_file(MATRIX_FULL_PATH(shermanACa ));
    std::string output_file(RESULT_FULL_PATH(shermanACa ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shermanACb)
{
    std::string input_file(MATRIX_FULL_PATH(shermanACb ));
    std::string output_file(RESULT_FULL_PATH(shermanACb ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shermanACd)
{
    std::string input_file(MATRIX_FULL_PATH(shermanACd ));
    std::string output_file(RESULT_FULL_PATH(shermanACd ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ship_001)
{
    std::string input_file(MATRIX_FULL_PATH(ship_001 ));
    std::string output_file(RESULT_FULL_PATH(ship_001 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ship_003)
{
    std::string input_file(MATRIX_FULL_PATH(ship_003 ));
    std::string output_file(RESULT_FULL_PATH(ship_003 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shipsec1)
{
    std::string input_file(MATRIX_FULL_PATH(shipsec1 ));
    std::string output_file(RESULT_FULL_PATH(shipsec1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shipsec5)
{
    std::string input_file(MATRIX_FULL_PATH(shipsec5 ));
    std::string output_file(RESULT_FULL_PATH(shipsec5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shipsec8)
{
    std::string input_file(MATRIX_FULL_PATH(shipsec8 ));
    std::string output_file(RESULT_FULL_PATH(shipsec8 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shl_0)
{
    std::string input_file(MATRIX_FULL_PATH(shl_0 ));
    std::string output_file(RESULT_FULL_PATH(shl_0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shl_200)
{
    std::string input_file(MATRIX_FULL_PATH(shl_200 ));
    std::string output_file(RESULT_FULL_PATH(shl_200 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shl_400)
{
    std::string input_file(MATRIX_FULL_PATH(shl_400 ));
    std::string output_file(RESULT_FULL_PATH(shl_400 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shock_9)
{
    std::string input_file(MATRIX_FULL_PATH(shock-9 ));
    std::string output_file(RESULT_FULL_PATH(shock-9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shuttle_eddy)
{
    std::string input_file(MATRIX_FULL_PATH(shuttle_eddy ));
    std::string output_file(RESULT_FULL_PATH(shuttle_eddy ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shyy161)
{
    std::string input_file(MATRIX_FULL_PATH(shyy161 ));
    std::string output_file(RESULT_FULL_PATH(shyy161 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, shyy41)
{
    std::string input_file(MATRIX_FULL_PATH(shyy41 ));
    std::string output_file(RESULT_FULL_PATH(shyy41 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Si10H16)
{
    std::string input_file(MATRIX_FULL_PATH(Si10H16 ));
    std::string output_file(RESULT_FULL_PATH(Si10H16 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Si2)
{
    std::string input_file(MATRIX_FULL_PATH(Si2 ));
    std::string output_file(RESULT_FULL_PATH(Si2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Si34H36)
{
    std::string input_file(MATRIX_FULL_PATH(Si34H36 ));
    std::string output_file(RESULT_FULL_PATH(Si34H36 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Si41Ge41H72)
{
    std::string input_file(MATRIX_FULL_PATH(Si41Ge41H72 ));
    std::string output_file(RESULT_FULL_PATH(Si41Ge41H72 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Si5H12)
{
    std::string input_file(MATRIX_FULL_PATH(Si5H12 ));
    std::string output_file(RESULT_FULL_PATH(Si5H12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Si87H76)
{
    std::string input_file(MATRIX_FULL_PATH(Si87H76 ));
    std::string output_file(RESULT_FULL_PATH(Si87H76 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Sieber)
{
    std::string input_file(MATRIX_FULL_PATH(Sieber ));
    std::string output_file(RESULT_FULL_PATH(Sieber ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, SiH4)
{
    std::string input_file(MATRIX_FULL_PATH(SiH4 ));
    std::string output_file(RESULT_FULL_PATH(SiH4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, SiNa)
{
    std::string input_file(MATRIX_FULL_PATH(SiNa ));
    std::string output_file(RESULT_FULL_PATH(SiNa ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sinc12)
{
    std::string input_file(MATRIX_FULL_PATH(sinc12 ));
    std::string output_file(RESULT_FULL_PATH(sinc12 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sinc15)
{
    std::string input_file(MATRIX_FULL_PATH(sinc15 ));
    std::string output_file(RESULT_FULL_PATH(sinc15 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sinc18)
{
    std::string input_file(MATRIX_FULL_PATH(sinc18 ));
    std::string output_file(RESULT_FULL_PATH(sinc18 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, SiO2)
{
    std::string input_file(MATRIX_FULL_PATH(SiO2 ));
    std::string output_file(RESULT_FULL_PATH(SiO2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, SiO)
{
    std::string input_file(MATRIX_FULL_PATH(SiO ));
    std::string output_file(RESULT_FULL_PATH(SiO ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sit100)
{
    std::string input_file(MATRIX_FULL_PATH(sit100 ));
    std::string output_file(RESULT_FULL_PATH(sit100 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, skirt)
{
    std::string input_file(MATRIX_FULL_PATH(skirt ));
    std::string output_file(RESULT_FULL_PATH(skirt ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, SmaGri)
{
    std::string input_file(MATRIX_FULL_PATH(SmaGri ));
    std::string output_file(RESULT_FULL_PATH(SmaGri ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sme3Da)
{
    std::string input_file(MATRIX_FULL_PATH(sme3Da ));
    std::string output_file(RESULT_FULL_PATH(sme3Da ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sme3Db)
{
    std::string input_file(MATRIX_FULL_PATH(sme3Db ));
    std::string output_file(RESULT_FULL_PATH(sme3Db ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sme3Dc)
{
    std::string input_file(MATRIX_FULL_PATH(sme3Dc ));
    std::string output_file(RESULT_FULL_PATH(sme3Dc ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, smt)
{
    std::string input_file(MATRIX_FULL_PATH(smt ));
    std::string output_file(RESULT_FULL_PATH(smt ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, soc_Epinions1)
{
    std::string input_file(MATRIX_FULL_PATH(soc-Epinions1 ));
    std::string output_file(RESULT_FULL_PATH(soc-Epinions1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, soc_sign_epinions)
{
    std::string input_file(MATRIX_FULL_PATH(soc-sign-epinions ));
    std::string output_file(RESULT_FULL_PATH(soc-sign-epinions ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, soc_sign_Slashdot081106)
{
    std::string input_file(MATRIX_FULL_PATH(soc-sign-Slashdot081106 ));
    std::string output_file(RESULT_FULL_PATH(soc-sign-Slashdot081106 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, soc_sign_Slashdot090216)
{
    std::string input_file(MATRIX_FULL_PATH(soc-sign-Slashdot090216 ));
    std::string output_file(RESULT_FULL_PATH(soc-sign-Slashdot090216 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, soc_sign_Slashdot090221)
{
    std::string input_file(MATRIX_FULL_PATH(soc-sign-Slashdot090221 ));
    std::string output_file(RESULT_FULL_PATH(soc-sign-Slashdot090221 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, soc_Slashdot0811)
{
    std::string input_file(MATRIX_FULL_PATH(soc-Slashdot0811 ));
    std::string output_file(RESULT_FULL_PATH(soc-Slashdot0811 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, soc_Slashdot0902)
{
    std::string input_file(MATRIX_FULL_PATH(soc-Slashdot0902 ));
    std::string output_file(RESULT_FULL_PATH(soc-Slashdot0902 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sparsine)
{
    std::string input_file(MATRIX_FULL_PATH(sparsine ));
    std::string output_file(RESULT_FULL_PATH(sparsine ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sphere3)
{
    std::string input_file(MATRIX_FULL_PATH(sphere3 ));
    std::string output_file(RESULT_FULL_PATH(sphere3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, spiral)
{
    std::string input_file(MATRIX_FULL_PATH(spiral ));
    std::string output_file(RESULT_FULL_PATH(spiral ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, spmsrtls)
{
    std::string input_file(MATRIX_FULL_PATH(spmsrtls ));
    std::string output_file(RESULT_FULL_PATH(spmsrtls ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, srb1)
{
    std::string input_file(MATRIX_FULL_PATH(srb1 ));
    std::string output_file(RESULT_FULL_PATH(srb1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sstmodel)
{
    std::string input_file(MATRIX_FULL_PATH(sstmodel ));
    std::string output_file(RESULT_FULL_PATH(sstmodel ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Stanford)
{
    std::string input_file(MATRIX_FULL_PATH(Stanford ));
    std::string output_file(RESULT_FULL_PATH(Stanford ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, std1_Jac2_db)
{
    std::string input_file(MATRIX_FULL_PATH(std1_Jac2_db ));
    std::string output_file(RESULT_FULL_PATH(std1_Jac2_db ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, std1_Jac2)
{
    std::string input_file(MATRIX_FULL_PATH(std1_Jac2 ));
    std::string output_file(RESULT_FULL_PATH(std1_Jac2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, std1_Jac3_db)
{
    std::string input_file(MATRIX_FULL_PATH(std1_Jac3_db ));
    std::string output_file(RESULT_FULL_PATH(std1_Jac3_db ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, std1_Jac3)
{
    std::string input_file(MATRIX_FULL_PATH(std1_Jac3 ));
    std::string output_file(RESULT_FULL_PATH(std1_Jac3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, steam1)
{
    std::string input_file(MATRIX_FULL_PATH(steam1 ));
    std::string output_file(RESULT_FULL_PATH(steam1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, steam2)
{
    std::string input_file(MATRIX_FULL_PATH(steam2 ));
    std::string output_file(RESULT_FULL_PATH(steam2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, stokes128)
{
    std::string input_file(MATRIX_FULL_PATH(stokes128 ));
    std::string output_file(RESULT_FULL_PATH(stokes128 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, stokes64s)
{
    std::string input_file(MATRIX_FULL_PATH(stokes64s ));
    std::string output_file(RESULT_FULL_PATH(stokes64s ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, stokes64)
{
    std::string input_file(MATRIX_FULL_PATH(stokes64 ));
    std::string output_file(RESULT_FULL_PATH(stokes64 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, stomach)
{
    std::string input_file(MATRIX_FULL_PATH(stomach ));
    std::string output_file(RESULT_FULL_PATH(stomach ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, str_0)
{
    std::string input_file(MATRIX_FULL_PATH(str_0 ));
    std::string output_file(RESULT_FULL_PATH(str_0 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, str_200)
{
    std::string input_file(MATRIX_FULL_PATH(str_200 ));
    std::string output_file(RESULT_FULL_PATH(str_200 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, str_400)
{
    std::string input_file(MATRIX_FULL_PATH(str_400 ));
    std::string output_file(RESULT_FULL_PATH(str_400 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, str_600)
{
    std::string input_file(MATRIX_FULL_PATH(str_600 ));
    std::string output_file(RESULT_FULL_PATH(str_600 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, struct3)
{
    std::string input_file(MATRIX_FULL_PATH(struct3 ));
    std::string output_file(RESULT_FULL_PATH(struct3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, struct4)
{
    std::string input_file(MATRIX_FULL_PATH(struct4 ));
    std::string output_file(RESULT_FULL_PATH(struct4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, sts4098)
{
    std::string input_file(MATRIX_FULL_PATH(sts4098 ));
    std::string output_file(RESULT_FULL_PATH(sts4098 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, stufe_10)
{
    std::string input_file(MATRIX_FULL_PATH(stufe-10 ));
    std::string output_file(RESULT_FULL_PATH(stufe-10 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, stufe)
{
    std::string input_file(MATRIX_FULL_PATH(stufe ));
    std::string output_file(RESULT_FULL_PATH(stufe ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, swang1)
{
    std::string input_file(MATRIX_FULL_PATH(swang1 ));
    std::string output_file(RESULT_FULL_PATH(swang1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, swang2)
{
    std::string input_file(MATRIX_FULL_PATH(swang2 ));
    std::string output_file(RESULT_FULL_PATH(swang2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t2dah_a)
{
    std::string input_file(MATRIX_FULL_PATH(t2dah_a ));
    std::string output_file(RESULT_FULL_PATH(t2dah_a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t2dah_e)
{
    std::string input_file(MATRIX_FULL_PATH(t2dah_e ));
    std::string output_file(RESULT_FULL_PATH(t2dah_e ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t2dah)
{
    std::string input_file(MATRIX_FULL_PATH(t2dah ));
    std::string output_file(RESULT_FULL_PATH(t2dah ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t2dal_a)
{
    std::string input_file(MATRIX_FULL_PATH(t2dal_a ));
    std::string output_file(RESULT_FULL_PATH(t2dal_a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t2dal_bci)
{
    std::string input_file(MATRIX_FULL_PATH(t2dal_bci ));
    std::string output_file(RESULT_FULL_PATH(t2dal_bci ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t2dal_e)
{
    std::string input_file(MATRIX_FULL_PATH(t2dal_e ));
    std::string output_file(RESULT_FULL_PATH(t2dal_e ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t2dal)
{
    std::string input_file(MATRIX_FULL_PATH(t2dal ));
    std::string output_file(RESULT_FULL_PATH(t2dal ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t2d_q4)
{
    std::string input_file(MATRIX_FULL_PATH(t2d_q4 ));
    std::string output_file(RESULT_FULL_PATH(t2d_q4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t2d_q9)
{
    std::string input_file(MATRIX_FULL_PATH(t2d_q9 ));
    std::string output_file(RESULT_FULL_PATH(t2d_q9 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t3dh_a)
{
    std::string input_file(MATRIX_FULL_PATH(t3dh_a ));
    std::string output_file(RESULT_FULL_PATH(t3dh_a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t3dh_e)
{
    std::string input_file(MATRIX_FULL_PATH(t3dh_e ));
    std::string output_file(RESULT_FULL_PATH(t3dh_e ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t3dh)
{
    std::string input_file(MATRIX_FULL_PATH(t3dh ));
    std::string output_file(RESULT_FULL_PATH(t3dh ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t3dl_a)
{
    std::string input_file(MATRIX_FULL_PATH(t3dl_a ));
    std::string output_file(RESULT_FULL_PATH(t3dl_a ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t3dl_e)
{
    std::string input_file(MATRIX_FULL_PATH(t3dl_e ));
    std::string output_file(RESULT_FULL_PATH(t3dl_e ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t3dl)
{
    std::string input_file(MATRIX_FULL_PATH(t3dl ));
    std::string output_file(RESULT_FULL_PATH(t3dl ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t520)
{
    std::string input_file(MATRIX_FULL_PATH(t520 ));
    std::string output_file(RESULT_FULL_PATH(t520 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, t60k)
{
    std::string input_file(MATRIX_FULL_PATH(t60k ));
    std::string output_file(RESULT_FULL_PATH(t60k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tandem_dual)
{
    std::string input_file(MATRIX_FULL_PATH(tandem_dual ));
    std::string output_file(RESULT_FULL_PATH(tandem_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tandem_vtx)
{
    std::string input_file(MATRIX_FULL_PATH(tandem_vtx ));
    std::string output_file(RESULT_FULL_PATH(tandem_vtx ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ted_AB)
{
    std::string input_file(MATRIX_FULL_PATH(ted_AB ));
    std::string output_file(RESULT_FULL_PATH(ted_AB ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ted_AB_unscaled)
{
    std::string input_file(MATRIX_FULL_PATH(ted_AB_unscaled ));
    std::string output_file(RESULT_FULL_PATH(ted_AB_unscaled ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ted_A)
{
    std::string input_file(MATRIX_FULL_PATH(ted_A ));
    std::string output_file(RESULT_FULL_PATH(ted_A ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ted_A_unscaled)
{
    std::string input_file(MATRIX_FULL_PATH(ted_A_unscaled ));
    std::string output_file(RESULT_FULL_PATH(ted_A_unscaled ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ted_B)
{
    std::string input_file(MATRIX_FULL_PATH(ted_B ));
    std::string output_file(RESULT_FULL_PATH(ted_B ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ted_B_unscaled)
{
    std::string input_file(MATRIX_FULL_PATH(ted_B_unscaled ));
    std::string output_file(RESULT_FULL_PATH(ted_B_unscaled ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, thermal1)
{
    std::string input_file(MATRIX_FULL_PATH(thermal1 ));
    std::string output_file(RESULT_FULL_PATH(thermal1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, thermal)
{
    std::string input_file(MATRIX_FULL_PATH(thermal ));
    std::string output_file(RESULT_FULL_PATH(thermal ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, thermomech_dK)
{
    std::string input_file(MATRIX_FULL_PATH(thermomech_dK ));
    std::string output_file(RESULT_FULL_PATH(thermomech_dK ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, thermomech_dM)
{
    std::string input_file(MATRIX_FULL_PATH(thermomech_dM ));
    std::string output_file(RESULT_FULL_PATH(thermomech_dM ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, thermomech_TC)
{
    std::string input_file(MATRIX_FULL_PATH(thermomech_TC ));
    std::string output_file(RESULT_FULL_PATH(thermomech_TC ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, thermomech_TK)
{
    std::string input_file(MATRIX_FULL_PATH(thermomech_TK ));
    std::string output_file(RESULT_FULL_PATH(thermomech_TK ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, thread)
{
    std::string input_file(MATRIX_FULL_PATH(thread ));
    std::string output_file(RESULT_FULL_PATH(thread ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tols1090)
{
    std::string input_file(MATRIX_FULL_PATH(tols1090 ));
    std::string output_file(RESULT_FULL_PATH(tols1090 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tols2000)
{
    std::string input_file(MATRIX_FULL_PATH(tols2000 ));
    std::string output_file(RESULT_FULL_PATH(tols2000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tols340)
{
    std::string input_file(MATRIX_FULL_PATH(tols340 ));
    std::string output_file(RESULT_FULL_PATH(tols340 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tols4000)
{
    std::string input_file(MATRIX_FULL_PATH(tols4000 ));
    std::string output_file(RESULT_FULL_PATH(tols4000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tomography)
{
    std::string input_file(MATRIX_FULL_PATH(tomography ));
    std::string output_file(RESULT_FULL_PATH(tomography ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, torsion1)
{
    std::string input_file(MATRIX_FULL_PATH(torsion1 ));
    std::string output_file(RESULT_FULL_PATH(torsion1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, torso1)
{
    std::string input_file(MATRIX_FULL_PATH(torso1 ));
    std::string output_file(RESULT_FULL_PATH(torso1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, torso2)
{
    std::string input_file(MATRIX_FULL_PATH(torso2 ));
    std::string output_file(RESULT_FULL_PATH(torso2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, torso3)
{
    std::string input_file(MATRIX_FULL_PATH(torso3 ));
    std::string output_file(RESULT_FULL_PATH(torso3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, trans4)
{
    std::string input_file(MATRIX_FULL_PATH(trans4 ));
    std::string output_file(RESULT_FULL_PATH(trans4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, trans5)
{
    std::string input_file(MATRIX_FULL_PATH(trans5 ));
    std::string output_file(RESULT_FULL_PATH(trans5 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, transient)
{
    std::string input_file(MATRIX_FULL_PATH(transient ));
    std::string output_file(RESULT_FULL_PATH(transient ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, trdheim)
{
    std::string input_file(MATRIX_FULL_PATH(trdheim ));
    std::string output_file(RESULT_FULL_PATH(trdheim ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Trefethen_150)
{
    std::string input_file(MATRIX_FULL_PATH(Trefethen_150 ));
    std::string output_file(RESULT_FULL_PATH(Trefethen_150 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Trefethen_20000b)
{
    std::string input_file(MATRIX_FULL_PATH(Trefethen_20000b ));
    std::string output_file(RESULT_FULL_PATH(Trefethen_20000b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Trefethen_20000)
{
    std::string input_file(MATRIX_FULL_PATH(Trefethen_20000 ));
    std::string output_file(RESULT_FULL_PATH(Trefethen_20000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Trefethen_2000)
{
    std::string input_file(MATRIX_FULL_PATH(Trefethen_2000 ));
    std::string output_file(RESULT_FULL_PATH(Trefethen_2000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Trefethen_200b)
{
    std::string input_file(MATRIX_FULL_PATH(Trefethen_200b ));
    std::string output_file(RESULT_FULL_PATH(Trefethen_200b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Trefethen_200)
{
    std::string input_file(MATRIX_FULL_PATH(Trefethen_200 ));
    std::string output_file(RESULT_FULL_PATH(Trefethen_200 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Trefethen_300)
{
    std::string input_file(MATRIX_FULL_PATH(Trefethen_300 ));
    std::string output_file(RESULT_FULL_PATH(Trefethen_300 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Trefethen_500)
{
    std::string input_file(MATRIX_FULL_PATH(Trefethen_500 ));
    std::string output_file(RESULT_FULL_PATH(Trefethen_500 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Trefethen_700)
{
    std::string input_file(MATRIX_FULL_PATH(Trefethen_700 ));
    std::string output_file(RESULT_FULL_PATH(Trefethen_700 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, troll)
{
    std::string input_file(MATRIX_FULL_PATH(troll ));
    std::string output_file(RESULT_FULL_PATH(troll ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSC_OPF_1047)
{
    std::string input_file(MATRIX_FULL_PATH(TSC_OPF_1047 ));
    std::string output_file(RESULT_FULL_PATH(TSC_OPF_1047 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSC_OPF_300)
{
    std::string input_file(MATRIX_FULL_PATH(TSC_OPF_300 ));
    std::string output_file(RESULT_FULL_PATH(TSC_OPF_300 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b162_c1)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b162_c1 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b162_c1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b162_c3)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b162_c3 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b162_c3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b162_c4)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b162_c4 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b162_c4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b300_c1)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b300_c1 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b300_c1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b300_c2)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b300_c2 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b300_c2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b300_c3)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b300_c3 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b300_c3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b300)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b300 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b300 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b39_c19)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b39_c19 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b39_c19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b39_c30)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b39_c30 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b39_c30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b39_c7)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b39_c7 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b39_c7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b9_c1)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b9_c1 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b9_c1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_FS_b9_c6)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_FS_b9_c6 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_FS_b9_c6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b162_c1)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b162_c1 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b162_c1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b162_c3)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b162_c3 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b162_c3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b162_c4)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b162_c4 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b162_c4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b2052_c1)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b2052_c1 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b2052_c1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b2383_c1)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b2383_c1 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b2383_c1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b2383)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b2383 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b2383 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b300_c1)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b300_c1 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b300_c1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b300_c2)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b300_c2 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b300_c2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b300_c3)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b300_c3 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b300_c3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b39_c19)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b39_c19 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b39_c19 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b39_c30)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b39_c30 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b39_c30 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b39_c7)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b39_c7 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b39_c7 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b678_c1)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b678_c1 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b678_c1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b678_c2)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b678_c2 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b678_c2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TSOPF_RS_b9_c6)
{
    std::string input_file(MATRIX_FULL_PATH(TSOPF_RS_b9_c6 ));
    std::string output_file(RESULT_FULL_PATH(TSOPF_RS_b9_c6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, TS)
{
    std::string input_file(MATRIX_FULL_PATH(TS ));
    std::string output_file(RESULT_FULL_PATH(TS ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tsyl201)
{
    std::string input_file(MATRIX_FULL_PATH(tsyl201 ));
    std::string output_file(RESULT_FULL_PATH(tsyl201 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tub1000)
{
    std::string input_file(MATRIX_FULL_PATH(tub1000 ));
    std::string output_file(RESULT_FULL_PATH(tub1000 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tube1)
{
    std::string input_file(MATRIX_FULL_PATH(tube1 ));
    std::string output_file(RESULT_FULL_PATH(tube1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tube2)
{
    std::string input_file(MATRIX_FULL_PATH(tube2 ));
    std::string output_file(RESULT_FULL_PATH(tube2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tuma1)
{
    std::string input_file(MATRIX_FULL_PATH(tuma1 ));
    std::string output_file(RESULT_FULL_PATH(tuma1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, tuma2)
{
    std::string input_file(MATRIX_FULL_PATH(tuma2 ));
    std::string output_file(RESULT_FULL_PATH(tuma2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, turon_m)
{
    std::string input_file(MATRIX_FULL_PATH(turon_m ));
    std::string output_file(RESULT_FULL_PATH(turon_m ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, twotone)
{
    std::string input_file(MATRIX_FULL_PATH(twotone ));
    std::string output_file(RESULT_FULL_PATH(twotone ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ukerbe1_dual)
{
    std::string input_file(MATRIX_FULL_PATH(ukerbe1_dual ));
    std::string output_file(RESULT_FULL_PATH(ukerbe1_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ukerbe1)
{
    std::string input_file(MATRIX_FULL_PATH(ukerbe1 ));
    std::string output_file(RESULT_FULL_PATH(ukerbe1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, uk)
{
    std::string input_file(MATRIX_FULL_PATH(uk ));
    std::string output_file(RESULT_FULL_PATH(uk ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, USAir97)
{
    std::string input_file(MATRIX_FULL_PATH(USAir97 ));
    std::string output_file(RESULT_FULL_PATH(USAir97 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, USpowerGrid)
{
    std::string input_file(MATRIX_FULL_PATH(USpowerGrid ));
    std::string output_file(RESULT_FULL_PATH(USpowerGrid ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, usroads_48)
{
    std::string input_file(MATRIX_FULL_PATH(usroads-48 ));
    std::string output_file(RESULT_FULL_PATH(usroads-48 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, usroads)
{
    std::string input_file(MATRIX_FULL_PATH(usroads ));
    std::string output_file(RESULT_FULL_PATH(usroads ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, utm1700b)
{
    std::string input_file(MATRIX_FULL_PATH(utm1700b ));
    std::string output_file(RESULT_FULL_PATH(utm1700b ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, utm300)
{
    std::string input_file(MATRIX_FULL_PATH(utm300 ));
    std::string output_file(RESULT_FULL_PATH(utm300 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, utm3060)
{
    std::string input_file(MATRIX_FULL_PATH(utm3060 ));
    std::string output_file(RESULT_FULL_PATH(utm3060 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, utm5940)
{
    std::string input_file(MATRIX_FULL_PATH(utm5940 ));
    std::string output_file(RESULT_FULL_PATH(utm5940 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, vanbody)
{
    std::string input_file(MATRIX_FULL_PATH(vanbody ));
    std::string output_file(RESULT_FULL_PATH(vanbody ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, venkat01)
{
    std::string input_file(MATRIX_FULL_PATH(venkat01 ));
    std::string output_file(RESULT_FULL_PATH(venkat01 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, venkat25)
{
    std::string input_file(MATRIX_FULL_PATH(venkat25 ));
    std::string output_file(RESULT_FULL_PATH(venkat25 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, venkat50)
{
    std::string input_file(MATRIX_FULL_PATH(venkat50 ));
    std::string output_file(RESULT_FULL_PATH(venkat50 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, vfem)
{
    std::string input_file(MATRIX_FULL_PATH(vfem ));
    std::string output_file(RESULT_FULL_PATH(vfem ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, vibrobox)
{
    std::string input_file(MATRIX_FULL_PATH(vibrobox ));
    std::string output_file(RESULT_FULL_PATH(vibrobox ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, viscoplastic1)
{
    std::string input_file(MATRIX_FULL_PATH(viscoplastic1 ));
    std::string output_file(RESULT_FULL_PATH(viscoplastic1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, viscoplastic2)
{
    std::string input_file(MATRIX_FULL_PATH(viscoplastic2 ));
    std::string output_file(RESULT_FULL_PATH(viscoplastic2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, viscorocks)
{
    std::string input_file(MATRIX_FULL_PATH(viscorocks ));
    std::string output_file(RESULT_FULL_PATH(viscorocks ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wang1)
{
    std::string input_file(MATRIX_FULL_PATH(wang1 ));
    std::string output_file(RESULT_FULL_PATH(wang1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wang2)
{
    std::string input_file(MATRIX_FULL_PATH(wang2 ));
    std::string output_file(RESULT_FULL_PATH(wang2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wang3)
{
    std::string input_file(MATRIX_FULL_PATH(wang3 ));
    std::string output_file(RESULT_FULL_PATH(wang3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wang4)
{
    std::string input_file(MATRIX_FULL_PATH(wang4 ));
    std::string output_file(RESULT_FULL_PATH(wang4 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, water_tank)
{
    std::string input_file(MATRIX_FULL_PATH(water_tank ));
    std::string output_file(RESULT_FULL_PATH(water_tank ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wathen100)
{
    std::string input_file(MATRIX_FULL_PATH(wathen100 ));
    std::string output_file(RESULT_FULL_PATH(wathen100 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wathen120)
{
    std::string input_file(MATRIX_FULL_PATH(wathen120 ));
    std::string output_file(RESULT_FULL_PATH(wathen120 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, watt_1)
{
    std::string input_file(MATRIX_FULL_PATH(watt_1 ));
    std::string output_file(RESULT_FULL_PATH(watt_1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, watt_2)
{
    std::string input_file(MATRIX_FULL_PATH(watt_2 ));
    std::string output_file(RESULT_FULL_PATH(watt_2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, waveguide3D)
{
    std::string input_file(MATRIX_FULL_PATH(waveguide3D ));
    std::string output_file(RESULT_FULL_PATH(waveguide3D ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wave)
{
    std::string input_file(MATRIX_FULL_PATH(wave ));
    std::string output_file(RESULT_FULL_PATH(wave ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wb_cs_stanford)
{
    std::string input_file(MATRIX_FULL_PATH(wb-cs-stanford ));
    std::string output_file(RESULT_FULL_PATH(wb-cs-stanford ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, web_NotreDame)
{
    std::string input_file(MATRIX_FULL_PATH(web-NotreDame ));
    std::string output_file(RESULT_FULL_PATH(web-NotreDame ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, web_Stanford)
{
    std::string input_file(MATRIX_FULL_PATH(web-Stanford ));
    std::string output_file(RESULT_FULL_PATH(web-Stanford ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, west0381)
{
    std::string input_file(MATRIX_FULL_PATH(west0381 ));
    std::string output_file(RESULT_FULL_PATH(west0381 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, west0479)
{
    std::string input_file(MATRIX_FULL_PATH(west0479 ));
    std::string output_file(RESULT_FULL_PATH(west0479 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, west0497)
{
    std::string input_file(MATRIX_FULL_PATH(west0497 ));
    std::string output_file(RESULT_FULL_PATH(west0497 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, west0655)
{
    std::string input_file(MATRIX_FULL_PATH(west0655 ));
    std::string output_file(RESULT_FULL_PATH(west0655 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, west0989)
{
    std::string input_file(MATRIX_FULL_PATH(west0989 ));
    std::string output_file(RESULT_FULL_PATH(west0989 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, west1505)
{
    std::string input_file(MATRIX_FULL_PATH(west1505 ));
    std::string output_file(RESULT_FULL_PATH(west1505 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, west2021)
{
    std::string input_file(MATRIX_FULL_PATH(west2021 ));
    std::string output_file(RESULT_FULL_PATH(west2021 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, whitaker3_dual)
{
    std::string input_file(MATRIX_FULL_PATH(whitaker3_dual ));
    std::string output_file(RESULT_FULL_PATH(whitaker3_dual ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, whitaker3)
{
    std::string input_file(MATRIX_FULL_PATH(whitaker3 ));
    std::string output_file(RESULT_FULL_PATH(whitaker3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wiki_Vote)
{
    std::string input_file(MATRIX_FULL_PATH(wiki-Vote ));
    std::string output_file(RESULT_FULL_PATH(wiki-Vote ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, windscreen)
{
    std::string input_file(MATRIX_FULL_PATH(windscreen ));
    std::string output_file(RESULT_FULL_PATH(windscreen ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wing_nodal)
{
    std::string input_file(MATRIX_FULL_PATH(wing_nodal ));
    std::string output_file(RESULT_FULL_PATH(wing_nodal ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, wing)
{
    std::string input_file(MATRIX_FULL_PATH(wing ));
    std::string output_file(RESULT_FULL_PATH(wing ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Wordnet3)
{
    std::string input_file(MATRIX_FULL_PATH(Wordnet3 ));
    std::string output_file(RESULT_FULL_PATH(Wordnet3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, ww_vref_6405)
{
    std::string input_file(MATRIX_FULL_PATH(ww_vref_6405 ));
    std::string output_file(RESULT_FULL_PATH(ww_vref_6405 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, x104)
{
    std::string input_file(MATRIX_FULL_PATH(x104 ));
    std::string output_file(RESULT_FULL_PATH(x104 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, xenon1)
{
    std::string input_file(MATRIX_FULL_PATH(xenon1 ));
    std::string output_file(RESULT_FULL_PATH(xenon1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, xenon2)
{
    std::string input_file(MATRIX_FULL_PATH(xenon2 ));
    std::string output_file(RESULT_FULL_PATH(xenon2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, xingo3012)
{
    std::string input_file(MATRIX_FULL_PATH(xingo3012 ));
    std::string output_file(RESULT_FULL_PATH(xingo3012 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, xingo_afonso_itaipu)
{
    std::string input_file(MATRIX_FULL_PATH(xingo_afonso_itaipu ));
    std::string output_file(RESULT_FULL_PATH(xingo_afonso_itaipu ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, yeast)
{
    std::string input_file(MATRIX_FULL_PATH(yeast ));
    std::string output_file(RESULT_FULL_PATH(yeast ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, young1c)
{
    std::string input_file(MATRIX_FULL_PATH(young1c ));
    std::string output_file(RESULT_FULL_PATH(young1c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, young2c)
{
    std::string input_file(MATRIX_FULL_PATH(young2c ));
    std::string output_file(RESULT_FULL_PATH(young2c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, young3c)
{
    std::string input_file(MATRIX_FULL_PATH(young3c ));
    std::string output_file(RESULT_FULL_PATH(young3c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, young4c)
{
    std::string input_file(MATRIX_FULL_PATH(young4c ));
    std::string output_file(RESULT_FULL_PATH(young4c ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Zd_Jac2_db)
{
    std::string input_file(MATRIX_FULL_PATH(Zd_Jac2_db ));
    std::string output_file(RESULT_FULL_PATH(Zd_Jac2_db ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Zd_Jac2)
{
    std::string input_file(MATRIX_FULL_PATH(Zd_Jac2 ));
    std::string output_file(RESULT_FULL_PATH(Zd_Jac2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Zd_Jac3_db)
{
    std::string input_file(MATRIX_FULL_PATH(Zd_Jac3_db ));
    std::string output_file(RESULT_FULL_PATH(Zd_Jac3_db ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Zd_Jac3)
{
    std::string input_file(MATRIX_FULL_PATH(Zd_Jac3 ));
    std::string output_file(RESULT_FULL_PATH(Zd_Jac3 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Zd_Jac6_db)
{
    std::string input_file(MATRIX_FULL_PATH(Zd_Jac6_db ));
    std::string output_file(RESULT_FULL_PATH(Zd_Jac6_db ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Zd_Jac6)
{
    std::string input_file(MATRIX_FULL_PATH(Zd_Jac6 ));
    std::string output_file(RESULT_FULL_PATH(Zd_Jac6 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, zenios)
{
    std::string input_file(MATRIX_FULL_PATH(zenios ));
    std::string output_file(RESULT_FULL_PATH(zenios ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, zeros_nopss_13k)
{
    std::string input_file(MATRIX_FULL_PATH(zeros_nopss_13k ));
    std::string output_file(RESULT_FULL_PATH(zeros_nopss_13k ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Zewail)
{
    std::string input_file(MATRIX_FULL_PATH(Zewail ));
    std::string output_file(RESULT_FULL_PATH(Zewail ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Zhao1)
{
    std::string input_file(MATRIX_FULL_PATH(Zhao1 ));
    std::string output_file(RESULT_FULL_PATH(Zhao1 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
...
TEST(test_perf_cusparse_spmv_csr_FP32, Zhao2)
{
    std::string input_file(MATRIX_FULL_PATH(Zhao2 ));
    std::string output_file(RESULT_FULL_PATH(Zhao2 ));
    Perf_test_cusparse_spmv_csr_fp32(input_file, output_file, false);
