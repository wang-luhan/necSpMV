                                   ...
                                   perfSpB_vector vecY)
{
    ...
    // cudaMalloc(&d_vecX_exp, sizeof(float) * matA_csr->nnz);
    ...
    // cudaMemset(d_vecX_exp, 0.0, sizeof(float) * (matA_csr->nnz));
    ...
    expanded_product_kernel<<<NUM_BLOCKS, 1024>>>(d_indices, d_val, d_vecX_csr, matA_csr->nnz, d_middle);
    cuda_time_test_start();
    for (int i = 0; i < test_iter; i++)
    {
        
        summation_kernel<<<NUM_BLOCKS1, NUM_THREAD_PER_BLOCK>>>(d_ptr, d_middle, vecY->n, matA_csr->nnz, d_vecY_csr);
    }
