...
public:
    ...
    void WriteSerializedGraph(std::fstream &out)
    {
        ...
        std::streamsize index_bytes = (num_nodes + 1) * sizeof(perfSpB_index);
        std::streamsize neigh_bytes;
        if (std::is_same<DestID_, NodeID_>::value)
            neigh_bytes = edges_to_write * sizeof(perfSpB_index);
        else
            neigh_bytes = edges_to_write * sizeof(NodeWeight<perfSpB_index, int32_t>);
        ...
        perfSpB_index * offsets = g_.out_offsets();
